...
const ILU_EPOCAS = [
  ["ant", "Antigua"], ["med", "Medieval"], ["ren", "Renacimiento"], ["mod", "Moderna"], ["con", "Contemporánea"]
];
const ILU_SUBJECTS = { fil: "Filosofía 1.º", hf: "Historia de la Filosofía" };
...
function renderIluControls(){
  ...
  const subjBtns = ["all"].concat(present).map(s =>
    ...
    (s === "all" ? "Todos" : ILU_SUBJECTS[s]) + '</button>').join("");
  const epBtns = [["all", "Todas"]].concat(ILU_EPOCAS).map(e =>
  ...
  box.innerHTML =
    ...
    '<div class="fgroup"><span class="flabel">Época</span>' + epBtns + '</div>' +
    '<div class="ilu-tools"><input class="ilu-search" id="ilusearch" type="search" placeholder="Buscar pensador, obra o idea…" autocomplete="off" aria-label="Buscar en Ilustres"></div>';
...
function renderIluList(){
  ...
  if (cnt){
    cnt.hidden = false;
    cnt.textContent = rows.length + (rows.length === 1 ? " pensador" : " pensadores") + (rows.length !== total ? " (de " + total + ")" : "");
  }
  if (!rows.length){ box.innerHTML = '<p class="ilu-empty">No hay pensadores que coincidan con la búsqueda.</p>'; return; }
  const q = iluQuery;
  let html = "", last = null;
  rows.forEach(p => {
    if (p.block !== last){
...
function iluTemaChips(p){
  ...
  if (!ks.length) return "";
  return '<div class="ilu-sec ilu-sec-temas"><h3>Aparece en la teoría</h3><div class="ilu-temas">' +
...
function loadIlustre(id){
  ...
  const r = iluRetrato(p);
  box.innerHTML =
    '<div class="ilu-nav"><button class="btn ghost" data-back>← Todos los ilustres</button>' +
    '<span class="ilu-pn">' +
      (prev ? '<button class="btn ghost" data-go-ilu="' + iluEsc(prev.id) + '" title="' + iluEsc(prev.name) + '">‹ Anterior</button>' : '') +
      (next ? '<button class="btn ghost" data-go-ilu="' + iluEsc(next.id) + '" title="' + iluEsc(next.name) + '">Siguiente ›</button>' : '') +
    '</span></div>' +
    '<article class="ilu-ficha" data-b="' + p.block + '">' +
      ...
      (p.anecdota ? '<div class="ilu-sec ilu-anec"><h3><span>Anécdota</span>' + (p.tradicion ? ' <span class="ilu-trad">según la tradición</span>' : '') + '</h3>' +
        p.anecdota + (p.fuente ? '<p class="ilu-fuente"><span>Fuente</span>: ' + iluEsc(p.fuente) + '</p>' : '') + '</div>' : '') +
      ((p.obras || []).length ? '<div class="ilu-sec"><h3>Obras principales</h3><ul class="ilu-obras">' + p.obras.map(o => '<li>' + iluEsc(o) + '</li>').join("") + '</ul></div>' : '') +
      iluTemaChips(p) +
    '</article>';
...
function iluInjectTheory(){
  ...
  strip.innerHTML = '<h3>Ilustres de este tema</h3><div class="ilu-strip-list">' +
