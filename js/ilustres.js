// Generado por tools/build_subject.js (hf) — alumnado, sin material del profesor.
const ILUSTRES = {
 "homero": {
  "name": "Homero",
  "dates": "s. VIII a. C.",
  "born": -750,
  "died": -700,
  "place": "",
  "role": "poeta épico griego",
  "idea": "Sus poemas épicos fijaron la imagen griega de los dioses y los héroes, y fueron la base de la educación y del pensamiento mítico anterior a la filosofía.",
  "bio": "<p>Homero es el nombre que la tradición griega dio al poeta de la <em>Ilíada</em> y la <em>Odisea</em>, compuestas probablemente en el siglo VIII a. C. en Jonia. Casi nada seguro se sabe de su vida: varias ciudades, como Esmirna o Quíos, se disputaban su cuna, y según la tradición era un aedo ciego. Muchos estudiosos piensan que los poemas recogen una larga tradición oral que alguien fijó por escrito; es la llamada cuestión homérica.</p>\n<p>Sus poemas fueron la base de la educación griega: en ellos se aprendía la <strong>areté</strong> (excelencia) del héroe y se conocía a los dioses, que actúan con pasiones humanas y deciden el destino de los mortales. Por eso aparece en el temario como ejemplo del <strong>pensamiento mítico</strong>, frente al que reaccionan los primeros filósofos, y como blanco de la crítica de Jenófanes.</p>",
  "obras": [
   "Ilíada",
   "Odisea"
  ],
  "anecdota": "<p>Una leyenda antigua cuenta que, ya anciano, Homero se encontró en la isla de Íos con unos muchachos que volvían de pescar. Les preguntó qué habían cogido y ellos respondieron con un acertijo: «Lo que atrapamos lo dejamos; lo que no atrapamos lo llevamos encima». El poeta que lo sabía todo sobre dioses y héroes no supo resolverlo. La respuesta eran los piojos que se habían quitado sentados en la playa. Ya Heráclito aludía a esta historia para burlarse de quienes confían en la sabiduría de los poetas: hasta el más grande de los griegos se dejó engañar por lo que tenía delante.</p>",
  "fuente": "Vida de Homero atribuida a Heródoto; Certamen de Homero y Hesíodo; Heráclito, fragmento sobre Homero y los piojos",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-mito"
  ]
 },
 "hesiodo": {
  "name": "Hesíodo",
  "dates": "fl. c. 700 a. C.",
  "born": -740,
  "died": -670,
  "place": "Ascra (Beocia)",
  "role": "poeta griego",
  "idea": "Narró el origen del mundo y de los dioses como una genealogía que parte del Caos, el gran modelo de explicación mítica frente al que nacerá la filosofía.",
  "bio": "<p>Hesíodo es, junto con Homero, el gran poeta de los orígenes de la cultura griega. Vivió hacia el año 700 a. C. en Ascra, una aldea de Beocia, donde se había establecido su padre, llegado de Cime, en Asia Menor. Él mismo cuenta en sus versos que era campesino y pastor, que las Musas se le aparecieron en el monte Helicón y que tuvo un pleito con su hermano Perses por la herencia. Es el primer poeta griego que habla de sí mismo en su obra.</p>\n<p>En la <em>Teogonía</em> narra el origen del mundo y de los dioses. Al principio está el <strong>Caos</strong>; después surgen Gea (la Tierra) y generaciones de dioses y parejas de contrarios, hasta que Zeus impone su orden. Es una <strong>cosmogonía</strong> mítica: explica el mundo mediante genealogías divinas, es decir, contando quién engendró a quién. En <em>Los trabajos y los días</em> aconseja a su hermano y defiende el trabajo honrado y la justicia (<strong>diké</strong>), protegida por Zeus.</p>\n<p>Sus poemas, con los de Homero, fueron la base de la educación griega. Los primeros filósofos de Mileto pensaron en buena medida contra Hesíodo: en lugar de genealogías de dioses, buscaron un principio natural, el arjé. Aun así, heredaron de él la búsqueda de un orden y la estructura de opuestos. Jenófanes le reprochó atribuir a los dioses acciones vergonzosas.</p>",
  "obras": [
   "Teogonía",
   "Los trabajos y los días"
  ],
  "anecdota": "<p>Hesíodo cuenta en <em>Los trabajos y los días</em> que viajó a Calcis, en Eubea, para participar en los juegos fúnebres en honor de Anfidamante, y que ganó con un himno un trípode que dedicó a las Musas del Helicón. Siglos después, la tradición convirtió ese certamen en un duelo con el mismísimo Homero. Según el <em>Certamen de Homero y Hesíodo</em>, el público prefería a Homero, pero el rey que presidía los juegos dio el premio a Hesíodo: era justo que venciera quien cantaba la agricultura y la paz, y no quien cantaba la guerra y las matanzas.</p>",
  "fuente": "Hesíodo, Los trabajos y los días; Certamen de Homero y Hesíodo",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-mito"
  ]
 },
 "solon": {
  "name": "Solón",
  "dates": "c. 640 – c. 560 a. C.",
  "born": -640,
  "died": -560,
  "place": "Atenas",
  "role": "legislador y poeta ateniense",
  "idea": "Con sus reformas canceló las deudas, prohibió la esclavitud por deudas y repartió los derechos políticos según la riqueza, preparando el camino de la democracia ateniense.",
  "bio": "<p>Solón fue un político, legislador y poeta ateniense. Nació hacia el 640 a. C. en una familia noble, y los griegos lo contaron entre los Siete Sabios. Hacia el 594 a. C. fue elegido arconte, el principal magistrado de la ciudad, en plena crisis social: muchos campesinos endeudados habían perdido sus tierras o habían sido convertidos en esclavos. Defendió sus reformas en poemas de los que se conservan fragmentos.</p>\n<p>Con la <strong>seisachtheia</strong> (descarga de cargas) canceló las deudas y prohibió esclavizar a alguien por deudas. Repartió los derechos políticos según la riqueza, y no según el linaje, y abrió a todos los ciudadanos la asamblea y los tribunales. Aparece en el temario porque sus reformas, junto con las de Clístenes, prepararon la <strong>democracia</strong> ateniense del siglo V a. C.</p>",
  "obras": [
   "Elegías (fragmentos)"
  ],
  "anecdota": "<p>Heródoto relata que Solón, durante sus viajes, visitó en Sardes a Creso, el rey de Lidia, famoso por sus riquezas. Tras enseñarle sus tesoros, Creso le preguntó quién era el hombre más feliz que había conocido, esperando oír su propio nombre. Solón citó a un ateniense sencillo, Telo, que vio crecer a sus hijos y murió con honor defendiendo su ciudad. Y añadió que a nadie se le puede llamar feliz mientras vive, porque la fortuna cambia. Creso lo despidió con desprecio, aunque más tarde, tras perder su reino, recordaría sus palabras. La cronología hace dudar de que el encuentro fuera real.</p>",
  "fuente": "Heródoto, Historias I",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-sofistas"
  ]
 },
 "tales": {
  "name": "Tales de Mileto",
  "dates": "c. 624 – c. 546 a. C.",
  "born": -624,
  "died": -546,
  "place": "Mileto (Jonia, actual Turquía)",
  "role": "filósofo presocrático",
  "idea": "Considerado el primer filósofo, buscó un principio natural común a todas las cosas, el arjé, y afirmó que ese principio es el agua.",
  "bio": "<p>Tales vivió en Mileto, una próspera ciudad comercial de Jonia, en la costa de la actual Turquía, entre finales del siglo VII y mediados del VI a. C. Los griegos lo contaron entre los Siete Sabios. Según la tradición fue un sabio práctico: viajó a Egipto, donde habría aprendido geometría y astronomía, predijo el eclipse de sol del año 585 a. C. y aconsejó a los jonios en asuntos políticos. No dejó nada escrito, o nada se conserva; lo conocemos sobre todo por Aristóteles y por anécdotas de difícil comprobación.</p>\n<p>Se le considera el <strong>primer filósofo</strong> porque intentó explicar la naturaleza (<strong>physis</strong>) sin recurrir a los dioses. Se preguntó por el <strong>arjé</strong>, el principio común del que proceden todas las cosas, y respondió que es el <strong>agua</strong>. Según Aristóteles, pudo llegar a esa idea al observar que todo lo vivo necesita humedad. Lo decisivo no es la respuesta, sino el método: busca causas naturales, argumenta y deja su tesis abierta a la crítica. También se le atribuye la idea de que todo está lleno de dioses, es decir, de que la materia está animada.</p>\n<p>Con Tales empieza la escuela de Mileto, continuada por Anaximandro y Anaxímenes, que discutieron su respuesta y propusieron otros principios. Su figura simboliza el paso del <strong>mito al logos</strong>: frente al Océano divino de Homero, elige un principio observable. La tradición le atribuye además el teorema geométrico que lleva su nombre, aunque esa autoría es dudosa.</p>",
  "obras": [],
  "anecdota": "<p>Aristóteles cuenta que a Tales le reprochaban su pobreza, como si la filosofía no sirviera para nada. Para demostrar lo contrario, gracias a sus conocimientos de astronomía previó en invierno que la siguiente cosecha de aceituna sería excelente. Con el poco dinero que tenía alquiló por adelantado todas las prensas de aceite de Mileto y de Quíos, a bajo precio porque nadie más pujaba. Cuando llegó la cosecha y todos las necesitaban, las realquiló al precio que quiso y ganó una fortuna. Así mostró que los filósofos podrían enriquecerse fácilmente si quisieran, pero que su interés está en otra parte.</p>",
  "fuente": "Aristóteles, Política I",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso"
  ]
 },
 "anaximandro": {
  "name": "Anaximandro",
  "dates": "c. 610 – c. 546 a. C.",
  "born": -610,
  "died": -546,
  "place": "Mileto (Jonia, actual Turquía)",
  "role": "filósofo presocrático",
  "idea": "El principio de todas las cosas no es ningún elemento concreto, sino el ápeiron, algo ilimitado e indeterminado del que surgen y al que vuelven los contrarios.",
  "bio": "<p>Anaximandro nació en Mileto hacia el 610 a. C. y fue discípulo o compañero de Tales. Se le atribuyen el primer mapa del mundo conocido, un modelo del cosmos y la introducción en Grecia del gnomon, una varilla que sirve de reloj de sol. Escribió un libro en prosa, uno de los primeros de la literatura griega, pero no se conserva: solo ha llegado hasta nosotros un fragmento breve, citado por autores posteriores.</p>\n<p>Rechazó que el <strong>arjé</strong> fuera un elemento concreto como el agua. Según una noticia antigua, fue el primero en usar ese término. Si uno de los elementos fuera ilimitado, destruiría a los demás; por eso el principio tiene que ser el <strong>ápeiron</strong>: lo ilimitado, indefinido o indeterminado, eterno e indestructible. De él se separan los contrarios (caliente y frío, seco y húmedo), que luchan entre sí y vuelven a él. En su fragmento, ese proceso se describe como una <strong>justicia</strong> cósmica: las cosas pagan su injusticia según el orden del tiempo, lo que introduce la idea de <strong>necesidad</strong>.</p>\n<p>Anaximandro dio un paso hacia la abstracción: el principio ya no se ve ni se toca, solo se piensa. Anaxímenes, su sucesor, volvió a un elemento observable, el aire. También sostuvo que la Tierra permanece en el centro del cosmos sin apoyarse en nada y que los primeros seres humanos procedían de animales acuáticos, ideas audaces para su época.</p>",
  "obras": [],
  "anecdota": "<p>Cicerón recoge una noticia sorprendente: Anaximandro advirtió a los espartanos de que se acercaba un terremoto y les aconsejó abandonar sus casas y pasar la noche en el campo, armados. Poco después, según el relato, la ciudad se vino abajo y una parte del monte Taigeto se desprendió. Cicerón lo menciona al discutir si es posible adivinar el futuro, y lo presenta no como una profecía, sino como el fruto de la observación de la naturaleza. Sea cierta o no, la historia encaja con la imagen de Anaximandro como un investigador que buscaba causas naturales para todo lo que ocurre.</p>",
  "fuente": "Cicerón, Sobre la adivinación I",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso"
  ]
 },
 "anaximenes": {
  "name": "Anaxímenes",
  "dates": "c. 586 – c. 528 a. C.",
  "born": -586,
  "died": -528,
  "place": "Mileto (Jonia, actual Turquía)",
  "role": "filósofo presocrático",
  "idea": "El principio de todo es el aire, que por rarefacción se hace fuego y por condensación se convierte en viento, nubes, agua, tierra y piedra.",
  "bio": "<p>Anaxímenes fue el tercer gran pensador de Mileto, discípulo o compañero de Anaximandro. Vivió en el siglo VI a. C., en una época en que las ciudades jonias quedaron bajo el dominio persa. Escribió un libro en prosa sencilla, del que apenas quedan unas líneas; conocemos sus ideas sobre todo por Aristóteles y sus comentaristas.</p>\n<p>Para él, el <strong>arjé</strong> es el <strong>aire</strong>: invisible, pero presente en la experiencia, ilimitado y siempre en movimiento. Así media entre sus dos predecesores: como Tales, elige un elemento natural; como Anaximandro, uno ilimitado. Su gran aportación es explicar cómo se forman las cosas: por <strong>rarefacción</strong>, el aire se calienta y se hace fuego; por <strong>condensación</strong>, se enfría y se convierte en viento, nubes, agua, tierra y piedra. Las diferencias de cualidad se explican por diferencias de densidad.</p>\n<p>Comparaba el aire que envuelve el cosmos con el alma, que es aire y mantiene vivo el cuerpo: un ejemplo de <strong>hilozoísmo</strong>, la idea de que la materia está viva. Su propuesta de explicar los cambios por un mecanismo natural, casi cuantitativo, anticipa la ciencia posterior. Con él se cierra la escuela de Mileto; después, la reflexión sobre el arjé se desplaza a otras ciudades, de Éfeso a la Magna Grecia.</p>",
  "obras": [],
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso"
  ]
 },
 "pitagoras": {
  "name": "Pitágoras de Samos",
  "dates": "c. 570 – c. 495 a. C.",
  "born": -570,
  "died": -495,
  "place": "Samos (Grecia)",
  "role": "filósofo y matemático presocrático",
  "idea": "La esencia de la realidad es el número: el universo es un orden armónico y matemático, y el alma inmortal debe purificarse a lo largo de sucesivas reencarnaciones.",
  "bio": "<p>Pitágoras nació en la isla de Samos, frente a la costa de Jonia, hacia el 570 a. C. Hacia el 530 a. C. emigró al sur de Italia, la Magna Grecia, y fundó en Crotona una comunidad a la vez religiosa, científica y política. No escribió nada, y sus seguidores le atribuían todos sus descubrimientos, por lo que es difícil separar lo que pensó él de lo que desarrollaron los pitagóricos. Según la tradición, fue el primero en llamarse filósofo, amante del saber, y no sabio.</p>\n<p>Los pitagóricos descubrieron que las notas musicales corresponden a proporciones numéricas y generalizaron la idea: el <strong>número</strong> es el principio de todas las cosas y el universo es un <strong>cosmos</strong>, un orden armónico de estructura matemática. Enseñaron también el <strong>dualismo</strong> de alma y cuerpo y la <strong>metempsícosis</strong> o reencarnación, tomada del orfismo: el alma es inmortal, está presa en el cuerpo como en una cárcel y debe purificarse mediante la música, el estudio y la vida contemplativa. Ordenaron la realidad en una tabla de opuestos jerarquizados.</p>\n<p>Su influencia en <strong>Platón</strong> es decisiva: la matemática como modelo de conocimiento verdadero, la inmortalidad del alma, la purificación y la división de la realidad en un polo superior y otro inferior pasan a la teoría de las Ideas y a obras como el <em>Fedón</em>. El teorema que lleva su nombre ya lo conocían en la práctica los babilonios, aunque la tradición atribuye a su escuela una demostración.</p>",
  "obras": [],
  "anecdota": "<p>Un contemporáneo de Pitágoras, el poeta Jenófanes, se burló de su creencia en la reencarnación con una escena que se ha conservado. Cuenta que Pitágoras pasaba junto a un cachorro al que estaban apaleando y, conmovido, gritó: «¡Para, deja de pegarle! Es el alma de un amigo mío; la he reconocido al oír su voz». Aunque es una caricatura, es uno de los testimonios más antiguos sobre Pitágoras, y confirma que ya en vida se le asociaba con la doctrina de la <strong>transmigración</strong> de las almas, también entre animales, que explica que los pitagóricos rechazaran comer carne.</p>",
  "fuente": "Jenófanes, fragmento citado por Diógenes Laercio, Vidas VIII",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso"
  ]
 },
 "jenofanes": {
  "name": "Jenófanes de Colofón",
  "dates": "c. 570 – c. 475 a. C.",
  "born": -570,
  "died": -475,
  "place": "Colofón (Jonia, actual Turquía)",
  "role": "poeta y filósofo presocrático",
  "idea": "Cada pueblo imagina a los dioses a su imagen; frente a ese antropomorfismo, propuso un dios único que no se parece a los mortales ni en figura ni en pensamiento.",
  "bio": "<p>Jenófanes nació en Colofón, en Jonia, hacia el 570 a. C. Cuando los persas conquistaron la región, abandonó su ciudad y llevó una vida errante por el mundo griego, sobre todo por Sicilia y el sur de Italia, recitando sus poemas como un rapsoda. Según él mismo cuenta, seguía activo con más de noventa años. De su obra, escrita en verso, solo se conservan fragmentos.</p>\n<p>Es el autor de la crítica más explícita a la religión tradicional. Reprocha a Homero y Hesíodo que atribuyan a los dioses robos, adulterios y engaños, y observa que cada pueblo imagina a los dioses a su imagen: los etíopes, negros; los tracios, de ojos azules y pelo rojo. Si los bueyes y los caballos pudieran pintar, harían dioses con su forma. Denuncia así el <strong>antropomorfismo</strong>, la tendencia a dar forma humana a lo divino. Frente a ello defiende un <strong>dios único</strong>, distinto de los mortales, inmóvil, que lo gobierna todo con su mente.</p>\n<p>Su crítica es un momento clave del paso del <strong>mito al logos</strong>: lo divino deja de ser un personaje de relato para convertirse en un principio racional. También fue prudente sobre el conocimiento humano: ningún mortal alcanza una verdad segura, solo opiniones. En la Antigüedad se le consideró maestro de Parménides, aunque hoy se duda; su dios uno anticipa el ser de los eléatas y la religión universalista de los estoicos.</p>",
  "obras": [
   "Elegías (fragmentos)",
   "Silos (fragmentos)"
  ],
  "anecdota": "<p>Aristóteles cuenta que los habitantes de Elea preguntaron a Jenófanes si debían ofrecer sacrificios a Leucótea, una mujer mortal convertida en diosa marina según el mito, y a la vez llorarla en sus ritos fúnebres. Jenófanes les respondió con un argumento que no dejaba salida: si la consideraban una diosa, no debían llorarla, porque los dioses no mueren; y si la consideraban una mujer mortal, no debían ofrecerle sacrificios. La respuesta resume su crítica a la religión tradicional: aplicar la razón a los mitos revela sus contradicciones.</p>",
  "fuente": "Aristóteles, Retórica II",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-mito",
   "hf-preso"
  ]
 },
 "heraclito": {
  "name": "Heráclito de Éfeso",
  "dates": "c. 540 – c. 480 a. C.",
  "born": -540,
  "died": -480,
  "place": "Éfeso (Jonia, actual Turquía)",
  "role": "filósofo presocrático",
  "idea": "Todo fluye y nada permanece: la realidad es un cambio incesante, nacido de la lucha de contrarios pero regido por un logos o ley racional común.",
  "bio": "<p>Heráclito nació en Éfeso, ciudad jonia vecina de Mileto, hacia el 540 a. C., en una familia aristocrática. Según la tradición renunció a sus privilegios en favor de su hermano y vivió apartado, con desprecio por la multitud y por los poetas. Escribió un libro, del que se conservan más de cien fragmentos breves en forma de aforismos. Por su estilo enigmático, los antiguos lo llamaron el Oscuro.</p>\n<p>Su tesis central es que <strong>todo fluye</strong> (panta rei): la realidad es <strong>devenir</strong>, cambio incesante, como un río en el que no podemos bañarnos dos veces porque sus aguas ya son otras. El cambio nace de la <strong>lucha de contrarios</strong> (día y noche, vida y muerte), que se necesitan mutuamente. El fuego, siempre cambiante, es el principio y el símbolo de este cosmos. Pero el cambio no es caótico: lo rige un <strong>logos</strong>, una ley racional común a todo, que mantiene la medida y la armonía del conjunto, aunque la mayoría de los humanos no lo comprenda.</p>\n<p>Heráclito y Parménides representan las dos grandes respuestas al problema del cambio. <strong>Platón</strong> recogió de Heráclito la idea de que el mundo sensible está en cambio permanente, y por eso no puede ser objeto de ciencia. Su noción de logos influyó en los estoicos, y siglos después Hegel y Nietzsche lo reivindicaron como pensador del devenir.</p>",
  "obras": [
   "Sobre la naturaleza (fragmentos)"
  ],
  "anecdota": "<p>Aristóteles relata que unos visitantes llegaron a casa de Heráclito con ganas de conocer al célebre sabio. Lo encontraron calentándose junto al horno de la cocina y se quedaron parados en la puerta, sorprendidos por un lugar tan humilde. Heráclito les animó a entrar: «Pasad, que también aquí hay dioses». Aristóteles usa la anécdota para defender el estudio de los animales más modestos, porque en todas las cosas naturales hay algo admirable. Encaja con el pensamiento de Heráclito: el fuego y el logos divino están presentes en todo, también en lo más cotidiano.</p>",
  "fuente": "Aristóteles, Partes de los animales I",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso",
   "hf-platon"
  ]
 },
 "parmenides": {
  "name": "Parménides de Elea",
  "dates": "c. 515 – c. 450 a. C.",
  "born": -515,
  "died": -450,
  "place": "Elea (Magna Grecia, actual Italia)",
  "role": "filósofo presocrático",
  "idea": "El ser es y el no ser no es: lo real es único, eterno e inmóvil, y el cambio que muestran los sentidos es solo apariencia.",
  "bio": "<p>Parménides nació en Elea, colonia griega del sur de Italia (Magna Grecia), a finales del siglo VI a. C. Según Platón, siendo ya anciano visitó Atenas y conversó con un joven Sócrates, aunque el encuentro puede ser una invención literaria. Expuso su filosofía en un poema, del que se conservan fragmentos importantes.</p>\n<p>En el poema, una diosa le muestra los caminos de la investigación, que se reducen a dos. La <strong>vía de la verdad</strong> afirma que el ser es y el no ser no es: lo que no es no puede pensarse ni decirse. De ahí deduce que el <strong>ser</strong> es único, eterno, inmóvil e indivisible, porque nacer, cambiar o dividirse supondría admitir el no ser. Por eso pensar y ser son lo mismo. La <strong>vía de la opinión</strong> es la de los sentidos, que nos muestran un mundo plural y cambiante; pero ese cambio es solo <strong>apariencia</strong>. Así establece la distinción entre razón y sentidos, verdad y opinión.</p>\n<p>Parménides es el fundador de la <strong>ontología</strong>, la reflexión sobre el ser, y el gran oponente de Heráclito. Su discípulo Zenón defendió sus tesis con aporías contra el movimiento, como la de Aquiles y la tortuga. Los pluralistas (Empédocles, Anaxágoras, Demócrito) intentaron explicar el cambio sin contradecirlo. Platón recogió su idea de que lo verdaderamente real es eterno e inmutable y solo la razón lo conoce: es el antecedente del mundo de las Ideas.</p>",
  "obras": [
   "Sobre la naturaleza (poema, fragmentos)"
  ],
  "anecdota": "<p>Parménides no fue solo un pensador abstracto. Según varias fuentes antiguas, redactó las leyes de su ciudad, Elea, y sus conciudadanos quedaron tan satisfechos que cada año los magistrados juraban respetar las leyes de Parménides. Plutarco lo menciona para responder a quienes acusaban a los filósofos de no servir para la vida práctica, y Estrabón atribuye al buen gobierno de Elea, obra de Parménides y Zenón, que la ciudad resistiera frente a sus vecinos. Así, el filósofo del ser inmóvil dejó también una huella muy concreta en la <strong>polis</strong>.</p>",
  "fuente": "Plutarco, Contra Colotes; Estrabón, Geografía VI; Diógenes Laercio, Vidas IX",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso",
   "hf-platon"
  ]
 },
 "anaxagoras": {
  "name": "Anaxágoras de Clazómenas",
  "dates": "c. 500 – 428 a. C.",
  "born": -500,
  "died": -428,
  "place": "Clazómenas (Jonia, actual Turquía)",
  "role": "filósofo presocrático",
  "idea": "Todo está formado por infinitas semillas mezcladas, y una inteligencia, el Nous, puso en marcha el torbellino que separó y ordenó el cosmos.",
  "bio": "<p>Anaxágoras nació en Clazómenas, en Jonia, hacia el 500 a. C. Hacia mediados del siglo V se instaló en Atenas, adonde llevó la filosofía jonia, y fue amigo y consejero de Pericles. Sostenía que el Sol era una piedra incandescente, y por ello fue acusado de impiedad; tuvo que abandonar Atenas y murió en Lámpsaco hacia el 428 a. C. De su libro <em>Sobre la naturaleza</em> se conservan algunos fragmentos.</p>\n<p>Aceptó el principio de Parménides de que nada nace de la nada ni desaparece en la nada. Por eso explicó el cambio como mezcla y separación de infinitas <strong>semillas</strong> (spérmata), llamadas más tarde <strong>homeomerías</strong>: partículas diminutas de todas las cualidades. En cada cosa hay partes de todas las demás, y cada una parece lo que es por la porción que predomina. Así, el pan puede convertirse en carne y hueso porque ya los contiene. Lo que pone en marcha este proceso es el <strong>Nous</strong>, una mente o inteligencia que no se mezcla con nada y que imprimió al conjunto un movimiento de torbellino que lo ordenó.</p>\n<p>Anaxágoras es uno de los <strong>pluralistas</strong>, junto con Empédocles y Demócrito. Con el Nous, un principio inteligente aparece por primera vez como causa del orden del cosmos. Según cuenta Platón en el <em>Fedón</em>, Sócrates se entusiasmó con esta idea, aunque se decepcionó al ver que Anaxágoras apenas la usaba y explicaba casi todo con causas mecánicas.</p>",
  "obras": [
   "Sobre la naturaleza (fragmentos)"
  ],
  "anecdota": "<p>Se cuenta que Anaxágoras descuidó la fortuna de su familia para dedicarse a estudiar la naturaleza, y dejó que sus tierras se convirtieran en pasto de ovejas. Cuando alguien le reprochó que no se preocupara por su patria, señaló el cielo y respondió: «Me preocupo muchísimo por mi patria». En otra ocasión le preguntaron para qué merecía la pena haber nacido, y contestó: «Para contemplar el cielo y el orden de todo el universo». Aristóteles recoge esta última respuesta como ejemplo de una vida entregada al conocimiento.</p>",
  "fuente": "Diógenes Laercio, Vidas II; Aristóteles, Ética eudemia I",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso"
  ]
 },
 "empedocles": {
  "name": "Empédocles de Agrigento",
  "dates": "c. 495 – c. 435 a. C.",
  "born": -495,
  "died": -435,
  "place": "Agrigento (Sicilia)",
  "role": "filósofo presocrático",
  "idea": "Todo se compone de cuatro raíces eternas, tierra, agua, aire y fuego, que el Amor une y el Odio separa en un ciclo cósmico sin fin.",
  "bio": "<p>Empédocles nació en Agrigento, en Sicilia, hacia el 495 a. C., en una familia rica e influyente. Fue a la vez filósofo, poeta, médico y político, y según la tradición defendió la democracia en su ciudad. Se presentaba como un sabio con poderes casi divinos. Una leyenda cuenta que murió arrojándose al volcán Etna para que lo creyeran un dios, pero es un relato sin base fiable. Escribió en verso dos poemas, <em>Sobre la naturaleza</em> y <em>Purificaciones</em>, de los que quedan numerosos fragmentos.</p>\n<p>Acepta que nada nace de la nada ni perece del todo, pero rechaza que la realidad sea un único ser inmóvil. Propone cuatro <strong>raíces</strong> eternas: tierra, agua, aire y fuego. Las cosas nacen cuando estas raíces se mezclan y mueren cuando se separan. Dos fuerzas lo mueven todo: el <strong>Amor</strong>, que une, y el <strong>Odio</strong>, que separa. Su alternancia produce un <strong>ciclo cósmico</strong> en el que predomina uno u otro, y en las fases intermedias surge el mundo que conocemos. En las <em>Purificaciones</em> defiende, como los pitagóricos, la transmigración de las almas.</p>\n<p>Empédocles es uno de los <strong>pluralistas</strong>: intenta conciliar el ser de Parménides con el cambio que muestran los sentidos. Su teoría de los cuatro elementos, recogida por Aristóteles, dominó la ciencia y la medicina hasta la época moderna. También explicó la percepción por los efluvios que desprenden las cosas y que entran por los poros de los órganos de los sentidos.</p>",
  "obras": [
   "Sobre la naturaleza (fragmentos)",
   "Purificaciones (fragmentos)"
  ],
  "anecdota": "<p>Diógenes Laercio cuenta que en Agrigento soplaban unos vientos tan fuertes que arruinaban las cosechas. Empédocles mandó despellejar asnos y fabricar con su piel unos odres que colocó en las colinas y los desfiladeros para atrapar el viento. Cuando el viento amainó, sus conciudadanos empezaron a llamarlo «el que detiene los vientos». La historia, recogida de historiadores antiguos, mezcla técnica y magia, como la propia figura de Empédocles, que en sus versos prometía a sus discípulos enseñarles a calmar las tempestades y a dominar las fuerzas de la naturaleza.</p>",
  "fuente": "Diógenes Laercio, Vidas VIII",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso"
  ]
 },
 "protagoras": {
  "name": "Protágoras de Abdera",
  "dates": "c. 485 – c. 411 a. C.",
  "born": -485,
  "died": -411,
  "place": "Abdera (Tracia, actual Grecia)",
  "role": "sofista griego",
  "idea": "El ser humano es la medida de todas las cosas: no hay verdades ni valores absolutos, sino lo que a cada persona o comunidad le parece verdadero o justo.",
  "bio": "<p>Protágoras nació en Abdera, en Tracia, hacia el 485 a. C., y es el más célebre de los <strong>sofistas</strong>, los maestros itinerantes que enseñaban retórica cobrando por ello. Viajó por Grecia y pasó largas temporadas en Atenas, donde tuvo trato con Pericles, que según la tradición le encargó las leyes de la colonia de Turios. Fuentes antiguas cuentan que su obra <em>Sobre los dioses</em> provocó una acusación de impiedad; la noticia de que sus libros fueron quemados es dudosa. Solo se conservan fragmentos de sus escritos.</p>\n<p>Su tesis más famosa es el <strong>homo mensura</strong>: el hombre es la medida de todas las cosas. Significa que no hay una verdad, un bien o una justicia absolutos: lo que a cada persona o comunidad le parece verdadero o justo lo es para ella. Es la primera formulación del <strong>relativismo</strong>. Lo aplica también a la religión, con un <strong>agnosticismo</strong> prudente: no podemos saber si los dioses existen. Como las leyes son convención (nomos) y no naturaleza (physis), la sabiduría consiste en persuadir a otros de las opiniones más útiles. Por eso enseñaba a defender una tesis y su contraria.</p>\n<p>Protágoras encarna el <strong>giro antropológico</strong> de la Ilustración griega: el ser humano y la polis pasan al centro de la filosofía. Platón le dedicó un diálogo que lleva su nombre y criticó su relativismo en el <em>Teeteto</em>; Sócrates se le opuso defendiendo definiciones universales. El debate entre relativismo y verdad universal sigue abierto hoy.</p>",
  "obras": [
   "Sobre los dioses (fragmentos)",
   "La verdad (fragmentos)"
  ],
  "anecdota": "<p>Aulo Gelio relata que un joven, Evatlo, pactó con Protágoras que le pagaría sus clases de retórica cuando ganara su primer juicio. Terminó los estudios, pero no se dedicó a los tribunales ni pagó. Protágoras lo demandó y le planteó un dilema: si él ganaba el juicio, Evatlo tendría que pagar por sentencia; si ganaba Evatlo, tendría que pagar por el pacto. El alumno respondió con el dilema contrario: si ganaba, no pagaría por sentencia; si perdía, tampoco, porque aún no habría ganado ningún juicio. Los jueces, se dice, aplazaron la decisión. Es una muestra perfecta del arte de defender una tesis y su contraria.</p>",
  "fuente": "Aulo Gelio, Noches áticas V; Diógenes Laercio, Vidas IX",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-sofistas"
  ]
 },
 "gorgias": {
  "name": "Gorgias de Leontinos",
  "dates": "c. 485 – c. 380 a. C.",
  "born": -485,
  "died": -380,
  "place": "Leontinos (Sicilia)",
  "role": "sofista y maestro de retórica",
  "idea": "Nada existe, y si existiera no podría conocerse ni comunicarse; por eso el lenguaje no expresa lo que son las cosas, sino que es un poderoso instrumento de persuasión.",
  "bio": "<p>Gorgias nació en Leontinos, en Sicilia, hacia el 485 a. C. En el 427 a. C. viajó a Atenas como embajador de su ciudad para pedir ayuda contra Siracusa, y su forma de hablar causó sensación. Desde entonces enseñó retórica por toda Grecia y se hizo muy rico. Según la tradición vivió más de cien años. Se conservan dos discursos completos, el <em>Encomio de Helena</em> y la <em>Defensa de Palamedes</em>, y resúmenes de su tratado <em>Sobre el no ser</em>.</p>\n<p>En <em>Sobre el no ser</em> lleva el <strong>escepticismo</strong> al extremo con tres tesis: nada existe; si algo existiera, no podría conocerse; y si pudiera conocerse, no podría comunicarse, porque las palabras no son las cosas. Si no hay verdad que transmitir, el <strong>lenguaje</strong> no sirve para expresar lo que las cosas son, sino para mover los ánimos. De ahí su defensa de la <strong>retórica</strong>, el arte de persuadir: en el <em>Encomio de Helena</em> compara el poder de la palabra sobre el alma con el de los fármacos sobre el cuerpo.</p>\n<p>Gorgias es, con Protágoras, el sofista más influyente. Platón lo convirtió en personaje del <em>Gorgias</em>, donde Sócrates contrapone la retórica que busca el éxito a la filosofía que busca la verdad y el bien. La tradición lo asocia también a la <strong>erística</strong>, el arte de disputar por disputar. Sus reflexiones sobre el poder persuasivo del lenguaje siguen vigentes en el análisis de la propaganda y la publicidad.</p>",
  "obras": [
   "Sobre el no ser (resúmenes)",
   "Encomio de Helena",
   "Defensa de Palamedes"
  ],
  "anecdota": "<p>Gorgias era famoso por su capacidad de improvisar. Según Filóstrato, entraba en el teatro de Atenas y retaba al público: «Proponed un tema», y hablaba de él al instante, sin preparación. Platón hace referencia a esta costumbre al comienzo de su <em>Gorgias</em>, donde el sofista presume de poder responder a cualquier pregunta. Su éxito le hizo tan rico que dedicó en el santuario de Delfos una estatua de sí mismo dorada. Para Platón, ese brillo resumía el peligro de la retórica: deslumbrar al público sin preocuparse de la verdad.</p>",
  "fuente": "Filóstrato, Vidas de los sofistas I; Platón, Gorgias; Pausanias, Descripción de Grecia X",
  "tradicion": false,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-sofistas"
  ]
 },
 "socrates": {
  "name": "Sócrates",
  "dates": "470/469 – 399 a. C.",
  "born": -470,
  "died": -399,
  "place": "Atenas",
  "role": "filósofo griego",
  "idea": "Quien conoce de verdad el bien obra bien; por eso la tarea filosófica es examinar la propia vida y buscar, dialogando, definiciones universales.",
  "bio": "<p>Sócrates nació en Atenas, hijo de un escultor y de una comadrona, y vivió el esplendor y la crisis de la democracia ateniense en tiempos de Pericles y de la guerra del Peloponeso. Sirvió como soldado y pasó la vida conversando en el ágora. No escribió nada: lo conocemos por su discípulo <strong>Platón</strong>, por Jenofonte y por la comedia <em>Las nubes</em> de Aristófanes. En el 399 a. C. fue acusado de impiedad y de corromper a la juventud, y murió bebiendo cicuta.</p>\n<p>Comparte con los sofistas el interés por el ser humano y la vida en la polis, pero se opone a ellos: no cobra, prefiere el diálogo al discurso y combate el relativismo. Parte de reconocer la propia ignorancia y busca <strong>definiciones universales</strong> de conceptos como la justicia o la valentía. Su método tiene dos partes: la <strong>ironía</strong>, que lleva al interlocutor a descubrir que no sabe, y la <strong>mayéutica</strong>, que le ayuda a alumbrar la verdad por sí mismo. En ética defiende el <strong>intelectualismo moral</strong>: el mal se comete por ignorancia.</p>\n<p>Sócrates marca el giro de la filosofía hacia el ser humano, hasta el punto de que a los pensadores anteriores se les llama presocráticos. Platón recoge su búsqueda de lo universal y la convierte en la teoría de las Ideas, y de su círculo salen también cínicos y cirenaicos.</p>",
  "obras": [],
  "anecdota": "<p>Según la <em>Apología</em> de Platón, un amigo de Sócrates, Querefonte, preguntó al oráculo de Delfos si había alguien más sabio que Sócrates, y la sacerdotisa respondió que no. Sócrates quedó desconcertado, porque sabía que no era sabio, y decidió comprobarlo interrogando a políticos, poetas y artesanos que tenían fama de saber. Descubrió que todos creían saber lo que no sabían. Concluyó que el oráculo tenía razón en un sentido: él era el más sabio solo porque reconocía su ignorancia. Desde entonces vio en ese examen de los demás una misión encomendada por el dios.</p>",
  "fuente": "Platón, Apología de Sócrates; Jenofonte, Apología",
  "tradicion": false,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metodos",
   "hf-sofistas",
   "hf-platon",
   "hf-antropologia",
   "hf-etica",
   "hf-politica"
  ]
 },
 "aspasia": {
  "name": "Aspasia de Mileto",
  "dates": "c. 470 – c. 400 a. C.",
  "born": -470,
  "died": -400,
  "place": "Mileto (Jonia, actual Turquía)",
  "role": "oradora y maestra de retórica",
  "idea": "Una mujer podía dominar la palabra y el pensamiento político tanto como los varones, y enseñar retórica a los ciudadanos más influyentes de Atenas.",
  "bio": "<p>Aspasia nació en Mileto, una ciudad griega de la costa de Jonia, y se trasladó a Atenas, donde vivió como extranjera residente. Fue la compañera de <strong>Pericles</strong>, con quien tuvo un hijo, y su casa se convirtió en un lugar de encuentro de políticos, artistas y filósofos. Como en la Atenas clásica solo los varones libres eran ciudadanos, su presencia en la vida pública fue excepcional y le valió también ataques y burlas de los comediógrafos.</p>\n<p>No se conserva ningún escrito suyo: sabemos de ella por testimonios de otros autores, como Platón, Jenofonte y Plutarco, lo que obliga a separar los datos de la leyenda. Estas fuentes la presentan como una oradora brillante y una maestra de <strong>retórica</strong>, el arte de persuadir. En el <em>Menéxeno</em> de Platón, Sócrates la llama su maestra y le atribuye un discurso fúnebre; según la tradición, incluso habría ayudado a Pericles a preparar sus discursos.</p>\n<p>Aspasia aparece en el temario como ejemplo de las pocas mujeres que lograron un lugar en la filosofía antigua. Su figura muestra los límites de la democracia ateniense, que excluía a las mujeres de la <strong>isonomía</strong> y la isegoría, y la dificultad de recuperar un pensamiento femenino del que apenas quedan huellas. Se la relaciona con Sócrates y con el ambiente de los sofistas.</p>",
  "obras": [],
  "anecdota": "<p>En un diálogo perdido del socrático Esquines, del que Cicerón conservó un fragmento, Aspasia interroga a la esposa de Jenofonte: si su vecina tuviera joyas mejores que las suyas, ¿preferiría las de la vecina? Sí, responde. ¿Y si tuviera mejores vestidos? También. ¿Y si tuviera un marido mejor que el suyo? La mujer se queda callada. Aspasia hace luego lo mismo con Jenofonte y concluye que cada uno debe esforzarse por ser el mejor cónyuge posible. Cicerón cita la escena como ejemplo de razonamiento por inducción: el mismo método que se atribuye a Sócrates.</p>",
  "fuente": "Esquines de Esfeto, Aspasia (fragmento transmitido por Cicerón, Sobre la invención retórica I)",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-sofistas"
  ]
 },
 "democrito": {
  "name": "Demócrito de Abdera",
  "dates": "c. 460 – c. 370 a. C.",
  "born": -460,
  "died": -370,
  "place": "Abdera (Tracia)",
  "role": "filósofo presocrático atomista",
  "idea": "Todo lo que existe está formado por átomos indivisibles que se mueven en el vacío; sus choques y combinaciones explican todos los cambios de la naturaleza.",
  "bio": "<p>Demócrito nació en Abdera, una colonia griega de Tracia, y fue contemporáneo de Sócrates, aunque por su pensamiento se le incluye entre los presocráticos. Fue discípulo de <strong>Leucipo</strong>, con quien comparte la teoría atómica. Según la tradición, viajó mucho y llegó a una edad muy avanzada. Escribió numerosas obras sobre física, ética, matemáticas o música, pero todas se han perdido: solo conservamos fragmentos citados por otros autores.</p>\n<p>Su teoría responde al problema del cambio planteado por Parménides. La realidad está formada por <strong>átomos</strong> (del griego «indivisible»): partículas eternas e inmutables que solo se diferencian por su forma, tamaño y posición. Se mueven en el <strong>vacío</strong>, que Demócrito identifica con el no-ser y considera tan real como el ser, porque sin él no habría ni movimiento ni multiplicidad. Todo ocurre de forma mecánica, por necesidad, sin intervención de una mente ordenadora. Incluso el alma sería un conjunto de átomos muy finos, por lo que su postura es un <strong>materialismo</strong> completo. En ética propone una vida serena y moderada.</p>\n<p>Epicuro retomó su atomismo como base de su física y de su ética, y el poeta romano Lucrecio lo divulgó. Platón y Aristóteles lo criticaron. En la Edad Moderna, la ciencia recuperó la idea de átomo, y hoy se le cita como precursor del monismo materialista, frente al dualismo de Platón o Descartes.</p>",
  "obras": [],
  "anecdota": "<p>La tradición recuerda a Demócrito como «el filósofo que ríe»: se reía de las preocupaciones y ambiciones humanas, que le parecían ridículas, frente a Heráclito, que lloraba por ellas. Unas cartas antiguas atribuidas a Hipócrates, que no son auténticas, cuentan que sus vecinos de Abdera creyeron que había enloquecido y llamaron al célebre médico para curarlo. Hipócrates lo encontró estudiando la anatomía de unos animales, conversó con él y concluyó que Demócrito era el más cuerdo de la ciudad. La leyenda refleja su ideal de <strong>serenidad</strong> ante los asuntos humanos.</p>",
  "fuente": "Séneca, Sobre la ira II; Cartas pseudohipocráticas",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-preso"
  ]
 },
 "hipias": {
  "name": "Hipias de Élide",
  "dates": "c. 460 – c. 400 a. C.",
  "born": -460,
  "died": -400,
  "place": "Élide (Peloponeso)",
  "role": "sofista griego",
  "idea": "Las leyes de cada ciudad son convenciones humanas, variables y cambiables, distintas de las leyes naturales, que son universales y valen igual para todos.",
  "bio": "<p>Hipias nació en Élide, la región del Peloponeso donde se celebraban los Juegos Olímpicos, y fue contemporáneo de Sócrates. Su ciudad lo envió en varias ocasiones como embajador, y aprovechaba sus viajes para dar lecciones y conferencias cobrando por ellas, como los demás <strong>sofistas</strong>. Era famoso por su saber enciclopédico: enseñaba matemáticas, astronomía, música, historia y poesía, y presumía de una memoria prodigiosa. Sus obras se han perdido; lo conocemos sobre todo por los diálogos de Platón <em>Hipias mayor</em>, <em>Hipias menor</em> y <em>Protágoras</em>, que lo retratan con cierta ironía.</p>\n<p>Su aportación más importante al temario es la distinción entre <strong>physis</strong> y <strong>nomos</strong>. Las leyes naturales son inmutables y universales; las leyes establecidas por los seres humanos, en cambio, son convenciones que varían de una comunidad a otra. Si las leyes son convención, pueden cambiarse. En el <em>Protágoras</em>, Platón le hace decir que la ley es un tirano que obliga a muchas cosas contrarias a la naturaleza, y que los seres humanos son parientes por naturaleza, no por ley.</p>\n<p>Esta idea abre un debate central en la filosofía política: si las normas son naturales o pactadas. Su convencionalismo enlaza con el relativismo de Protágoras, y Sócrates y Platón lo combatieron buscando una justicia universal. La idea de un parentesco natural entre todos los seres humanos anticipa el cosmopolitismo de cínicos y estoicos.</p>",
  "obras": [],
  "anecdota": "<p>En el <em>Hipias menor</em>, Platón hace recordar a Sócrates una aparición célebre de Hipias en los Juegos Olímpicos. El sofista presumió ante todos de que cuanto llevaba encima lo había hecho él mismo: el anillo y el sello que tallaba, el frasco del aceite, el rascador para el baño, las sandalias, el manto y la túnica, e incluso el cinturón bordado. Además traía poemas, tragedias y discursos de su propia cosecha. Platón lo cuenta con ironía, pero la escena retrata bien el ideal de saber enciclopédico y autosuficiente del que Hipias se enorgullecía.</p>",
  "fuente": "Platón, Hipias menor",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-sofistas"
  ]
 },
 "antistenes": {
  "name": "Antístenes",
  "dates": "c. 445 – c. 365 a. C.",
  "born": -445,
  "died": -365,
  "place": "Atenas",
  "role": "filósofo socrático",
  "idea": "La virtud basta para ser feliz; para alcanzarla hay que llevar una vida austera, dominar los placeres y no depender de riquezas ni honores.",
  "bio": "<p>Antístenes nació en Atenas. Fue primero alumno del sofista Gorgias, del que aprendió retórica, y después se convirtió en uno de los discípulos más fieles de <strong>Sócrates</strong>: según Platón, estuvo presente en su muerte. Enseñó en el gimnasio del Cinosargo, en las afueras de Atenas. Escribió muchas obras, pero todas se han perdido y solo quedan fragmentos y testimonios.</p>\n<p>Tomó de Sócrates la idea de que la virtud se puede enseñar y la exageró: la virtud basta para la felicidad, y todo lo demás (riqueza, fama, placer) es indiferente o dañino. Por eso defendía una vida austera y el <strong>autodominio</strong>. La tradición lo considera fundador del <strong>cinismo</strong>, que Diógenes de Sinope llevó al extremo, y así aparece en el temario, aunque algunos historiadores discuten ese vínculo directo.</p>",
  "obras": [],
  "anecdota": "<p>Diógenes Laercio cuenta que Antístenes recorría cada día a pie unos cuarenta estadios, más de siete kilómetros, desde el Pireo hasta Atenas para escuchar a Sócrates. Imitaba la austeridad de su maestro hasta el extremo, y solía llevar un manto viejo y roto, con el lado agujereado bien a la vista. Un día Sócrates se dio cuenta y le dijo: «A través de los agujeros de tu manto veo tu vanidad». La anécdota muestra el riesgo de convertir la pobreza en espectáculo, algo que los cínicos posteriores llevarían todavía más lejos.</p>",
  "fuente": "Diógenes Laercio, Vidas VI",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-helenismo"
  ]
 },
 "platon": {
  "name": "Platón",
  "dates": "c. 427 – 347 a. C.",
  "born": -427,
  "died": -347,
  "place": "Atenas",
  "role": "filósofo griego",
  "idea": "La realidad verdadera son las Ideas, eternas e inmutables, que solo capta la razón; el mundo sensible que percibimos es solo su copia imperfecta.",
  "bio": "<p>Platón nació en Atenas en una familia aristocrática, durante la guerra del Peloponeso. Fue discípulo de <strong>Sócrates</strong>, y su condena a muerte en el 399 a. C. lo apartó de la política activa. Viajó por el Mediterráneo, conoció a los pitagóricos en el sur de Italia e intentó sin éxito influir en el gobierno de Siracusa. Hacia el 387 a. C. fundó en Atenas la <strong>Academia</strong>, la primera escuela filosófica estable de Occidente. Escribió en forma de diálogos, casi siempre con Sócrates como protagonista.</p>\n<p>Su filosofía se basa en un <strong>dualismo ontológico</strong>: el mundo sensible, cambiante y múltiple, es copia del <strong>mundo de las Ideas</strong>, eterno e inmutable, presidido por la Idea del Bien. Conocer es ascender de la opinión (<em>doxa</em>) a la ciencia (<em>episteme</em>), como explica la alegoría de la caverna, y también recordar lo que el alma contempló antes de nacer (reminiscencia). El ser humano es un alma inmortal, con tres partes (racional, irascible y concupiscible), encerrada en un cuerpo. En la <em>República</em> propone una ciudad justa en la que cada clase cumple su función y gobiernan los filósofos.</p>\n<p>Su influencia es enorme. Su discípulo Aristóteles lo criticó desde el realismo. El neoplatonismo, Agustín de Hipona y buena parte del pensamiento cristiano lo adaptaron, y el racionalismo de Descartes retoma su dualismo. Nietzsche, en cambio, lo consideró el origen de una desvalorización de la vida.</p>",
  "obras": [
   "República",
   "Fedón",
   "Banquete",
   "Fedro"
  ],
  "anecdota": "<p>Diógenes Laercio cuenta que el joven Platón tenía ambiciones literarias: escribía poemas y tragedias. Se disponía a presentar una tragedia a concurso cuando escuchó a Sócrates hablar ante el teatro de Dioniso. Quedó tan impresionado que arrojó sus versos al fuego, parodiando un verso de Homero: «Ven aquí, Hefesto, que Platón te necesita». Desde entonces se dedicó a la filosofía. Sin embargo, no dejó de ser un gran escritor: sus diálogos, con personajes, escenas y mitos, conservan mucho del arte dramático que parecía abandonar.</p>",
  "fuente": "Diógenes Laercio, Vidas III",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-historicidad",
   "hf-metodos",
   "hf-preso",
   "hf-sofistas",
   "hf-platon",
   "hf-antropologia",
   "hf-etica",
   "hf-politica",
   "hf-medieval",
   "hf-fe-razon",
   "hf-sospecha",
   "hf-descartes-makro"
  ]
 },
 "diogenes": {
  "name": "Diógenes de Sinope",
  "dates": "c. 412 – 323 a. C.",
  "born": -412,
  "died": -323,
  "place": "Sinope (actual Turquía)",
  "role": "filósofo cínico",
  "idea": "Para ser libre y feliz hay que necesitar lo mínimo, vivir conforme a la naturaleza y despreciar las convenciones sociales, la riqueza y el poder.",
  "bio": "<p>Diógenes nació en Sinope, una colonia griega a orillas del mar Negro. Según la tradición, tuvo que marcharse de su ciudad tras un escándalo por falsificación de moneda en el que estaba implicado su padre, y se instaló en Atenas, donde habría seguido las enseñanzas de Antístenes. Pasó sus últimos años en Corinto. Casi todo lo que sabemos de él procede de anécdotas recogidas siglos después por Diógenes Laercio, que conviene leer como tradición y no como historia segura. No se conserva ninguna de las obras que se le atribuyen.</p>\n<p>Es la figura más representativa del <strong>cinismo</strong>, nombre que viene del griego <em>kynos</em>, «perro», porque los cínicos vivían sin pudor ni convenciones, como los perros. Su ideal es la <strong>autarquía</strong>, la autosuficiencia: cuanto menos se necesita, más libre y feliz se es. Por eso rechazaba la riqueza, la fama y el poder como necesidades artificiales y defendía vivir conforme a la naturaleza. La tradición cuenta que vivía en una gran tinaja, que buscaba con una lámpara «un hombre» en pleno día y que pidió a Alejandro Magno que se apartara porque le tapaba el sol. Cuando le preguntaban de dónde era, respondía que era ciudadano del mundo.</p>\n<p>Su discípulo Crates fue maestro de Zenón de Citio, y los estoicos heredaron del cinismo el ideal de autosuficiencia y el <strong>cosmopolitismo</strong>. Su crítica provocadora de las convenciones ha inspirado a pensadores posteriores.</p>",
  "obras": [],
  "anecdota": "<p>Diógenes Laercio relata que Platón había definido al ser humano como «un animal bípedo sin plumas», y la definición tuvo éxito en la Academia. Diógenes desplumó un gallo, lo llevó a la escuela y lo soltó ante todos diciendo: «Aquí tenéis al hombre de Platón». Según el relato, a la definición se le añadió después «de uñas anchas». La escena resume el estilo cínico: en lugar de discutir con largos argumentos, Diógenes ridiculiza con un gesto la filosofía abstracta y muestra que las definiciones pueden alejarse mucho de la realidad.</p>",
  "fuente": "Diógenes Laercio, Vidas VI",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-helenismo"
  ]
 },
 "aristoteles": {
  "name": "Aristóteles",
  "dates": "384 – 322 a. C.",
  "born": -384,
  "died": -322,
  "place": "Estagira (Macedonia)",
  "role": "filósofo y científico griego",
  "idea": "La realidad está en las sustancias concretas, compuestas de materia y forma; la felicidad consiste en realizar nuestra naturaleza racional mediante la virtud y la vida en comunidad.",
  "bio": "<p>Aristóteles nació en Estagira, en el norte de Grecia, hijo de un médico de la corte macedonia. A los diecisiete años entró en la <strong>Academia</strong> de Platón, donde permaneció unos veinte años. Tras la muerte de su maestro viajó por Asia Menor y fue preceptor del joven Alejandro Magno. En el 335 a. C. fundó en Atenas su propia escuela, el <strong>Liceo</strong>. Investigó casi todos los campos del saber, de la lógica a la biología, y murió en Calcis poco después de abandonar Atenas.</p>\n<p>Frente al dualismo de Platón, defiende que la realidad está en las sustancias concretas, compuestas de materia y forma (<strong>hilemorfismo</strong>), y que todo conocimiento empieza por los sentidos. El alma es la forma del cuerpo vivo, no una realidad separada. Fundó la lógica con la teoría del silogismo y explicó el cambio mediante la potencia, el acto y las cuatro causas. En ética, el fin último es la <strong>eudaimonía</strong> o felicidad, que se alcanza con la virtud, un hábito que busca el término medio. En política, el ser humano es un animal político (<em>zoon politikón</em>) que solo se realiza en la polis.</p>\n<p>Su obra dominó la ciencia durante siglos. A través de pensadores árabes como Averroes llegó a la Edad Media, y Tomás de Aquino la armonizó con la fe cristiana. La revolución científica de Galileo y Descartes rompió con su física, pero su ética de la virtud y su lógica siguen muy presentes.</p>",
  "obras": [
   "Metafísica",
   "Ética a Nicómaco",
   "Política",
   "Acerca del alma"
  ],
  "anecdota": "<p>A la muerte de Alejandro Magno, en el 323 a. C., creció en Atenas la hostilidad contra los macedonios y contra quienes se relacionaban con ellos. Aristóteles, antiguo preceptor de Alejandro, fue acusado de impiedad, como Sócrates décadas antes. Según las biografías antiguas, decidió marcharse a Calcis y explicó que no quería que los atenienses pecaran dos veces contra la filosofía. La frase, quizá embellecida por la tradición, compara su situación con la condena de Sócrates y muestra que prefería salvar su vida y su trabajo antes que convertirse en mártir.</p>",
  "fuente": "Eliano, Historias curiosas III; biografías antiguas de Aristóteles",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metodos",
   "hf-mito",
   "hf-antropologia",
   "hf-etica",
   "hf-politica",
   "hf-modernidad",
   "hf-contrato",
   "hf-utilitarismo",
   "hf-descartes-makro"
  ]
 },
 "pirron": {
  "name": "Pirrón de Elis",
  "dates": "c. 360 – c. 270 a. C.",
  "born": -360,
  "died": -270,
  "place": "Elis (Peloponeso)",
  "role": "filósofo escéptico",
  "idea": "A toda afirmación puede oponerse otra igual de convincente; por eso el sabio suspende el juicio y así alcanza la tranquilidad del alma.",
  "bio": "<p>Pirrón nació en Elis, en el Peloponeso, y según las fuentes antiguas fue pintor antes de dedicarse a la filosofía. Acompañó a Alejandro Magno en su expedición a Oriente y llegó hasta la India, donde, según la tradición, conoció a los sabios ascetas que los griegos llamaban gimnosofistas. De vuelta en su ciudad llevó una vida sencilla y fue muy respetado. No escribió nada: conocemos su pensamiento por su discípulo Timón de Fliunte y, sobre todo, por autores posteriores como Sexto Empírico.</p>\n<p>Pirrón es el fundador del <strong>escepticismo</strong>, la postura que duda de que podamos alcanzar un conocimiento seguro. No sabemos cómo son las cosas en sí mismas; solo cómo se nos aparecen, y a cada razón se le puede oponer otra con la misma fuerza. Por eso el sabio practica la <strong>epojé</strong> o suspensión del juicio: no afirma ni niega nada. De esa actitud nace la <strong>ataraxia</strong>, la paz interior, porque quien no se aferra a ninguna opinión no sufre por defenderla. Como las demás escuelas helenísticas, su objetivo último es práctico: vivir con serenidad.</p>\n<p>El escepticismo antiguo, recuperado en el Renacimiento a través de Sexto Empírico, influyó en Montaigne y fue el adversario que <strong>Descartes</strong> quiso vencer: la duda cartesiana es metódica, un instrumento para llegar a la certeza, mientras que la de Pirrón es un fin. También se relaciona con el escepticismo moderno de Hume.</p>",
  "obras": [],
  "anecdota": "<p>Diógenes Laercio relata que Pirrón viajaba en barco cuando estalló una tempestad. Mientras sus compañeros se desesperaban, él señaló un cerdito que seguía comiendo tranquilamente en la cubierta y les dijo que el sabio debía mantenerse tan imperturbable como aquel animal. Otras anécdotas de la misma fuente exageran su indiferencia, hasta decir que sus amigos tenían que apartarlo de los carros y los precipicios, aunque ya en la Antigüedad se discutía su veracidad. El cerdito, en cambio, ilustra bien el ideal escéptico de la <strong>ataraxia</strong>, la calma que nace de no aferrarse a ninguna opinión.</p>",
  "fuente": "Diógenes Laercio, Vidas IX",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-helenismo",
   "hf-descartes-makro"
  ]
 },
 "epicuro": {
  "name": "Epicuro",
  "dates": "341 – 270 a. C.",
  "born": -341,
  "died": -270,
  "place": "Samos",
  "role": "filósofo griego helenístico",
  "idea": "La felicidad es el placer entendido como ausencia de dolor en el cuerpo y de turbación en el alma, que se logra con moderación, amistad y sin miedos.",
  "bio": "<p>Epicuro nació en la isla de Samos, en una familia ateniense. Conoció de joven la filosofía atomista de Demócrito y enseñó en varias ciudades de Asia Menor. Hacia el 306 a. C. se estableció en Atenas y fundó una escuela en una casa con huerto, por lo que se la conoce como el <strong>Jardín</strong>. Admitía a mujeres y esclavos, algo poco habitual, y la amistad era la base de la vida en común. Escribió mucho, pero solo se conservan tres cartas y algunas colecciones de máximas.</p>\n<p>Para Epicuro, el bien supremo es el <strong>placer</strong>, pero no el desenfreno: el placer verdadero es la ausencia de dolor en el cuerpo (<em>aponía</em>) y de turbación en el alma (<strong>ataraxia</strong>). Hay que satisfacer los deseos naturales y necesarios y moderar los demás. Su física, basada en los átomos de Demócrito, sirve para liberar de los miedos. El <strong>tetrafármaco</strong> o cuádruple remedio lo resume: no hay que temer a los dioses, que no se ocupan de nosotros; ni a la muerte, porque cuando ella está nosotros ya no estamos; el bien es fácil de conseguir y el mal, fácil de soportar.</p>\n<p>Su escuela duró siglos, y el poeta romano Lucrecio difundió sus ideas en <em>De la naturaleza de las cosas</em>. Se diferencia del hedonismo inmediato de Aristipo y del ideal de apatía de los estoicos. Su visión del placer como criterio moral anticipa en parte al utilitarismo moderno.</p>",
  "obras": [
   "Carta a Meneceo",
   "Carta a Heródoto",
   "Máximas capitales"
  ],
  "anecdota": "<p>Séneca cuenta que en la puerta del Jardín de Epicuro podía leerse una invitación: «Forastero, aquí estarás bien; aquí el bien supremo es el placer». Pero quien entraba esperando un banquete encontraba a un anfitrión amable que le ofrecía pan y agua. El propio Epicuro, en una carta citada por Diógenes Laercio, pide a un amigo que le envíe un tarrito de queso para poder darse un festín cuando quisiera. Ambas escenas desmienten la imagen de Epicuro como un vividor: para él, el placer está en la sencillez y en la ausencia de dolor.</p>",
  "fuente": "Séneca, Cartas a Lucilio 21; Diógenes Laercio, Vidas X",
  "tradicion": false,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-helenismo"
  ]
 },
 "zenon": {
  "name": "Zenón de Citio",
  "dates": "c. 334 – c. 262 a. C.",
  "born": -334,
  "died": -262,
  "place": "Citio (Chipre)",
  "role": "filósofo fundador del estoicismo",
  "idea": "El universo está ordenado por una razón divina; el sabio acepta el destino, domina sus pasiones y solo se preocupa de lo que depende de él.",
  "bio": "<p>Zenón nació en Citio, en la isla de Chipre, y se dedicó al comercio como su padre. Según la tradición, un naufragio lo llevó a Atenas, donde descubrió la filosofía leyendo sobre Sócrates. Allí fue discípulo del cínico <strong>Crates</strong> y de otros maestros. Hacia el 300 a. C. empezó a enseñar en la <strong>Stoa Poikile</strong> (el «pórtico pintado») del ágora, y de ese pórtico recibe su nombre la escuela estoica. Escribió varias obras, entre ellas una <em>República</em>, pero todas se han perdido y solo quedan fragmentos.</p>\n<p>Para Zenón, el universo está gobernado por el <strong>logos</strong>, una razón divina que lo ordena todo, de modo que nada ocurre por azar: todo sucede según el destino. La sabiduría consiste en vivir conforme a la naturaleza y a la razón, aceptando lo que no depende de nosotros. El sabio alcanza la <strong>apatía</strong>, que no es indiferencia, sino ausencia de las pasiones que perturban, y con ella la paz interior. Como todos participamos del mismo logos, todos los seres humanos somos ciudadanos de un único mundo: es el <strong>cosmopolitismo</strong>.</p>\n<p>Tomó del cinismo la austeridad y la idea de autosuficiencia, pero las integró en un sistema con física y lógica. Crisipo desarrolló después la doctrina, y en Roma la continuaron Séneca, Epicteto y Marco Aurelio. El estoicismo influyó en el pensamiento cristiano y en la idea moderna de derechos universales.</p>",
  "obras": [],
  "anecdota": "<p>Diógenes Laercio relata que el cínico Crates, primer maestro de Zenón, quiso curarlo de su timidez y de su excesivo sentido del ridículo. Le dio una olla de lentejas y le ordenó llevarla por el Cerámico, un barrio concurrido de Atenas. Al ver que Zenón intentaba esconderla, Crates rompió la olla de un golpe con su bastón, y las lentejas se le derramaron por encima. Mientras Zenón huía avergonzado, Crates le gritó: «¿Por qué huyes? No te ha pasado nada terrible». Era una lección cínica que el estoicismo conservaría: no dejarse dominar por la opinión ajena.</p>",
  "fuente": "Diógenes Laercio, Vidas VII",
  "tradicion": true,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-helenismo"
  ]
 },
 "tertuliano": {
  "name": "Tertuliano",
  "dates": "c. 155 – c. 220",
  "born": 155,
  "died": 220,
  "place": "Cartago (actual Túnez)",
  "role": "apologista cristiano",
  "idea": "La fe cristiana no necesita el apoyo de la filosofía griega: lo revelado debe creerse aunque la razón humana no lo comprenda o lo juzgue absurdo.",
  "bio": "<p>Quinto Septimio Florente Tertuliano nació en Cartago, en el norte de África romano, hacia el año 155. Recibió una sólida formación en retórica y derecho y se convirtió al cristianismo en su madurez. Fue el primer gran autor cristiano que escribió en latín, y contribuyó a crear el vocabulario teológico de Occidente: a él se debe, por ejemplo, el uso del término <strong>Trinidad</strong>. En sus últimos años se acercó al montanismo, un movimiento rigorista que la Iglesia acabó rechazando.</p>\n<p>Tertuliano fue un <strong>apologista</strong>, es decir, un defensor del cristianismo frente a las acusaciones de los paganos. Ante la relación entre fe y razón adoptó la postura más desconfiada: la filosofía griega, a su juicio, era fuente de herejías. Por eso se preguntaba qué tenía que ver Atenas, símbolo de la filosofía, con Jerusalén, símbolo de la fe. Se le atribuye la fórmula <strong>«Creo porque es absurdo»</strong>, que no aparece literalmente en sus obras, pero resume bien su idea de que las verdades reveladas, como la encarnación o la resurrección, deben aceptarse aunque parezcan imposibles a la razón.</p>\n<p>Su postura, llamada a veces <strong>fideísmo</strong>, representa un extremo en el debate medieval. Frente a ella, Agustín de Hipona propondrá que la fe guíe a la razón, y Tomás de Aquino, que ambas colaboren. Su influencia fue grande en la teología latina posterior.</p>",
  "obras": [
   "Apologético (197)",
   "Sobre la prescripción de los herejes",
   "Sobre la carne de Cristo"
  ],
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-fe-razon"
  ]
 },
 "plotino": {
  "name": "Plotino",
  "dates": "205 – 270",
  "born": 205,
  "died": 270,
  "place": "Licópolis (Egipto)",
  "role": "filósofo neoplatónico",
  "idea": "Toda la realidad procede del Uno por emanación necesaria, en grados decrecientes de perfección, y el alma puede volver a él mediante la contemplación y la purificación.",
  "bio": "<p>Plotino nació en Egipto hacia el año 205; según su biógrafo Porfirio, prefería no hablar de su origen. Estudió filosofía en Alejandría con Amonio Saccas durante once años. Después acompañó una expedición militar a Persia para conocer la sabiduría oriental y, hacia el año 244, se instaló en Roma, donde fundó una escuela muy prestigiosa. Murió en Campania en el año 270. Su discípulo Porfirio ordenó sus escritos en seis grupos de nueve tratados: las <em>Enéadas</em>.</p>\n<p>Plotino es el fundador del <strong>neoplatonismo</strong>, una reinterpretación de Platón que une filosofía y experiencia religiosa. En la cima de la realidad sitúa al <strong>Uno</strong>, principio impersonal, perfecto e indescriptible. Del Uno surge, por <strong>emanación</strong>, la Inteligencia (<em>Nous</em>), donde residen las Ideas; de ella, el Alma del mundo; y, en el último grado, la materia. La emanación no es un acto libre, sino un desbordamiento necesario y eterno, como la luz que irradia del sol. La materia, lo más alejado del Uno, es privación de ser y origen del mal. El alma humana puede recorrer el camino inverso y unirse al Uno mediante la contemplación.</p>\n<p>Su influencia fue enorme. Agustín de Hipona leyó a los neoplatónicos antes de su conversión y tomó de ellos la idea del mal como privación, aunque sustituyó la emanación por una creación libre de un Dios personal. Hipatia de Alejandría enseñó dentro de esta corriente, que marcó también la filosofía islámica y el Renacimiento.</p>",
  "obras": [
   "Enéadas"
  ],
  "anecdota": "<p>Porfirio cuenta que Plotino parecía avergonzarse de tener cuerpo, y por eso se negó siempre a posar para un retrato. Cuando su discípulo Amelio se lo pidió, respondió: ¿no basta con cargar la imagen con que la naturaleza nos ha envuelto, para encima dejar una imagen de esa imagen? Amelio no se rindió: llevó a sus clases a un pintor, Carterio, que lo observó durante varias sesiones y lo pintó de memoria. La negativa encaja con su filosofía: lo sensible es solo un reflejo pálido de la verdadera realidad.</p>",
  "fuente": "Porfirio, Vida de Plotino",
  "tradicion": false,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-medieval",
   "hf-fe-razon"
  ]
 },
 "agustin": {
  "name": "Agustín de Hipona",
  "dates": "354 – 430",
  "born": 354,
  "died": 430,
  "place": "Tagaste (actual Argelia)",
  "role": "filósofo y teólogo cristiano",
  "idea": "La verdad no se busca fuera sino en el interior del alma, donde Dios la ilumina; la fe guía a la razón para que pueda comprender.",
  "bio": "<p>Agustín nació en Tagaste, en el norte de África romano, en el año 354, hijo de un padre pagano y de una madre cristiana, Mónica. Estudió retórica en Cartago y la enseñó en Roma y Milán. Durante su juventud se adhirió al maniqueísmo y pasó por el escepticismo, hasta que la lectura de los neoplatónicos y los sermones del obispo Ambrosio lo llevaron a convertirse al cristianismo en el año 386. Fue obispo de Hipona desde el 395 hasta su muerte, en el 430.</p>\n<p>Es la gran figura de la <strong>patrística</strong>. Su lema «cree para entender» expresa que la fe guía y la razón comprende. Busca a Dios en el interior del alma: al volverse hacia sí misma, el alma recibe la <strong>iluminación</strong> divina que le permite conocer las verdades eternas. Reinterpreta a Platón: las Ideas son ejemplares que están en la mente de Dios, que crea el mundo libremente y de la nada. Frente a los escépticos, argumenta que si me engaño, existo («si fallor, sum»). En la <em>Ciudad de Dios</em> ofrece una visión lineal de la historia, orientada hacia la salvación.</p>\n<p>Su influencia dominó la filosofía cristiana hasta el siglo XIII y siguió viva en Lutero y Calvino. Su defensa de la certeza de la propia existencia anticipa el <strong>cogito</strong> de Descartes, y su idea de la belleza como proporción fue retomada por Tomás de Aquino.</p>",
  "obras": [
   "Confesiones (c. 397-400)",
   "La ciudad de Dios (413-426)",
   "Sobre la Trinidad",
   "Sobre el libre albedrío"
  ],
  "anecdota": "<p>En el verano de 386, en un jardín de Milán, Agustín lloraba bajo una higuera, desgarrado entre su deseo de cambiar de vida y su incapacidad para hacerlo. De pronto oyó desde una casa vecina una voz infantil que repetía una especie de cantinela: <strong>«Toma y lee, toma y lee»</strong>. Lo interpretó como una orden divina, abrió al azar las cartas de san Pablo que tenía a mano y leyó un pasaje que invitaba a dejar los excesos. Él mismo lo relata en sus <em>Confesiones</em> como el instante decisivo de su conversión.</p>",
  "fuente": "Agustín de Hipona, Confesiones VIII",
  "tradicion": false,
  "block": "med",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-medieval",
   "hf-fe-razon",
   "hf-descartes-makro"
  ]
 },
 "hipatia": {
  "name": "Hipatia de Alejandría",
  "dates": "c. 355/370 – 415 d. C.",
  "born": 355,
  "died": 415,
  "place": "Alejandría (Egipto)",
  "role": "matemática, astrónoma y filósofa",
  "idea": "Unió la filosofía neoplatónica con la enseñanza de las matemáticas y la astronomía, entendiendo el estudio racional del cosmos como camino hacia la sabiduría.",
  "bio": "<p>Hipatia nació en Alejandría, el gran centro científico y cultural del mundo helenístico, hacia mediados del siglo IV; la fecha exacta es discutida. Era hija del matemático y astrónomo Teón, que la formó. Llegó a dirigir la escuela neoplatónica de la ciudad, donde enseñó a alumnos paganos y cristianos, entre ellos Sinesio de Cirene, obispo más tarde, cuyas cartas son una de las principales fuentes sobre ella. Tuvo gran prestigio público y trato con las autoridades de la ciudad.</p>\n<p>Fue matemática, astrónoma y filósofa. Siguió la línea del <strong>neoplatonismo</strong>, que unía la razón filosófica con una búsqueda casi religiosa de lo divino, y consideraba las matemáticas un paso hacia ese conocimiento superior. Según las fuentes antiguas, escribió comentarios a obras de matemáticos como Diofanto y Apolonio, y colaboró con su padre en la edición de textos astronómicos de Ptolomeo. <strong>No se conserva ninguna obra suya</strong>, por lo que conocemos su pensamiento solo a través de testimonios de otros autores.</p>\n<p>En el año 415, en medio de graves tensiones políticas y religiosas, fue asesinada por una turba de cristianos. Su muerte se ha convertido en símbolo del <strong>fin de la filosofía pagana</strong> y del mundo clásico, y del tránsito hacia la Edad Media. Hoy es también un referente del papel de la <strong>mujer en la ciencia</strong>, a menudo invisibilizado, junto a figuras como Marie Curie o Rosalind Franklin.</p>",
  "obras": [],
  "anecdota": "<p>El filósofo Damascio cuenta que Hipatia se ponía el manto de los filósofos, el tribon, y recorría el centro de Alejandría explicando en público a Platón, a Aristóteles o a cualquier otro pensador a quien quisiera escucharla. El historiador Sócrates Escolástico añade que los magistrados de la ciudad acudían a consultarla y que ella se presentaba ante reuniones de hombres con total naturalidad. En una época en que el saber se encerraba en las escuelas, Hipatia llevó la filosofía a la calle.</p>",
  "fuente": "Damascio, Vida de Isidoro (fragmentos en la Suda); Sócrates Escolástico, Historia eclesiástica VII",
  "tradicion": false,
  "block": "ant",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-helenismo"
  ]
 },
 "anselmo": {
  "name": "Anselmo de Canterbury",
  "dates": "1033 – 1109",
  "born": 1033,
  "died": 1109,
  "place": "Aosta (Italia)",
  "role": "filósofo y teólogo escolástico",
  "idea": "Dios es aquello mayor que lo cual nada puede pensarse; por eso no puede existir solo en la mente, sino que debe existir también en la realidad.",
  "bio": "<p>Anselmo nació en Aosta, en el norte de Italia, en 1033. Ingresó como monje en la abadía benedictina de Bec, en Normandía, de la que llegó a ser abad. En 1093 fue nombrado arzobispo de Canterbury, cargo en el que sufrió varios destierros por sus conflictos con los reyes de Inglaterra. Murió en 1109 y es considerado uno de los iniciadores de la <strong>escolástica</strong>. Su lema fue «la fe que busca entender».</p>\n<p>En el <em>Proslogion</em> formuló el <strong>argumento ontológico</strong>, una prueba de la existencia de Dios a partir de su propia definición: si Dios es el ser más perfecto que se puede pensar, debe existir también en la realidad. Aparece en el temario porque Descartes lo reformuló en el siglo XVII. Tomás de Aquino lo rechazó y Kant lo criticó a fondo.</p>",
  "obras": [
   "Monologion (1076)",
   "Proslogion (1077-1078)",
   "Por qué Dios se hizo hombre"
  ],
  "anecdota": "<p>Según su biógrafo y secretario Eadmero, Anselmo buscaba un único argumento que bastara para probar la existencia de Dios. La idea lo obsesionaba tanto que le quitaba el apetito, el sueño y hasta la concentración en los rezos, y llegó a pensar que era una tentación. Una noche, durante los maitines, la encontró por fin. La escribió en unas tablillas de cera, que se extraviaron; la copió en otras, que aparecieron rotas en el suelo. Al final la hizo pasar a pergamino: así nació el <em>Proslogion</em>.</p>",
  "fuente": "Eadmero, Vida de san Anselmo",
  "tradicion": false,
  "block": "med",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-racionalismo"
  ]
 },
 "abelardo": {
  "name": "Pedro Abelardo",
  "dates": "1079 – 1142",
  "born": 1079,
  "died": 1142,
  "place": "Le Pallet (Bretaña, Francia)",
  "role": "filósofo y lógico escolástico",
  "idea": "Los universales no son cosas reales ni meras palabras, sino conceptos de la mente que expresan lo que tienen en común los individuos.",
  "bio": "<p>Pedro Abelardo nació en Le Pallet, en Bretaña, en 1079. Renunció a la carrera de las armas para dedicarse a la lógica y fue uno de los maestros más famosos de París. Su vida estuvo marcada por su relación con Eloísa, su alumna, con quien tuvo un hijo: la familia de ella se vengó mutilándolo, y ambos ingresaron en la vida religiosa. Contó estas vivencias en su <em>Historia de mis desgracias</em>. Sus tesis teológicas fueron condenadas en los concilios de Soissons (1121) y de Sens (1141), este último impulsado por Bernardo de Claraval. Murió en 1142.</p>\n<p>Abelardo defendió el uso de la <strong>dialéctica</strong> (la lógica) en teología. En <em>Sí y no</em> reunió opiniones contradictorias de las autoridades sobre las mismas cuestiones, para que la razón las examinara: un paso decisivo hacia el método escolástico de la <em>quaestio</em> y la <em>disputatio</em>. En el <strong>problema de los universales</strong>, es decir, la pregunta por el estatus de los conceptos generales como «humanidad», adoptó una posición intermedia entre el realismo y el nominalismo, conocida como <strong>conceptualismo</strong>: los universales existen como conceptos en la mente, que los forma a partir de lo que tienen en común los individuos.</p>\n<p>Su espíritu crítico y su confianza en la razón influyeron en el desarrollo de las universidades. Su solución a los universales prepara la de Tomás de Aquino y contrasta con el nominalismo de Ockham.</p>",
  "obras": [
   "Sí y no (Sic et non)",
   "Historia de mis desgracias",
   "Ética o Conócete a ti mismo",
   "Diálogo entre un filósofo, un judío y un cristiano"
  ],
  "anecdota": "<p>Recién llegado a París, el joven Abelardo se hizo alumno de Guillermo de Champeaux, el maestro más famoso de la ciudad y defensor de un realismo extremo sobre los universales. Pronto empezó a contradecirle en plena clase, y a veces parecía ganar la discusión. Según su propio relato, acabó obligando a Guillermo a corregir su tesis, y muchos estudiantes abandonaron al maestro para seguir al discípulo. Lo cuenta él mismo en la <em>Historia de mis desgracias</em>, un testimonio valioso pero, lógicamente, poco imparcial.</p>",
  "fuente": "Pedro Abelardo, Historia de mis desgracias",
  "tradicion": false,
  "block": "med",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-medieval"
  ]
 },
 "hildegarda": {
  "name": "Hildegarda de Bingen",
  "dates": "1098 – 1179",
  "born": 1098,
  "died": 1179,
  "place": "Bermersheim (Renania, Alemania)",
  "role": "abadesa, mística y compositora",
  "idea": "El ser humano es un microcosmos que refleja el orden del universo creado por Dios, de modo que fe, ciencia natural y arte forman un único saber.",
  "bio": "<p>Hildegarda nació en 1098 en Bermersheim, en Renania, en una familia noble. Siendo niña fue entregada a la vida religiosa en el monasterio de Disibodenberg. En 1136 se convirtió en su abadesa y más tarde fundó su propio convento en Rupertsberg, cerca de Bingen. Desde la infancia afirmó tener visiones, que el papa Eugenio III autorizó a difundir. Mantuvo correspondencia con papas, emperadores y figuras como Bernardo de Claraval, y predicó en público, algo excepcional para una mujer de su tiempo. Murió en 1179 y fue canonizada y proclamada doctora de la Iglesia en 2012.</p>\n<p>Fue una personalidad <strong>polifacética</strong>: mística, teóloga, médica, naturalista y compositora. En <em>Scivias</em> describió e interpretó sus visiones. En sus obras de medicina y ciencias naturales recogió conocimientos sobre plantas, animales, minerales y enfermedades. Concebía al ser humano como un <strong>microcosmos</strong>, un «pequeño mundo» que refleja el <strong>macrocosmos</strong>, el universo ordenado y creado por Dios. Compuso además numerosos cantos litúrgicos y el <em>Ordo Virtutum</em>, un drama musical, una de las obras musicales más importantes de la época.</p>\n<p>Su figura muestra que la filosofía medieval no fue solo cosa de hombres ni de clérigos, y que el saber podía <strong>unir la fe, la ciencia y el arte</strong>. Hoy es un referente en la recuperación de las pensadoras olvidadas, junto a Hipatia de Alejandría, y su música se sigue interpretando y grabando.</p>",
  "obras": [
   "Scivias (1141-1151)",
   "Física (Physica)",
   "Causas y remedios",
   "Ordo Virtutum"
  ],
  "anecdota": "<p>En 1178, ya octogenaria, Hildegarda permitió enterrar en su monasterio a un joven noble que había estado excomulgado, porque, según ella, se había reconciliado con la Iglesia antes de morir. Las autoridades de Maguncia le ordenaron desenterrarlo y ella se negó. Como castigo, el convento quedó bajo interdicto y las monjas no pudieron cantar el oficio divino. Hildegarda protestó en una carta a los prelados en la que defendía la música como eco de la armonía del paraíso. El castigo se levantó poco antes de su muerte.</p>",
  "fuente": "Carta de Hildegarda a los prelados de Maguncia; Vida de santa Hildegarda",
  "tradicion": false,
  "block": "med",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-medieval"
  ]
 },
 "averroes": {
  "name": "Averroes",
  "dates": "1126 – 1198",
  "born": 1126,
  "died": 1198,
  "place": "Córdoba (al-Ándalus)",
  "role": "filósofo, jurista y médico andalusí",
  "idea": "La verdad no puede contradecir a la verdad: la demostración filosófica y la revelación coinciden, aunque cada una se dirige a un tipo distinto de personas.",
  "bio": "<p>Abu-l-Walid Muhammad ibn Rushd, conocido en Occidente como Averroes, nació en Córdoba en 1126. Fue cadí (juez) de Sevilla y de Córdoba y médico de la corte almohade. Por encargo del califa comentó las obras de Aristóteles, lo que le valió el sobrenombre de <strong>el Comentador</strong>. Hacia 1195 cayó en desgracia y fue desterrado a Lucena; aunque después fue rehabilitado, murió en Marrakech en 1198.</p>\n<p>Averroes quiso recuperar al Aristóteles auténtico y defendió la filosofía frente a quienes la acusaban de contraria al islam, como al-Ghazali. En su <em>Doctrina decisiva</em> sostuvo que la razón y la revelación no pueden contradecirse, porque la verdad no se opone a la verdad. Cuando un texto sagrado parece chocar con una demostración, debe interpretarse de forma alegórica. Distinguía además entre quienes se convencen por demostraciones (los filósofos) y quienes lo hacen por argumentos retóricos o dialécticos. De ahí surgió la teoría de la <strong>doble verdad</strong>, una para la fe y otra para la razón. Sin embargo, esta formulación se debe sobre todo a sus seguidores latinos, los <strong>averroístas</strong>, más que al propio Averroes.</p>\n<p>Sus comentarios, traducidos al latín, fueron decisivos para que el Occidente cristiano conociera a Aristóteles en el siglo XIII. Tomás de Aquino lo leyó con atención, aunque rechazó algunas de sus tesis, como la de un único intelecto para toda la humanidad.</p>",
  "obras": [
   "Doctrina decisiva",
   "La incoherencia de la incoherencia",
   "Comentarios a Aristóteles",
   "Libro de las generalidades de la medicina"
  ],
  "anecdota": "<p>Averroes contó a un discípulo cómo conoció al califa almohade Abu Yaqub Yusuf, gracias al filósofo Ibn Tufayl. Nada más saludarlo, el califa le preguntó qué pensaban los filósofos sobre el cielo: ¿era eterno o había sido creado? Averroes, asustado, fingió no saber nada de filosofía. Entonces el califa se puso a discutir el asunto con Ibn Tufayl, citando a Platón y Aristóteles, hasta que el joven se tranquilizó y habló. Salió de allí con regalos y, poco después, con el encargo de explicar a Aristóteles.</p>",
  "fuente": "Al-Marrakushi, Historia de los almohades (Kitab al-Muyib)",
  "tradicion": false,
  "block": "med",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-fe-razon"
  ]
 },
 "tomas": {
  "name": "Tomás de Aquino",
  "dates": "1225 – 1274",
  "born": 1225,
  "died": 1274,
  "place": "Roccasecca (Italia)",
  "role": "filósofo y teólogo escolástico",
  "idea": "Fe y razón no se contradicen, sino que colaboran: la razón demuestra la existencia de Dios a partir de la experiencia y prepara el camino a la fe.",
  "bio": "<p>Tomás nació en 1225 en el castillo de Roccasecca, cerca de Aquino. Contra la voluntad de los suyos ingresó en la orden dominica. Estudió con Alberto Magno en París y Colonia y fue profesor en la Universidad de París y en varias ciudades italianas. Murió en 1274, camino del Concilio de Lyon, dejando inacabada la <em>Suma teológica</em>. Fue canonizado en 1323.</p>\n<p>Es la gran figura de la <strong>escolástica</strong>. Realizó una síntesis entre la fe cristiana y la filosofía de Aristóteles, recién recuperada en Occidente. Para él, fe y razón colaboran: hay verdades naturales, alcanzables por la razón, y verdades sobrenaturales, conocidas solo por revelación, y nunca pueden contradecirse. La razón prepara y defiende la fe. Así, propone demostrar la existencia de Dios <strong>a posteriori</strong>, partiendo de la experiencia: sus <strong>cinco vías</strong> parten del movimiento, las causas, la contingencia, los grados de perfección y el orden del mundo. En ética y política defiende la <strong>ley natural</strong>, la ley moral que la razón descubre en la naturaleza humana.</p>\n<p>Su pensamiento, el <strong>tomismo</strong>, se convirtió en la filosofía de referencia de la Iglesia católica. Frente a Agustín, que busca a Dios en el interior, Tomás parte del mundo sensible; frente a Ockham, que separará fe y razón, representa el ideal de armonía entre ambas. Su teoría de la ley natural influyó en la filosofía del derecho y en autores modernos como Locke.</p>",
  "obras": [
   "Suma teológica (1265-1274)",
   "Suma contra los gentiles",
   "El ente y la esencia"
  ],
  "anecdota": "<p>Se cuenta que, cuando estudiaba en Colonia con Alberto Magno, Tomás era tan corpulento y callado que sus compañeros lo apodaron «el buey mudo de Sicilia». Un día, tras escucharlo defender con brillantez una cuestión difícil, Alberto dijo a sus alumnos que ellos lo llamaban buey mudo, pero que sus mugidos se oirían en el mundo entero. La escena procede de las biografías escritas para su canonización, con un tono ejemplar, pero retrata bien al pensador reservado que acabaría escribiendo la <em>Suma teológica</em>.</p>",
  "fuente": "Guillermo de Tocco, Historia de santo Tomás de Aquino",
  "tradicion": true,
  "block": "med",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-historicidad",
   "hf-medieval",
   "hf-fe-razon"
  ]
 },
 "ockham": {
  "name": "Guillermo de Ockham",
  "dates": "c. 1287 – 1347",
  "born": 1287,
  "died": 1347,
  "place": "Ockham (Surrey, Inglaterra)",
  "role": "filósofo y teólogo franciscano",
  "idea": "Solo existen los individuos y no deben multiplicarse los entes sin necesidad; la razón no puede demostrar las verdades de la fe.",
  "bio": "<p>Guillermo nació hacia 1287 en Ockham, una aldea inglesa del condado de Surrey. Ingresó en la orden franciscana y estudió teología en Oxford. En 1324 fue llamado a Aviñón, entonces sede del papado, para responder de la sospecha de herejía. En 1328 huyó de allí junto con otros franciscanos enfrentados con el papa Juan XXII por la cuestión de la pobreza, y se refugió en la corte del emperador Luis de Baviera, en Múnich. Murió en Múnich hacia 1347, probablemente a causa de la peste negra.</p>\n<p>Ockham es el principal representante del <strong>nominalismo</strong>: solo existen los individuos concretos, y los universales son meros nombres, signos que usamos para referirnos a muchas cosas a la vez. Aplica un principio de economía conocido como <strong>navaja de Ockham</strong>: no hay que multiplicar los entes sin necesidad. La fórmula habitual no es literalmente suya, pero resume su método. En la relación entre fe y razón, sostiene que la razón no puede demostrar las verdades de la fe: fe y razón se <strong>separan</strong> y la teología deja de ser una ciencia racional.</p>\n<p>Su pensamiento marca el final de la síntesis de Tomás de Aquino y abre la llamada <strong>vía moderna</strong>. Su atención a lo individual y a la experiencia anticipa el empirismo de Locke y Hume, y su separación entre fe y razón prepara el terreno para la ciencia moderna.</p>",
  "obras": [
   "Suma de lógica (c. 1323)",
   "Comentario a las Sentencias",
   "Quodlibetos",
   "Diálogo"
  ],
  "anecdota": "<p>Según la tradición, cuando Ockham huyó de Aviñón y se puso bajo la protección del emperador Luis de Baviera, le propuso un trato: «Defiéndeme con la espada, que yo te defenderé con la pluma». La frase no aparece en sus obras y probablemente es posterior, pero resume bien lo que ocurrió: en Múnich, Ockham dedicó sus últimos años a escribir tratados políticos contra el poder temporal del papa y a favor de la independencia del emperador. El lógico se convirtió así en polemista político.</p>",
  "fuente": "Frase atribuida por la tradición posterior; no consta en sus escritos",
  "tradicion": true,
  "block": "med",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-medieval",
   "hf-fe-razon"
  ]
 },
 "maquiavelo": {
  "name": "Nicolás Maquiavelo",
  "dates": "1469 – 1527",
  "born": 1469,
  "died": 1527,
  "place": "Florencia (Italia)",
  "role": "pensador político y diplomático",
  "idea": "La política debe estudiarse tal como es y no como debería ser: es una técnica autónoma, separada de la moral y de la religión, cuyo fin es conservar el Estado.",
  "bio": "<p>Nicolás Maquiavelo nació en Florencia en 1469, en plena época del Renacimiento. Entre 1498 y 1512 fue secretario de la República florentina y realizó numerosas misiones diplomáticas ante reyes, el papa y personajes como César Borgia, lo que le permitió observar de cerca el ejercicio del poder. Cuando los Médici recuperaron el gobierno en 1512, fue destituido, encarcelado y torturado. Retirado en una finca cerca de la ciudad, escribió <em>El príncipe</em>, que no se publicó hasta 1532, después de su muerte en 1527.</p>\n<p>Maquiavelo inaugura el <strong>realismo político</strong>. Separa la política de la moral y de la religión y describe el poder <strong>tal como es</strong>, no como debería ser. El gobernante debe asegurar el orden y la seguridad del Estado, y para ello puede usar la fuerza y el engaño si es necesario. Su éxito depende de la <strong>virtù</strong>, es decir, la habilidad y determinación para actuar en el momento oportuno, frente a la <strong>fortuna</strong>, el azar de las circunstancias. La política se convierte así en una técnica autónoma, con su propia lógica. La frase «el fin justifica los medios», que se le atribuye con frecuencia, no aparece literalmente en su obra.</p>\n<p>Es considerado el fundador de la ciencia política moderna. Su visión pesimista del ser humano y su interés por el origen y la conservación del poder preparan el terreno para las teorías del contrato social de Hobbes, Locke y Rousseau. El adjetivo «maquiavélico» simplifica un pensamiento más complejo.</p>",
  "obras": [
   "El príncipe (1513)",
   "Discursos sobre la primera década de Tito Livio",
   "Del arte de la guerra (1521)",
   "La mandrágora"
  ],
  "anecdota": "<p>Desterrado en su finca de Sant'Andrea in Percussina, Maquiavelo describió su rutina a su amigo Francesco Vettori. De día cazaba tordos, charlaba con leñadores y jugaba a las cartas en la taberna entre gritos y peleas. Al caer la noche, se quitaba la ropa sucia de barro, se ponía sus trajes de corte y entraba en su estudio a «conversar» con los antiguos, que le respondían con generosidad. De esas conversaciones, añadía, estaba saliendo un opúsculo sobre los principados: <em>El príncipe</em>.</p>",
  "fuente": "Maquiavelo, carta a Francesco Vettori del 10 de diciembre de 1513",
  "tradicion": false,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-modernidad",
   "hf-contrato"
  ]
 },
 "copernico": {
  "name": "Nicolás Copérnico",
  "dates": "1473 – 1543",
  "born": 1473,
  "died": 1543,
  "place": "Toruń (Polonia)",
  "role": "astrónomo y matemático",
  "idea": "La Tierra no es el centro inmóvil del universo, sino que gira sobre sí misma y alrededor del Sol, como los demás planetas.",
  "bio": "<p>Nicolás Copérnico nació en Toruń, en Polonia, en 1473. Estudió en Cracovia y después en las universidades italianas de Bolonia y Padua, donde se formó en astronomía, derecho canónico y medicina. Fue canónigo de la catedral de Frombork, donde realizó sus observaciones. Su obra principal se publicó en 1543, el año de su muerte.</p>\n<p>Copérnico propuso el <strong>heliocentrismo</strong>: el Sol, y no la Tierra, ocupa el centro del universo. Esta tesis abrió la <strong>revolución científica</strong>, que continuaron Kepler, Galileo y Newton y que derribó la cosmología de Aristóteles y Ptolomeo. Aparece en el temario por ese cambio en la imagen del mundo y porque Freud lo cita como la primera gran «humillación» del ser humano: no somos el centro del universo.</p>",
  "obras": [
   "Sobre las revoluciones de las esferas celestes (1543)"
  ],
  "anecdota": "<p>Copérnico retrasó durante décadas la publicación de su gran obra, hasta que su joven discípulo Rheticus lo convenció. Según una carta de su amigo Tiedemann Giese, el primer ejemplar impreso llegó a sus manos el mismo día de su muerte. El libro, además, traía una sorpresa: un prólogo sin firma, añadido por el teólogo Andreas Osiander, que presentaba el heliocentrismo como una simple hipótesis para facilitar los cálculos. Décadas después, Kepler reveló quién lo había escrito.</p>",
  "fuente": "Carta de Tiedemann Giese a Rheticus (1543)",
  "tradicion": true,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-modernidad",
   "hf-sospecha",
   "hf-descartes-makro"
  ]
 },
 "lutero": {
  "name": "Martín Lutero",
  "dates": "1483 – 1546",
  "born": 1483,
  "died": 1546,
  "place": "Eisleben (Sajonia, Alemania)",
  "role": "teólogo y reformador religioso",
  "idea": "El ser humano se salva solo por la fe, y cada creyente puede leer e interpretar directamente la Biblia sin depender de la autoridad de la Iglesia.",
  "bio": "<p>Martín Lutero nació en Eisleben, en Sajonia, en 1483. Estudió derecho, pero en 1505 ingresó en la orden de los agustinos. Fue profesor de teología en la Universidad de Wittenberg. En 1517 hizo públicas sus <strong>95 tesis</strong> contra la venta de indulgencias, fecha que se considera el inicio de la <strong>Reforma protestante</strong>. Excomulgado por el papa en 1521, se negó a retractarse ante el emperador Carlos V en la Dieta de Worms. Protegido por el príncipe de Sajonia, tradujo la Biblia al alemán. Murió en su ciudad natal en 1546.</p>\n<p>Lutero cuestionó la autoridad de la Iglesia y del papa. Defendió que la salvación se alcanza solo por la fe y la gracia de Dios, no por las obras. Sostuvo que la Escritura es la única autoridad en materia de fe y que cada creyente puede interpretarla: es el principio del <strong>libre examen</strong>. Propuso además una relación directa del creyente con Dios, sin necesidad de intermediarios. Con ello subrayó la importancia de la <strong>conciencia individual</strong>.</p>\n<p>La Reforma rompió la unidad religiosa e intelectual de Europa y provocó largas guerras de religión. Al dar más autonomía al individuo, impulsó la secularización de la cultura europea y forma parte del contexto del que surge la filosofía moderna de Descartes. Su pensamiento se inspiró en Agustín de Hipona, y Juan Calvino desarrolló después otra rama de la Reforma.</p>",
  "obras": [
   "Las 95 tesis (1517)",
   "La libertad del cristiano (1520)",
   "A la nobleza cristiana de la nación alemana (1520)",
   "La voluntad esclava (1525)"
  ],
  "anecdota": "<p>En julio de 1505, el joven estudiante de derecho Martín Lutero volvía a Erfurt cuando, cerca de la aldea de Stotternheim, le sorprendió una tormenta y un rayo cayó muy cerca de él. Aterrado, gritó: «¡Ayúdame, santa Ana, y me haré monje!». Cumplió su promesa pocas semanas después, contra la voluntad de su padre, e ingresó en el convento de los agustinos de Erfurt. El propio Lutero recordó muchas veces esta escena, que marcó el comienzo de su búsqueda religiosa.</p>",
  "fuente": "Testimonios del propio Lutero (Charlas de sobremesa)",
  "tradicion": false,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-modernidad",
   "hf-descartes-makro"
  ]
 },
 "calvino": {
  "name": "Juan Calvino",
  "dates": "1509 – 1564",
  "born": 1509,
  "died": 1564,
  "place": "Noyon (Picardía, Francia)",
  "role": "teólogo y reformador protestante",
  "idea": "Dios, soberano absoluto, ha predestinado desde la eternidad quiénes se salvarán, y el creyente debe vivir con disciplina y responsabilidad según su palabra.",
  "bio": "<p>Juan Calvino nació en Noyon, en el norte de Francia, en 1509. Estudió teología y derecho en París, Orleans y Bourges, y se adhirió a las ideas de la Reforma hacia 1533. Perseguido en Francia, huyó a Suiza. Desde 1541 dirigió la vida religiosa y moral de Ginebra, que convirtió en un gran centro del protestantismo, hasta su muerte en 1564.</p>\n<p>Calvino desarrolló una segunda gran rama de la Reforma. Destacó la soberanía absoluta de Dios y la doctrina de la <strong>predestinación</strong>: Dios ha decidido de antemano quiénes se salvarán. Como Lutero, subrayó la <strong>conciencia individual</strong> y la autoridad de la Biblia. Aparece en el temario porque la Reforma rompió la unidad intelectual de la Iglesia y forma parte del contexto de la filosofía moderna.</p>",
  "obras": [
   "Institución de la religión cristiana (1536)"
  ],
  "anecdota": "<p>En 1536, Calvino solo pensaba pasar una noche en Ginebra, de camino a Estrasburgo, donde quería dedicarse tranquilamente al estudio. Pero el predicador Guillaume Farel, que intentaba implantar allí la Reforma, se enteró y fue a buscarlo. Cuando Calvino se negó a quedarse, Farel le advirtió solemnemente que Dios maldeciría su retiro y sus estudios si abandonaba aquella tarea. Calvino, según su propio relato, quedó tan sobrecogido que renunció a su viaje. Ginebra ya no se separaría de su nombre.</p>",
  "fuente": "Calvino, prefacio al Comentario de los Salmos",
  "tradicion": false,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-descartes-makro"
  ]
 },
 "galileo": {
  "name": "Galileo Galilei",
  "dates": "1564 – 1642",
  "born": 1564,
  "died": 1642,
  "place": "Pisa (Italia)",
  "role": "físico y astrónomo",
  "idea": "La naturaleza está escrita en lenguaje matemático, y solo la experimentación controlada y la medición permiten conocer sus leyes, no la autoridad de Aristóteles.",
  "bio": "<p>Galileo Galilei nació en Pisa y fue profesor de matemáticas en Pisa y Padua. En 1609 perfeccionó el telescopio y lo dirigió al cielo: observó las montañas de la Luna, los satélites de Júpiter y las fases de Venus. Estas observaciones apoyaban el sistema de Copérnico. En 1633 la Inquisición lo condenó, le obligó a retractarse y pasó sus últimos años en arresto domiciliario cerca de Florencia.</p>\n<p>Galileo aparece en el temario como una figura clave de la <strong>revolución científica</strong>. Convirtió la <strong>experimentación</strong> y la <strong>matematización</strong> en el fundamento de la ciencia, y a partir de su trabajo se sistematizó el <strong>método hipotético-deductivo</strong>: formular hipótesis, deducir consecuencias y comprobarlas. Su física mecánica y cuantitativa derribó la de Aristóteles y planteó la necesidad de una filosofía nueva, como la de Descartes.</p>",
  "obras": [
   "El mensajero sideral (1610)",
   "Diálogo sobre los dos máximos sistemas del mundo (1632)",
   "Discursos y demostraciones matemáticas sobre dos nuevas ciencias (1638)"
  ],
  "anecdota": "<p>Según su discípulo y biógrafo Vincenzo Viviani, un Galileo de apenas diecinueve años observaba en la catedral de Pisa una lámpara que oscilaba colgada del techo. Tomándose el pulso como reloj, comprobó que cada oscilación duraba lo mismo, tanto si el balanceo era amplio como si era pequeño. Así habría descubierto el isocronismo del péndulo. La historia quizá esté embellecida, pero muestra lo que haría de él un pionero: medir los fenómenos cotidianos en lugar de confiar en las autoridades.</p>",
  "fuente": "Vincenzo Viviani, Relato histórico de la vida de Galileo",
  "tradicion": true,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-modernidad",
   "hf-descartes-makro"
  ]
 },
 "kepler": {
  "name": "Johannes Kepler",
  "dates": "1571 – 1630",
  "born": 1571,
  "died": 1630,
  "place": "Weil der Stadt (Alemania)",
  "role": "astrónomo y matemático",
  "idea": "Los planetas no giran en círculos perfectos, sino en órbitas elípticas con el Sol en uno de sus focos, siguiendo leyes matemáticas precisas.",
  "bio": "<p>Johannes Kepler nació en el sur de Alemania, estudió teología y matemáticas en Tubinga y fue un copernicano convencido desde joven. En Praga trabajó como ayudante del astrónomo danés Tycho Brahe y, a su muerte, heredó sus precisas observaciones de los planetas. Fue matemático imperial y vivió los conflictos religiosos de la guerra de los Treinta Años.</p>\n<p>A partir de los datos de Brahe, Kepler formuló las tres <strong>leyes del movimiento planetario</strong>. La más famosa afirma que los planetas describen <strong>órbitas elípticas</strong>, y no los círculos perfectos que la tradición creía. Aparece en el temario como uno de los protagonistas de la <strong>revolución científica</strong>, que sustituyó la cosmología de Aristóteles por un universo regido por leyes matemáticas. Sus leyes fueron después explicadas por la gravitación de Newton.</p>",
  "obras": [
   "El misterio cosmográfico (1596)",
   "Astronomía nueva (1609)",
   "La armonía del mundo (1619)"
  ],
  "anecdota": "<p>En julio de 1595, mientras daba clase en Graz, Kepler dibujaba en la pizarra las conjunciones de Júpiter y Saturno. Al trazar triángulos dentro de un círculo, vio aparecer otro círculo interior, y le pareció que la proporción entre ambos recordaba a la de las órbitas de esos planetas. De ahí saltó a una idea audaz: las distancias de los planetas se explicarían encajando los cinco sólidos regulares unos dentro de otros. La teoría era falsa, pero esa fe en un orden matemático lo guió hasta sus leyes.</p>",
  "fuente": "Kepler, El secreto del universo (Mysterium cosmographicum), prefacio",
  "tradicion": false,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-modernidad",
   "hf-descartes-makro"
  ]
 },
 "harvey": {
  "name": "William Harvey",
  "dates": "1578 – 1657",
  "born": 1578,
  "died": 1657,
  "place": "Folkestone (Inglaterra)",
  "role": "médico y fisiólogo",
  "idea": "La sangre circula continuamente por el cuerpo impulsada por el corazón, que actúa como una bomba, y así lo demuestran la observación y el experimento.",
  "bio": "<p>William Harvey nació en Inglaterra, estudió en Cambridge y se formó como médico en la Universidad de Padua, entonces uno de los grandes centros de la medicina europea. De vuelta en Londres, trabajó en el hospital de San Bartolomé y fue médico de los reyes Jacobo I y Carlos I.</p>\n<p>En 1628 publicó su descubrimiento de la <strong>circulación de la sangre</strong>: el corazón funciona como una bomba que impulsa la sangre, que recorre el cuerpo en un circuito cerrado. Lo demostró con disecciones, experimentos y cálculos, frente a la medicina tradicional heredada de Galeno. Aparece en el temario como parte de la <strong>revolución científica</strong> de la época de Descartes, quien conoció su obra y la utilizó en su visión del cuerpo como una <strong>máquina</strong>, aunque discrepaba de Harvey sobre la causa del movimiento del corazón.</p>",
  "obras": [
   "Sobre el movimiento del corazón y de la sangre en los animales (1628)"
  ],
  "anecdota": "<p>John Aubrey cuenta que, en la batalla de Edgehill (1642), la primera de la guerra civil inglesa, Harvey tenía a su cargo a los hijos del rey Carlos I. Para protegerlos se retiró con ellos junto a un seto y, mientras a su alrededor se combatía, sacó un libro del bolsillo y se puso a leer tranquilamente. Solo cambió de sitio cuando una bala de cañón cayó demasiado cerca. La anécdota, tal vez exagerada, retrata al investigador absorto en el estudio incluso en medio del caos.</p>",
  "fuente": "John Aubrey, Vidas breves",
  "tradicion": true,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-descartes-makro"
  ]
 },
 "hobbes": {
  "name": "Thomas Hobbes",
  "dates": "1588 – 1679",
  "born": 1588,
  "died": 1679,
  "place": "Westport, Malmesbury (Inglaterra)",
  "role": "filósofo y teórico político",
  "idea": "Sin Estado, los seres humanos viven en una guerra de todos contra todos; por miedo a la muerte, pactan ceder todo su poder a un soberano absoluto.",
  "bio": "<p>Thomas Hobbes nació en Inglaterra el año de la Armada Invencible, estudió en Oxford y trabajó durante décadas como preceptor de la familia aristocrática Cavendish. Viajó por Europa, conoció a Galileo y discutió con Descartes. Vivió la guerra civil inglesa desde su exilio en París, y esa experiencia de caos y violencia marcó su filosofía política.</p>\n<p>Hobbes es <strong>materialista</strong>: solo existen cuerpos en movimiento, y también el pensamiento es movimiento de la materia. Aplica este mecanicismo a la política. En el <strong>estado de naturaleza</strong>, sin Estado, el ser humano es egoísta y vive en una guerra de todos contra todos, donde el hombre es un lobo para el hombre. Para salir de esa situación, los individuos firman un <strong>contrato social</strong> y ceden todo su poder a un soberano absoluto, el <strong>Leviatán</strong>, que garantiza la paz y la seguridad. La legitimidad del Estado ya no procede de Dios, sino del acuerdo entre individuos.</p>\n<p>Hobbes es el primero de los grandes contractualistas y uno de los orígenes del liberalismo, por su punto de partida en el individuo. Locke responderá con una visión más optimista del estado de naturaleza y un poder limitado, y Rousseau con la soberanía popular. Su materialismo anticipa el de La Mettrie en la Ilustración.</p>",
  "obras": [
   "Elementos de derecho natural y político (1640)",
   "Sobre el ciudadano (1642)",
   "Leviatán (1651)",
   "Sobre el cuerpo (1655)"
  ],
  "anecdota": "<p>Según su amigo John Aubrey, Hobbes tenía ya unos cuarenta años cuando encontró, en la biblioteca de un caballero, los <em>Elementos</em> de Euclides abiertos por el teorema de Pitágoras. Al leer el enunciado exclamó que aquello era imposible. Leyó la demostración, que remitía a otra proposición anterior, y esta a otra, y así sucesivamente, hasta quedar convencido. Desde entonces se enamoró de la geometría. Aquel método, que parte de principios claros y avanza paso a paso, fue el que quiso aplicar a la política.</p>",
  "fuente": "John Aubrey, Vidas breves",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metafisica",
   "hf-contrato",
   "hf-utilitarismo"
  ]
 },
 "descartes": {
  "name": "René Descartes",
  "dates": "1596 – 1650",
  "born": 1596,
  "died": 1650,
  "place": "La Haye (Turena, Francia)",
  "role": "filósofo y matemático",
  "idea": "Dudando metódicamente de todo, se alcanza una verdad indudable, pienso, luego existo, desde la que la razón reconstruye con certeza todo el conocimiento.",
  "bio": "<p>René Descartes nació en una familia noble francesa y estudió en el colegio jesuita de La Flèche, donde solo la matemática le pareció un saber seguro. Tras servir como soldado voluntario, vivió de 1628 a 1649 en los Países Bajos, donde gozaba de mayor libertad intelectual y escribió sus obras principales. Invitado por la reina Cristina de Suecia, murió en Estocolmo en 1650.</p>\n<p>Descartes es el iniciador del <strong>racionalismo</strong>. Busca un método único para todas las ciencias, inspirado en la matemática. Aplica la <strong>duda metódica</strong>: los sentidos, el sueño y la hipótesis del genio maligno hacen dudar de todo, salvo de una verdad, el <strong>cogito</strong> (pienso, luego existo). De ahí obtiene el criterio de certeza: es verdadero lo que se percibe clara y distintamente. Dios, cuya existencia demuestra, garantiza ese conocimiento. La realidad se compone de tres sustancias: Dios, la <em>res cogitans</em> o sustancia pensante y la <em>res extensa</em> o materia. Este <strong>dualismo</strong> deja abierto el problema de cómo se relacionan alma y cuerpo, que él situó en la glándula pineal.</p>\n<p>Descartes es considerado el padre de la filosofía moderna, porque sitúa la verdad en la evidencia del sujeto pensante. Spinoza, Malebranche y Leibniz intentaron resolver su dualismo; los empiristas, como Locke y Hume, rechazaron sus ideas innatas, y Kant buscó una síntesis entre ambas corrientes.</p>",
  "obras": [
   "Discurso del método (1637)",
   "Meditaciones metafísicas (1641)",
   "Los principios de la filosofía (1644)",
   "Las pasiones del alma (1649)"
  ],
  "anecdota": "<p>En el invierno de 1619, Descartes, entonces soldado, quedó detenido en un pueblo de Alemania por el frío. Pasó el día entero encerrado en una habitación caldeada por una estufa, sin nadie que lo distrajera, entregado a sus pensamientos. Allí concibió la idea de que las obras hechas por un solo arquitecto son más perfectas que las remendadas por muchos, y decidió reconstruir todo su saber desde los cimientos. Según su biógrafo Baillet, esa misma noche tuvo tres sueños que interpretó como una llamada a esa tarea.</p>",
  "fuente": "Descartes, Discurso del método II; Adrien Baillet, Vida de Descartes",
  "tradicion": false,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metodos",
   "hf-modernidad",
   "hf-racionalismo",
   "hf-metafisica",
   "hf-kant",
   "hf-descartes-makro"
  ]
 },
 "isabel": {
  "name": "Isabel de Bohemia",
  "dates": "1618 – 1680",
  "born": 1618,
  "died": 1680,
  "place": "Heidelberg (Alemania)",
  "role": "princesa y filósofa",
  "idea": "Si el alma es inmaterial y el cuerpo es pura extensión, no se entiende cómo el alma puede mover el cuerpo: el dualismo cartesiano necesita una explicación.",
  "bio": "<p>Isabel de Bohemia, hija del elector del Palatinado y rey de Bohemia Federico V, creció en el exilio en los Países Bajos tras la derrota de su familia en la guerra de los Treinta Años. Recibió una formación excelente en lenguas, matemáticas y filosofía. En su madurez fue abadesa de un convento protestante en Herford (Alemania).</p>\n<p>Entre 1643 y 1649 mantuvo con Descartes una célebre <strong>correspondencia</strong>. En ella le planteó la objeción más aguda a su dualismo: cómo puede una sustancia pensante, sin extensión, mover un cuerpo. Descartes no logró darle una respuesta satisfactoria, y ese es el <strong>problema de la interacción alma-cuerpo</strong>. Las cartas trataron también sobre las pasiones y la felicidad, y dieron origen a <em>Las pasiones del alma</em>. No escribió tratados propios: su pensamiento se conserva en estas cartas.</p>",
  "obras": [
   "Correspondencia con Descartes (1643–1649)"
  ],
  "anecdota": "<p>En 1644, Descartes publicó sus <em>Principios de la filosofía</em>, su obra más sistemática, y se la dedicó a Isabel, que entonces tenía veinticinco años. En la dedicatoria afirmaba que no había conocido a nadie que entendiera tan bien como ella todos sus escritos: muchos dominaban la metafísica y otros las matemáticas, pero solo ella comprendía por igual ambas partes. Viniendo de un autor tan exigente, era un reconocimiento extraordinario a la joven princesa que se había atrevido a señalar los puntos débiles de su dualismo.</p>",
  "fuente": "Descartes, dedicatoria de los Principios de la filosofía (1644)",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-descartes-makro"
  ]
 },
 "spinoza": {
  "name": "Baruch Spinoza",
  "dates": "1632 – 1677",
  "born": 1632,
  "died": 1677,
  "place": "Ámsterdam (Países Bajos)",
  "role": "filósofo racionalista",
  "idea": "Solo existe una sustancia, Dios o la Naturaleza, y el pensamiento y la extensión son dos atributos suyos; la libertad consiste en comprender la necesidad.",
  "bio": "<p>Baruch Spinoza nació en Ámsterdam en una familia de judíos sefardíes de origen portugués que habían huido de la Inquisición. Por sus ideas religiosas, consideradas heréticas, fue expulsado de la comunidad judía en 1656. Vivió de forma modesta, en parte puliendo lentes para instrumentos ópticos, y rechazó una cátedra en Heidelberg para conservar su libertad de pensamiento. Su obra principal, la <em>Ética</em>, se publicó tras su muerte.</p>\n<p>Spinoza lleva el racionalismo cartesiano hasta sus últimas consecuencias. Expone su filosofía según el <strong>orden geométrico</strong>, con definiciones, axiomas y demostraciones, como Euclides. Si sustancia es lo que no necesita de otra cosa para existir, solo puede haber una: <strong>Dios o la Naturaleza</strong> (<em>Deus sive Natura</em>). El pensamiento y la extensión no son dos sustancias, sino dos atributos de esa única realidad. Con este <strong>monismo</strong> desaparece el problema cartesiano de la relación entre alma y cuerpo. Todo ocurre por necesidad, y la libertad humana consiste en comprender esa necesidad y dejar de ser esclavo de las pasiones.</p>\n<p>Spinoza fue muy criticado en su tiempo y acusado de ateísmo, pero ejerció una gran influencia en el idealismo alemán y en autores como Nietzsche. Su defensa de la libertad de pensamiento y de la tolerancia lo convierte también en un precursor de la Ilustración.</p>",
  "obras": [
   "Tratado teológico-político (1670)",
   "Ética demostrada según el orden geométrico (1677)",
   "Tratado de la reforma del entendimiento (1677)"
  ],
  "anecdota": "<p>En agosto de 1672, una multitud enfurecida asesinó en La Haya a los hermanos De Witt, líderes de la república holandesa a quienes Spinoza admiraba. Indignado, el filósofo, siempre tan sereno, quiso salir a colgar junto al lugar un cartel con las palabras latinas <em>ultimi barbarorum</em>, «los peores de los bárbaros». Su casero, temiendo que lo mataran también a él, cerró la puerta con llave y no le dejó salir. Spinoza se lo contó años después a Leibniz, que lo anotó.</p>",
  "fuente": "Leibniz, notas de su conversación con Spinoza (1676)",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-racionalismo",
   "hf-descartes-makro"
  ]
 },
 "locke": {
  "name": "John Locke",
  "dates": "1632 – 1704",
  "born": 1632,
  "died": 1704,
  "place": "Wrington (Inglaterra)",
  "role": "filósofo empirista y político",
  "idea": "La mente nace como una hoja en blanco que llena la experiencia, y el poder político solo es legítimo si protege los derechos naturales de vida, libertad y propiedad.",
  "bio": "<p>John Locke nació en Inglaterra, estudió en Oxford y se formó también como médico. Fue secretario y colaborador del conde de Shaftesbury, líder de la oposición liberal, y por ello tuvo que exiliarse en los Países Bajos. Regresó tras la Revolución Gloriosa de 1688, que limitó el poder del rey, y en pocos años publicó sus obras principales.</p>\n<p>En teoría del conocimiento, Locke es el fundador del <strong>empirismo</strong> moderno. Frente a Descartes, niega las ideas innatas: la mente al nacer es una <strong>tabula rasa</strong> (hoja en blanco), y todo conocimiento procede de la experiencia. En su concepción de la identidad personal, lo que da unidad al yo es la continuidad de la conciencia y la memoria. En política es un contractualista optimista. En el estado de naturaleza ya existen <strong>derechos naturales</strong> (vida, libertad y propiedad), pero falta un juez imparcial. Por eso los individuos pactan un <strong>gobierno limitado</strong>, basado en el consentimiento, con división de poderes. Si el gobierno viola esos derechos, el pueblo tiene derecho a rebelarse.</p>\n<p>Locke es considerado el padre del <strong>liberalismo</strong> político y un inspirador directo de la Ilustración, de la independencia de Estados Unidos y de las declaraciones de derechos. Su empirismo fue continuado por Berkeley y Hume, y su teoría política se opone al absolutismo de Hobbes.</p>",
  "obras": [
   "Carta sobre la tolerancia (1689)",
   "Ensayo sobre el entendimiento humano (1690)",
   "Dos tratados sobre el gobierno civil (1690)"
  ],
  "anecdota": "<p>Hacia 1671, cinco o seis amigos se reunían en la habitación de Locke para discutir cuestiones filosóficas; según uno de ellos, James Tyrrell, sobre la moral y la religión revelada. Pronto se encontraron atascados, sin avanzar hacia ninguna solución. Locke pensó entonces que antes de lanzarse a esas preguntas había que examinar primero las capacidades de nuestro entendimiento y averiguar qué objetos puede conocer y cuáles no. Aquella intuición de una tarde tardó casi veinte años en convertirse en su <em>Ensayo sobre el entendimiento humano</em>.</p>",
  "fuente": "Locke, «Epístola al lector» del Ensayo sobre el entendimiento humano",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-contrato",
   "hf-utilitarismo",
   "hf-ilustracion",
   "hf-descartes-makro"
  ]
 },
 "malebranche": {
  "name": "Nicolas Malebranche",
  "dates": "1638 – 1715",
  "born": 1638,
  "died": 1715,
  "place": "París (Francia)",
  "role": "filósofo y sacerdote racionalista",
  "idea": "Las criaturas no causan nada por sí mismas: Dios es la única causa verdadera y actúa en cada ocasión para coordinar el alma y el cuerpo.",
  "bio": "<p>Nicolas Malebranche nació en París y era de salud frágil, por lo que se educó en casa durante sus primeros años. Estudió filosofía y teología, y en 1660 ingresó en la congregación religiosa del Oratorio, donde fue ordenado sacerdote. Según su propio relato, la lectura del <em>Tratado del hombre</em> de Descartes le causó una impresión tan fuerte que decidió dedicarse a la filosofía. Pasó el resto de su vida en el Oratorio, escribiendo y polemizando con otros pensadores.</p>\n<p>Malebranche intenta conciliar la filosofía de Descartes con la teología cristiana de san Agustín. Su aportación más conocida es el <strong>ocasionalismo</strong>, una respuesta al problema del dualismo cartesiano. Si el alma y el cuerpo son sustancias completamente distintas, no pueden actuar una sobre otra. En realidad, ninguna criatura es causa de nada: <strong>Dios es la única causa</strong> verdadera. Cuando quiero mover el brazo, mi voluntad es solo la ocasión en la que Dios produce ese movimiento. Además, defendió que conocemos las cosas a través de las ideas que están en Dios, la llamada <strong>visión en Dios</strong>.</p>\n<p>Malebranche fue muy leído en su tiempo y polemizó con Arnauld y Leibniz. Su crítica de la causalidad, que no se puede observar entre las criaturas, influyó en el análisis que hará Hume de la relación causa-efecto.</p>",
  "obras": [
   "La búsqueda de la verdad (1674–1675)",
   "Tratado de la naturaleza y de la gracia (1680)",
   "Conversaciones sobre la metafísica y la religión (1688)"
  ],
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metafisica",
   "hf-descartes-makro"
  ]
 },
 "newton": {
  "name": "Isaac Newton",
  "dates": "1642 – 1727",
  "born": 1642,
  "died": 1727,
  "place": "Woolsthorpe (Inglaterra)",
  "role": "físico y matemático",
  "idea": "Una misma ley de gravitación universal explica tanto la caída de los cuerpos en la Tierra como el movimiento de los planetas, unificando la física terrestre y la celeste.",
  "bio": "<p>Isaac Newton nació en el campo inglés y estudió en la Universidad de Cambridge, donde fue profesor de matemáticas. Desarrolló el cálculo infinitesimal, al mismo tiempo que Leibniz, e hizo importantes descubrimientos sobre la luz y los colores. Más tarde presidió la Royal Society de Londres y dirigió la Casa de la Moneda.</p>\n<p>En sus <em>Principios matemáticos de la filosofía natural</em> formuló las tres <strong>leyes del movimiento</strong> y la <strong>ley de la gravitación universal</strong>, que explica con las mismas ecuaciones la caída de una manzana y las órbitas de los planetas. Culminó así la <strong>revolución científica</strong>. Aparece en el temario porque su física ofreció la imagen de un universo ordenado y regido por leyes, que inspiró la confianza ilustrada en la razón. También sirve de ejemplo de <strong>paradigma</strong> científico, sustituido siglos después por la física de Einstein.</p>",
  "obras": [
   "Principios matemáticos de la filosofía natural (1687)",
   "Óptica (1704)"
  ],
  "anecdota": "<p>En 1665 la peste cerró la Universidad de Cambridge y Newton volvió a la granja familiar de Woolsthorpe, donde pasó meses de enorme creatividad. Sesenta años después, tomando el té bajo unos manzanos con su amigo William Stukeley, le contó que allí mismo, en una situación parecida, la caída de una manzana le había sugerido la idea de la gravitación: ¿por qué cae siempre en vertical, hacia el centro de la Tierra? Newton no mencionó ningún golpe en la cabeza, y probablemente embelleció el recuerdo.</p>",
  "fuente": "William Stukeley, Memorias de la vida de Sir Isaac Newton",
  "tradicion": true,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-modernidad",
   "hf-ilustracion",
   "hf-descartes-makro"
  ]
 },
 "leibniz": {
  "name": "Gottfried Wilhelm Leibniz",
  "dates": "1646 – 1716",
  "born": 1646,
  "died": 1716,
  "place": "Leipzig (Alemania)",
  "role": "filósofo y matemático racionalista",
  "idea": "La realidad está formada por infinitas mónadas, sustancias simples que no interactúan, cuyo orden sincronizó Dios desde el principio en el mejor de los mundos posibles.",
  "bio": "<p>Gottfried Wilhelm Leibniz nació en Leipzig y estudió derecho y filosofía. Fue diplomático, jurista, historiador y bibliotecario al servicio de los duques de Hannover, y viajó por París y Londres, donde conoció a los grandes científicos de su tiempo. Inventó el cálculo infinitesimal a la vez que Newton, lo que provocó una dura disputa sobre la prioridad del descubrimiento, y fundó la Academia de Ciencias de Berlín.</p>\n<p>Leibniz propone una solución original al problema cartesiano de la sustancia. La realidad está compuesta por infinitas <strong>mónadas</strong>: sustancias simples, activas e indivisibles, parecidas a puntos de fuerza o de energía. Las mónadas no tienen ventanas, es decir, no reciben influencia desde fuera. El orden del mundo, y en particular la correspondencia entre alma y cuerpo, se explica por la <strong>armonía preestablecida</strong>: Dios las sincronizó desde el principio, como un relojero que fabrica dos relojes que marcan siempre la misma hora. Según el <strong>principio de razón suficiente</strong>, nada ocurre sin una razón, y por eso Dios ha creado el <strong>mejor de los mundos posibles</strong>.</p>\n<p>Leibniz fue una de las mentes más universales de la época moderna y precursor de la lógica simbólica. Su optimismo fue satirizado por Voltaire en <em>Cándido</em>, y su racionalismo, sistematizado por Wolff, fue la filosofía que Kant aprendió y luego criticó.</p>",
  "obras": [
   "Discurso de metafísica (1686)",
   "Nuevos ensayos sobre el entendimiento humano (escritos en 1704)",
   "Teodicea (1710)",
   "Monadología (1714)"
  ],
  "anecdota": "<p>El padre de Leibniz, profesor en Leipzig, murió cuando él tenía seis años. Hacia los ocho, el niño consiguió que le dejaran entrar en la biblioteca paterna, hasta entonces cerrada para él. Allí encontró un Tito Livio ilustrado y, ayudándose de los grabados y del contexto, aprendió latín prácticamente solo. Pronto devoraba a los clásicos, a los padres de la Iglesia y a los escolásticos. Él mismo lo recordaría en sus notas autobiográficas: fue el origen de una curiosidad universal que abarcó casi todas las ciencias.</p>",
  "fuente": "Notas autobiográficas de Leibniz",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metafisica",
   "hf-descartes-makro"
  ]
 },
 "berkeley": {
  "name": "George Berkeley",
  "dates": "1685 – 1753",
  "born": 1685,
  "died": 1753,
  "place": "Condado de Kilkenny (Irlanda)",
  "role": "filósofo empirista y obispo",
  "idea": "Ser es ser percibido: las cosas materiales no existen fuera de la mente, sino que son conjuntos de ideas percibidas, sostenidas en último término por Dios.",
  "bio": "<p>George Berkeley nació en Irlanda y estudió en el Trinity College de Dublín, donde escribió muy joven sus obras principales. Fue clérigo anglicano, viajó a América para intentar fundar un colegio en las Bermudas y, finalmente, fue nombrado obispo de Cloyne, en Irlanda.</p>\n<p>Berkeley es, junto con Locke y Hume, uno de los principales representantes del <strong>empirismo</strong> británico. Lleva el principio de que todo conocimiento procede de la experiencia hasta una conclusión radical, el <strong>inmaterialismo</strong>: solo conocemos nuestras percepciones, de modo que no hay razones para afirmar una materia que exista fuera de ellas. Su lema es que ser es ser percibido (<em>esse est percipi</em>). Las cosas siguen existiendo cuando nadie las mira porque Dios las percibe siempre. Su crítica de la sustancia material preparó el camino a Hume.</p>",
  "obras": [
   "Ensayo sobre una nueva teoría de la visión (1709)",
   "Tratado sobre los principios del conocimiento humano (1710)",
   "Tres diálogos entre Hilas y Filonús (1713)"
  ],
  "anecdota": "<p>James Boswell cuenta que en 1763, al salir de la iglesia, conversaba con el escritor Samuel Johnson sobre el ingenioso argumento de Berkeley para negar la existencia de la materia. Boswell comentó que era imposible refutarlo. Johnson dio entonces una fuerte patada a una piedra grande y exclamó: «¡Así lo refuto!». La respuesta se hizo célebre, aunque en realidad no refuta nada: Berkeley nunca negó que sintamos la dureza de la piedra, sino que exista algo material más allá de esas percepciones.</p>",
  "fuente": "James Boswell, Vida de Samuel Johnson",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-descartes-makro"
  ]
 },
 "montesquieu": {
  "name": "Montesquieu",
  "dates": "1689 – 1755",
  "born": 1689,
  "died": 1755,
  "place": "La Brède, cerca de Burdeos (Francia)",
  "role": "pensador político ilustrado",
  "idea": "Para evitar el despotismo y garantizar la libertad, el poder debe dividirse en legislativo, ejecutivo y judicial, de modo que cada poder frene a los otros.",
  "bio": "<p>Charles-Louis de Secondat, barón de Montesquieu, nació en una familia de la nobleza de toga cerca de Burdeos. Estudió derecho y heredó el cargo de presidente del Parlamento de Burdeos, un tribunal de justicia. Se hizo famoso con las <em>Cartas persas</em>, una sátira de la sociedad francesa, y viajó por Europa, con una larga estancia en Inglaterra, cuyo sistema político admiraba.</p>\n<p>En <em>Del espíritu de las leyes</em> estudia cómo las leyes dependen del clima, las costumbres y la forma de gobierno de cada pueblo. Su aportación más influyente es la <strong>división de poderes</strong>: el legislativo, el ejecutivo y el judicial deben estar en manos distintas, para que el poder frene al poder y se evite el <strong>despotismo</strong>. Aparece en el temario como uno de los ilustrados que fundamentan el Estado de derecho y las constituciones modernas.</p>",
  "obras": [
   "Cartas persas (1721)",
   "Del espíritu de las leyes (1748)"
  ],
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-ilustracion"
  ]
 },
 "voltaire": {
  "name": "Voltaire",
  "dates": "1694 – 1778",
  "born": 1694,
  "died": 1778,
  "place": "París (Francia)",
  "role": "escritor y filósofo ilustrado",
  "idea": "La razón debe combatir el fanatismo y la superstición; la tolerancia religiosa y la libertad de expresión son condiciones de una sociedad civilizada.",
  "bio": "<p>François-Marie Arouet, conocido como Voltaire, nació en París en una familia burguesa y se educó con los jesuitas. Sus escritos satíricos le llevaron dos veces a la Bastilla y al exilio en Inglaterra, donde admiró la libertad política, la física de Newton y la filosofía de Locke. Pasó sus últimos años en Ferney, cerca de Ginebra, y fue el escritor más célebre de la Ilustración.</p>\n<p>Voltaire aparece en el temario como defensor de la <strong>tolerancia religiosa</strong> frente al <strong>fanatismo</strong>. Intervino en casos de injusticia como el del comerciante protestante Jean Calas, ejecutado en 1762, y criticó con ironía la intolerancia de la Iglesia. En <em>Cándido</em> se burló del optimismo de Leibniz. Sin embargo, como la mayoría de los ilustrados, no extendió la igualdad a las mujeres.</p>",
  "obras": [
   "Cartas filosóficas (1734)",
   "Cándido (1759)",
   "Tratado sobre la tolerancia (1763)",
   "Diccionario filosófico (1764)"
  ],
  "anecdota": "<p>En 1729, el matemático La Condamine descubrió un fallo en una lotería organizada por el gobierno francés: los premios superaban lo que costaba comprar todos los billetes. Voltaire se unió a él y a otros socios, compraron billetes en masa durante meses y ganaron una fortuna. Con ese dinero, bien invertido después, Voltaire se hizo rico e independiente. No necesitó protectores ni pensiones, y esa independencia económica le permitió escribir con una libertad que pocos autores de su tiempo podían permitirse.</p>",
  "fuente": "Biografías de Voltaire, a partir de testimonios de la época",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-ilustracion"
  ]
 },
 "lamettrie": {
  "name": "Julien Offray de La Mettrie",
  "dates": "1709 – 1751",
  "born": 1709,
  "died": 1751,
  "place": "Saint-Malo (Francia)",
  "role": "médico y filósofo materialista",
  "idea": "El ser humano es una máquina compleja: el alma no es una sustancia espiritual, sino el resultado del funcionamiento de los órganos, sobre todo del cerebro.",
  "bio": "<p>Julien Offray de La Mettrie nació en Saint-Malo, en Bretaña, en 1709. Estudió medicina y se formó en Leiden con el célebre médico Herman Boerhaave. Sirvió como médico militar y, según su propio relato, una fiebre le hizo notar cómo el cuerpo alteraba su pensamiento. Sus escritos escandalizaron tanto que tuvo que huir de Francia y después de Holanda. Terminó sus días en Berlín, protegido por el rey Federico II de Prusia, donde murió en 1751.</p>\n<p>La Mettrie es uno de los defensores más radicales del <strong>materialismo</strong>: la idea de que solo existe la materia. Descartes había explicado los animales como máquinas, pero reservaba al ser humano un alma inmaterial. La Mettrie da el paso siguiente en <em>El hombre máquina</em>: el ser humano es un <strong>autómata</strong> muy complejo, y lo que llamamos alma es solo el resultado del funcionamiento de los órganos, en especial del <strong>cerebro</strong>. Pensamiento, voluntad y sentimientos dependerían, así, de causas corporales.</p>\n<p>Por eso aparece en el temario como ejemplo de materialismo frente al <strong>dualismo</strong> cartesiano, en la línea que había abierto Hobbes. Sus ideas influyeron en otros materialistas ilustrados, como Diderot o el barón de Holbach, y anticipan en parte la visión de la mente que defienden hoy muchas neurociencias.</p>",
  "obras": [
   "Historia natural del alma (1745)",
   "El hombre máquina (1748)"
  ],
  "anecdota": "<p>En noviembre de 1751, La Mettrie acudió a un banquete en casa del embajador francés en Berlín, lord Tyrconnell, a quien había tratado como médico. Poco después cayó enfermo y murió en pocos días; Voltaire y otros testigos atribuyeron el mal a un paté de trufas que habría comido en exceso. Sus enemigos se burlaron de aquel final tan poco filosófico, pero el propio Federico II escribió su elogio fúnebre y lo hizo leer en la Academia de Berlín. Era un gesto insólito: un rey defendía en público la memoria del autor más escandaloso de su tiempo.</p>",
  "fuente": "Cartas de Voltaire desde Berlín; Federico II, Elogio de La Mettrie",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metafisica"
  ]
 },
 "hume": {
  "name": "David Hume",
  "dates": "1711 – 1776",
  "born": 1711,
  "died": 1776,
  "place": "Edimburgo (Escocia)",
  "role": "filósofo empirista e historiador",
  "idea": "Todo conocimiento procede de las impresiones de la experiencia; ideas como la causalidad o el yo sustancial no se justifican por la razón, sino por la costumbre.",
  "bio": "<p>David Hume nació en Edimburgo en 1711 y fue una de las grandes figuras de la Ilustración escocesa. Publicó muy joven el <em>Tratado de la naturaleza humana</em>, que apenas tuvo eco, y luego rehízo sus ideas en obras más breves. Sus opiniones religiosas le cerraron las puertas de la universidad, pero ganó fama como ensayista y como autor de una extensa <em>Historia de Inglaterra</em>. Trabajó también como diplomático en París y fue amigo de Adam Smith.</p>\n<p>Hume lleva el <strong>empirismo</strong> hasta sus últimas consecuencias. La mente solo recibe <strong>impresiones</strong> (percepciones vivas) y forma ideas, que son sus copias débiles. Distingue las relaciones de ideas, propias de las matemáticas, de las cuestiones de hecho, que dependen de la experiencia. De ahí sus críticas: no percibimos ninguna conexión necesaria entre causa y efecto, sino que la suponemos por <strong>costumbre</strong>; y el yo no es una sustancia, sino un <strong>haz de percepciones</strong>. En ética sostiene que la moral nace del sentimiento, no de la razón, y en estética defiende que existe un buen gusto, propio del crítico formado.</p>\n<p>Su escepticismo marcó la filosofía posterior. Kant reconoció que Hume lo despertó de su «sueño dogmático» y buscó una síntesis entre racionalismo y empirismo. Su ética del sentimiento se considera precursora del emotivismo del siglo XX, y su análisis de la causalidad sigue presente en la filosofía de la ciencia.</p>",
  "obras": [
   "Tratado de la naturaleza humana (1739-1740)",
   "Investigación sobre el entendimiento humano (1748)",
   "Investigación sobre los principios de la moral (1751)",
   "Diálogos sobre la religión natural (1779)"
  ],
  "anecdota": "<p>Se cuenta que Hume, ya corpulento y famoso por su escepticismo religioso, se cayó en una zanja cenagosa al atajar por una zona en obras de Edimburgo. Una vecina que pasaba lo reconoció como «Hume el ateo» y se negó a sacarlo de allí hasta que rezase el padrenuestro y el credo. Él, con su buen humor habitual, los recitó sin protestar, y ella le tendió la mano. La escena resume bien su carácter: tenía ideas incómodas para su época, pero todos sus contemporáneos destacaban su trato afable y su falta de rencor.</p>",
  "fuente": "Tradición de Edimburgo, recogida en biografías de Hume (E. C. Mossner, The Life of David Hume)",
  "tradicion": true,
  "block": "ren",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-racionalismo",
   "hf-kant",
   "hf-descartes-makro"
  ]
 },
 "rousseau": {
  "name": "Jean-Jacques Rousseau",
  "dates": "1712 – 1778",
  "born": 1712,
  "died": 1778,
  "place": "Ginebra (Suiza)",
  "role": "filósofo y teórico político",
  "idea": "El ser humano es bueno por naturaleza y la sociedad lo corrompe; un contrato social legítimo exige obedecer a la voluntad general, expresión de la soberanía popular.",
  "bio": "<p>Jean-Jacques Rousseau nació en Ginebra en 1712 y tuvo una juventud errante, sin estudios regulares. En París trató a los ilustrados y colaboró en la <em>Enciclopedia</em>, pero pronto se distanció de ellos. Se hizo famoso con su <em>Discurso sobre las ciencias y las artes</em>, donde sostenía que el progreso no había mejorado moralmente a la humanidad. Algunas de sus obras fueron condenadas y tuvo que vivir en parte como exiliado. Murió en Ermenonville, cerca de París, en 1778.</p>\n<p>Rousseau parte de que el ser humano es <strong>bueno por naturaleza</strong>: en el estado de naturaleza era libre e igual, y es la sociedad, con la propiedad privada y la desigualdad, la que lo corrompe. En <em>El contrato social</em> propone un pacto por el que cada uno se une a todos y obedece a la <strong>voluntad general</strong>, que no es la suma de los intereses particulares, sino el interés común. La <strong>soberanía</strong> reside en el pueblo, y la libertad consiste en obedecer las leyes que uno mismo se ha dado. En el <em>Emilio</em> expuso sus ideas sobre la educación.</p>\n<p>Con Hobbes y Locke, es uno de los tres grandes contractualistas del temario, y su idea de soberanía popular inspiró la Revolución francesa y la democracia moderna. Kant admiró su pensamiento moral. En cambio, Mary Wollstonecraft criticó el modelo de educación femenina del <em>Emilio</em>, que destinaba a las mujeres a agradar y a la vida doméstica.</p>",
  "obras": [
   "Discurso sobre el origen de la desigualdad (1755)",
   "El contrato social (1762)",
   "Emilio, o De la educación (1762)"
  ],
  "anecdota": "<p>En el verano de 1749, Rousseau caminaba desde París hasta el castillo de Vincennes para visitar a su amigo Diderot, que estaba allí preso. Para descansar, hojeaba el <em>Mercure de France</em>, y en él leyó la pregunta de un concurso de la Academia de Dijon: si el progreso de las ciencias y las artes había mejorado las costumbres. Según contó él mismo, sintió una especie de iluminación repentina, tuvo que sentarse bajo un árbol y se encontró llorando. De aquel momento nació su primer <em>Discurso</em> y, con él, la idea que recorre toda su obra.</p>",
  "fuente": "Rousseau, Confesiones, libro VIII; Segunda carta a Malesherbes (1762)",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-contrato",
   "hf-ilustracion"
  ]
 },
 "diderot": {
  "name": "Denis Diderot",
  "dates": "1713 – 1784",
  "born": 1713,
  "died": 1784,
  "place": "Langres (Francia)",
  "role": "filósofo y escritor ilustrado",
  "idea": "El saber debe reunirse, ordenarse y difundirse para liberar a las personas de la ignorancia y los prejuicios, al servicio del progreso social.",
  "bio": "<p>Denis Diderot nació en Langres en 1713, hijo de un cuchillero, y se instaló en París, donde vivió de traducciones y encargos. Su <em>Carta sobre los ciegos</em>, de tono materialista y crítica con la religión, le costó varios meses de prisión en 1749. Escribió también novelas, teatro y crítica de arte. Murió en París en 1784.</p>\n<p>Su gran obra fue la <strong><em>Enciclopedia</em></strong>, que dirigió durante más de veinte años, al principio junto a D'Alembert. Pretendía reunir todo el saber de la época, incluidas las artes y los oficios, y difundirlo con espíritu crítico, pese a la censura y las prohibiciones. Aparece en el temario como símbolo de la confianza ilustrada en la razón y en el <strong>progreso</strong>: el conocimiento compartido debía mejorar la sociedad.</p>",
  "obras": [
   "Carta sobre los ciegos (1749)",
   "Enciclopedia (1751-1772)",
   "El sobrino de Rameau",
   "Jacques el fatalista"
  ],
  "anecdota": "<p>Hacia 1765, Diderot necesitaba dinero para dotar a su hija y decidió vender su biblioteca, su bien más preciado. La emperatriz Catalina II de Rusia, admiradora de los ilustrados, la compró por una buena suma, pero con una condición generosa: Diderot conservaría los libros en París mientras viviera y cobraría un sueldo como su bibliotecario. Cuando el pago se retrasó, la zarina le adelantó cincuenta años de salario de una vez. En 1773, Diderot viajó a San Petersburgo para agradecérselo y conversó largamente con ella sobre política y reformas.</p>",
  "fuente": "Correspondencia de Diderot; testimonios de la época (Grimm, Correspondance littéraire)",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-ilustracion"
  ]
 },
 "dalembert": {
  "name": "Jean le Rond d'Alembert",
  "dates": "1717 – 1783",
  "born": 1717,
  "died": 1783,
  "place": "París",
  "role": "matemático y filósofo ilustrado",
  "idea": "El conocimiento humano puede ordenarse de forma sistemática a partir de la experiencia y la razón, y la ciencia es el modelo del saber que impulsa el progreso.",
  "bio": "<p>Jean le Rond d'Alembert nació en París en 1717. Fue abandonado al nacer junto a la iglesia de Saint-Jean-le-Rond, de la que tomó su nombre, y lo crió una familia humilde. Destacó muy pronto como matemático y físico: su <em>Tratado de dinámica</em> hizo aportaciones importantes a la mecánica, y entró en la Academia de Ciencias con veintitantos años. Murió en París en 1783.</p>\n<p>Codirigió con Diderot los primeros volúmenes de la <strong><em>Enciclopedia</em></strong> y se encargó de la parte científica. Escribió su <em>Discurso preliminar</em>, que presenta un árbol ordenado de los conocimientos humanos y defiende el valor de la ciencia frente a la tradición y la autoridad. Por eso aparece en el temario junto a Diderot, como ejemplo del afán ilustrado de <strong>sistematizar</strong> el saber al servicio del progreso.</p>",
  "obras": [
   "Tratado de dinámica (1743)",
   "Discurso preliminar de la Enciclopedia (1751)"
  ],
  "anecdota": "<p>El bebé abandonado en las escaleras de Saint-Jean-le-Rond fue criado por la mujer de un vidriero, y D'Alembert, ya célebre académico, siguió viviendo en su modesta casa hasta pasados los cuarenta años. Según la tradición, su madre biológica, la aristócrata Madame de Tencin, intentó reconocerlo cuando se hizo famoso, y él respondió que su única madre era la vidriera. Sea o no exacta la frase, D'Alembert siempre mostró gratitud a quien lo había cuidado, y su éxito se convirtió en un ejemplo ilustrado: el mérito podía más que el nacimiento.</p>",
  "fuente": "Condorcet, Elogio de D'Alembert; tradición biográfica",
  "tradicion": true,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-ilustracion"
  ]
 },
 "smith": {
  "name": "Adam Smith",
  "dates": "1723 – 1790",
  "born": 1723,
  "died": 1790,
  "place": "Kirkcaldy (Escocia)",
  "role": "filósofo moral y economista",
  "idea": "Al buscar cada uno su propio interés en un mercado libre, los individuos contribuyen sin pretenderlo al bienestar general, como guiados por una mano invisible.",
  "bio": "<p>Adam Smith nació en Kirkcaldy, Escocia, en 1723. Estudió en Glasgow y en Oxford, y fue profesor de filosofía moral en la Universidad de Glasgow. Fue amigo de David Hume y una figura central de la Ilustración escocesa. Viajó por Francia como tutor de un joven noble, lo que le permitió conocer a los economistas franceses. En sus últimos años trabajó en la administración de aduanas de Edimburgo, donde murió en 1790.</p>\n<p>En la <em>Teoría de los sentimientos morales</em> explicó la moral a partir de la <strong>simpatía</strong>, la capacidad de ponernos en el lugar de los demás. Pero su obra más influyente es <em>La riqueza de las naciones</em>, considerada el punto de partida de la economía moderna. En ella defiende que la <strong>división del trabajo</strong> multiplica la producción y que la búsqueda del interés propio conduce, sin quererlo, al bienestar de la sociedad: una <strong>mano invisible</strong> armoniza los egoísmos individuales. Por eso pide que el Estado no entorpezca el mercado, aunque le reserva funciones como la defensa, la justicia y ciertas obras públicas.</p>\n<p>Smith es el gran referente del <strong>liberalismo económico</strong>. El temario lo relaciona con el utilitarismo porque ambos valoran las acciones y las instituciones por su utilidad social, y Bentham y John Stuart Mill conocieron bien su obra. Más tarde, Marx criticaría a fondo la economía política clásica que Smith había fundado.</p>",
  "obras": [
   "Teoría de los sentimientos morales (1759)",
   "La riqueza de las naciones (1776)"
  ],
  "anecdota": "<p>Adam Smith era famoso por sus despistes. Sus contemporáneos lo describían hablando solo por la calle y moviendo los labios como si discutiera con alguien invisible. Se cuenta que una mañana salió al jardín en bata, sumido en sus pensamientos, y siguió caminando hasta llegar a otra localidad, a varios kilómetros, donde lo despertaron las campanas de la iglesia. Otra historia asegura que, mientras explicaba la división del trabajo durante la visita a una curtiduría, cayó en una de las cubas. Son relatos exagerados, pero reflejan al pensador absorto que imaginaban sus vecinos.</p>",
  "fuente": "Dugald Stewart, Account of the Life and Writings of Adam Smith (sobre sus despistes); anécdotas de la tradición escocesa",
  "tradicion": true,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-utilitarismo"
  ]
 },
 "kant": {
  "name": "Immanuel Kant",
  "dates": "1724 – 1804",
  "born": 1724,
  "died": 1804,
  "place": "Königsberg (Prusia)",
  "role": "filósofo ilustrado alemán",
  "idea": "Solo conocemos los fenómenos ordenados por las formas a priori del sujeto; y la moral se funda en el deber que la razón autónoma se da a sí misma.",
  "bio": "<p>Immanuel Kant nació en Königsberg, en Prusia, en 1724, en una familia humilde y muy religiosa. Estudió y enseñó toda su vida en la universidad de su ciudad, de la que casi nunca salió, y fue famoso por la regularidad de sus costumbres. A partir de 1781 publicó sus grandes obras críticas. Murió en Königsberg en 1804.</p>\n<p>Su <strong>criticismo</strong> examina los alcances y los límites de la razón a partir de tres preguntas: qué puedo conocer, qué debo hacer y qué me cabe esperar. Frente al dogmatismo racionalista y el escepticismo empirista, propone una <strong>revolución copernicana</strong>: el objeto se ajusta a las estructuras del sujeto. Conocemos los <strong>fenómenos</strong>, ordenados por el espacio, el tiempo y las categorías, pero no el noúmeno o cosa en sí; por eso la metafísica no puede ser ciencia. En ética defiende una moral formal y autónoma, basada en el deber y en el <strong>imperativo categórico</strong>: tratar a la humanidad siempre como un fin y nunca solo como un medio. Resumió la Ilustración con el lema <em>sapere aude</em>, atrévete a saber.</p>\n<p>Kant realiza la síntesis entre el racionalismo de Descartes y el empirismo de Hume, que lo despertó del «sueño dogmático». Su obra abrió el camino al idealismo alemán, de Hegel en adelante, y su idea de la <strong>dignidad</strong> humana inspira hoy la ética y los derechos humanos. Sin embargo, como otros ilustrados, excluyó a las mujeres de esa igualdad universal.</p>",
  "obras": [
   "Crítica de la razón pura (1781)",
   "Fundamentación de la metafísica de las costumbres (1785)",
   "Crítica de la razón práctica (1788)",
   "Crítica del juicio (1790)"
  ],
  "anecdota": "<p>Según la tradición, los vecinos de Königsberg ponían en hora sus relojes cuando veían pasar a Kant en su paseo diario de la tarde, siempre a la misma hora y por el mismo recorrido. Solo una vez, se cuenta, faltó a la cita: los días en que se quedó en casa absorto leyendo el <em>Emilio</em> de Rousseau, recién publicado. La anécdota quizá esté adornada, pero la admiración era real: Kant tenía en su estudio un único retrato, el de Rousseau, y reconocía que él le había enseñado a respetar la dignidad de cualquier ser humano.</p>",
  "fuente": "Tradición biográfica; sobre sus costumbres, biografías de Borowski, Jachmann y Wasianski",
  "tradicion": true,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metodos",
   "hf-ilustracion",
   "hf-kant",
   "hf-etica-deber",
   "hf-descartes-makro"
  ]
 },
 "bentham": {
  "name": "Jeremy Bentham",
  "dates": "1748 – 1832",
  "born": 1748,
  "died": 1832,
  "place": "Londres",
  "role": "filósofo y jurista utilitarista",
  "idea": "Una acción es buena si aumenta el placer y reduce el dolor de los afectados; la moral y las leyes deben buscar la mayor felicidad para el mayor número.",
  "bio": "<p>Jeremy Bentham nació en Londres en 1748. Fue un niño precoz y estudió Derecho en Oxford, pero no quiso ejercer como abogado: prefirió dedicarse a criticar y reformar las leyes inglesas, que consideraba confusas e injustas. Defendió reformas como la ampliación del voto, la libertad de expresión o la mejora de las cárceles, para las que diseñó el <em>Panóptico</em>. Murió en Londres en 1832.</p>\n<p>Bentham es el precursor del <strong>utilitarismo</strong>. Parte de que la naturaleza ha puesto al ser humano bajo dos dueños, el placer y el dolor. Por eso una acción es correcta si produce más placer que dolor en los afectados, y el principio moral y político es buscar <strong>la mayor felicidad para el mayor número</strong>. Lo que cuenta son las consecuencias, no las intenciones. Para decidir, propone el <strong>cálculo hedónico</strong>, que mide los placeres según su intensidad, duración, certeza o cercanía, entre otros criterios. Para él, todos los placeres valen lo mismo: solo se diferencian en cantidad.</p>\n<p>Su discípulo John Stuart Mill, hijo de su colaborador James Mill, revisó su doctrina y distinguió entre placeres superiores e inferiores. El utilitarismo de ambos se opone a la ética del deber de Kant, que juzga las acciones por la intención y no por sus resultados. Sus ideas influyeron mucho en las reformas legales y sociales del siglo XIX.</p>",
  "obras": [
   "Fragmento sobre el gobierno (1776)",
   "Introducción a los principios de la moral y la legislación (1789)",
   "Panóptico (1791)"
  ],
  "anecdota": "<p>Bentham quiso ser útil incluso después de morir. En su testamento dispuso que su cuerpo se usara para la enseñanza de la anatomía y que después su esqueleto, vestido con su ropa y sentado en su silla, se conservara como un «autoicono». Así se hizo: hoy puede verse en una vitrina del University College de Londres, aunque la cabeza es de cera. Circula la leyenda de que asiste a las reuniones del consejo de la universidad con la anotación «presente, pero sin voto». Detrás de la excentricidad había una idea utilitarista: un cadáver también puede servir al bien común.</p>",
  "fuente": "Testamento de Bentham; University College London",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-utilitarismo",
   "hf-etica-deber"
  ]
 },
 "gouges": {
  "name": "Olympe de Gouges",
  "dates": "1748 – 1793",
  "born": 1748,
  "died": 1793,
  "place": "Montauban (Francia)",
  "role": "escritora y activista política",
  "idea": "Los derechos proclamados por la Revolución francesa deben valer también para las mujeres, que son ciudadanas con los mismos derechos que los hombres.",
  "bio": "<p>Olympe de Gouges, cuyo nombre real era Marie Gouze, nació en Montauban, en el sur de Francia, en 1748. Se casó muy joven y enviudó pronto; después se instaló en París, donde se dedicó a escribir obras de teatro y panfletos políticos. En una de sus obras denunció la esclavitud de las personas negras en las colonias. Participó activamente en los debates de la Revolución francesa.</p>\n<p>En 1791 publicó la <em>Declaración de los derechos de la mujer y de la ciudadana</em>. En ella reescribe la <strong>Declaración de los Derechos del Hombre y del Ciudadano</strong> de 1789 para incluir expresamente a las mujeres. Denuncia así la contradicción de una Revolución que proclamaba la <strong>igualdad</strong> universal, pero dejaba fuera a la mitad de la población. Reclama para las mujeres los mismos derechos políticos, el acceso a los cargos públicos, la libertad de expresión y la protección de las madres y de los hijos nacidos fuera del matrimonio. Si la mujer puede subir al cadalso, argumenta, también debe poder subir a la tribuna.</p>\n<p>Se opuso a Robespierre y a los jacobinos, y fue guillotinada en París en 1793. Hoy se la considera una de las pioneras de la <strong>primera ola del feminismo</strong>. En el temario aparece junto a Mary Wollstonecraft como respuesta a los ilustrados, como Rousseau o Kant, que excluían a las mujeres de la ciudadanía.</p>",
  "obras": [
   "Declaración de los derechos de la mujer y de la ciudadana (1791)"
  ],
  "anecdota": "<p>En diciembre de 1792, cuando la Convención se disponía a juzgar a Luis XVI, Olympe de Gouges escribió a los diputados para ofrecerse como defensora del rey, junto al abogado Malesherbes. No era monárquica: defendía que el rey fuera juzgado, pero no ejecutado, y pensaba que una república fuerte no necesitaba derramar su sangre. La Convención rechazó su oferta alegando, entre otras cosas, que era mujer. El episodio muestra su independencia de criterio y su valentía, que acabaron por enfrentarla con los jacobinos.</p>",
  "fuente": "Carta de Olympe de Gouges a la Convención (diciembre de 1792)",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-ilustracion"
  ]
 },
 "wollstonecraft": {
  "name": "Mary Wollstonecraft",
  "dates": "1759 – 1797",
  "born": 1759,
  "died": 1797,
  "place": "Londres",
  "role": "filósofa y escritora",
  "idea": "La supuesta inferioridad intelectual de las mujeres no es natural, sino fruto de su educación; con la misma formación, desarrollarían la misma razón que los hombres.",
  "bio": "<p>Mary Wollstonecraft nació en Londres en 1759, en una familia con problemas económicos. Tuvo que ganarse la vida muy joven como dama de compañía, maestra e institutriz, y después como escritora y traductora para un editor londinense. Viajó a París durante la Revolución francesa. Se casó con el filósofo William Godwin y murió en 1797, pocos días después de dar a luz a su hija, la futura novelista Mary Shelley.</p>\n<p>En 1792 publicó la <em>Vindicación de los derechos de la mujer</em>. En ella critica a Rousseau, que en el <em>Emilio</em> proponía educar a las mujeres solo para agradar a los hombres y ocuparse del hogar. Para Wollstonecraft, la supuesta debilidad de la razón femenina no es natural, sino <strong>cultural</strong>: es el resultado de una educación que las mantiene en la ignorancia. Su tesis es que la <strong>razón no tiene sexo</strong>. Por eso defiende la misma <strong>educación</strong> para niñas y niños, así como la independencia económica de las mujeres, para que puedan ser personas autónomas y ciudadanas.</p>\n<p>Su obra es uno de los textos fundacionales del feminismo moderno. En el temario aparece, junto a Olympe de Gouges, como representante de la primera ola del feminismo, que denunció la contradicción de una Ilustración que proclamaba la igualdad universal pero excluía a las mujeres. Su crítica anticipa ideas que desarrollarían después John Stuart Mill y Simone de Beauvoir.</p>",
  "obras": [
   "Pensamientos sobre la educación de las hijas (1787)",
   "Vindicación de los derechos del hombre (1790)",
   "Vindicación de los derechos de la mujer (1792)"
  ],
  "anecdota": "<p>En 1795, Mary Wollstonecraft emprendió un viaje poco común para una mujer de su tiempo: recorrió Suecia, Noruega y Dinamarca para resolver un asunto comercial de su pareja, acompañada solo por su hija pequeña y una niñera. De esa experiencia nacieron sus <em>Cartas escritas en Suecia, Noruega y Dinamarca</em>, donde mezcla la descripción de paisajes con reflexiones sobre la sociedad y la condición de las mujeres. El libro tuvo mucho éxito, y William Godwin confesó más tarde que, al leerlo, se había enamorado de su autora.</p>",
  "fuente": "Wollstonecraft, Cartas escritas en Suecia, Noruega y Dinamarca; William Godwin, Memorias de la autora de «Vindicación de los derechos de la mujer»",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-ilustracion"
  ]
 },
 "hegel": {
  "name": "Georg Wilhelm Friedrich Hegel",
  "dates": "1770 – 1831",
  "born": 1770,
  "died": 1831,
  "place": "Stuttgart (Alemania)",
  "role": "filósofo idealista alemán",
  "idea": "La realidad es un proceso dialéctico en el que el Espíritu se desarrolla a través de la historia; cada filosofía es su propia época expresada en pensamientos.",
  "bio": "<p>Georg Wilhelm Friedrich Hegel nació en Stuttgart en 1770. Estudió teología en Tubinga, donde fue compañero de Schelling y del poeta Hölderlin, y siguió con entusiasmo la Revolución francesa. Tras enseñar en Jena y Heidelberg, obtuvo en 1818 la cátedra de Berlín, donde alcanzó enorme prestigio. Murió en Berlín en 1831.</p>\n<p>Hegel es la gran figura del <strong>idealismo alemán</strong>. Para él, la realidad no es algo fijo, sino un proceso en el que el <strong>Espíritu</strong> o la Idea se despliega y toma conciencia de sí mismo a lo largo de la historia. Ese movimiento es <strong>dialéctico</strong>: avanza a través de contradicciones que se superan conservando lo valioso de cada momento. Por eso la filosofía es inseparable de la <strong>historicidad</strong>: cada filosofía es su propia época aprehendida en pensamientos. En la <em>Fenomenología del espíritu</em> expone la dialéctica del amo y el esclavo, según la cual la identidad se forma en la lucha por el reconocimiento del otro. En estética, sostiene que el arte es la manifestación sensible de la verdad, pero que en el mundo moderno ha perdido su papel central frente a la religión y la filosofía: es la tesis del «fin del arte».</p>\n<p>Su influencia fue inmensa. Marx transformó su dialéctica en un método materialista para analizar la historia y la sociedad. Simone de Beauvoir aplicó la dialéctica del amo y el esclavo a la relación entre hombres y mujeres.</p>",
  "obras": [
   "Fenomenología del espíritu (1807)",
   "Ciencia de la lógica (1812-1816)",
   "Enciclopedia de las ciencias filosóficas (1817)",
   "Fundamentos de la filosofía del derecho (1821)"
  ],
  "anecdota": "<p>En octubre de 1806, Hegel estaba en Jena terminando la <em>Fenomenología del espíritu</em> cuando las tropas de Napoleón ocuparon la ciudad, la víspera de la famosa batalla. Ese día escribió a su amigo Niethammer que había visto pasar al emperador a caballo, «esa alma del mundo», y que era una sensación extraordinaria contemplar a un individuo que, concentrado en un punto, dominaba el mundo. Se cuenta que tuvo que enviar las últimas páginas del manuscrito entre el caos de la guerra. Para Hegel, la historia universal parecía avanzar ante sus propios ojos.</p>",
  "fuente": "Carta de Hegel a Niethammer, 13 de octubre de 1806",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-historicidad",
   "hf-metodos",
   "hf-beauvoir"
  ]
 },
 "comte": {
  "name": "Auguste Comte",
  "dates": "1798 – 1857",
  "born": 1798,
  "died": 1857,
  "place": "Montpellier (Francia)",
  "role": "filósofo positivista y sociólogo",
  "idea": "El único conocimiento válido es el positivo, basado en hechos observables y leyes científicas; la humanidad progresa del estadio teológico al metafísico y al positivo.",
  "bio": "<p>Auguste Comte nació en Montpellier en 1798. Estudió en la Escuela Politécnica de París, de la que fue expulsado tras una protesta estudiantil. Fue secretario del pensador socialista Saint-Simon, con quien acabó rompiendo. Nunca obtuvo una cátedra universitaria y vivió de clases particulares y de la ayuda de sus seguidores. Murió en París en 1857.</p>\n<p>Comte es el fundador del <strong>positivismo</strong>. Sostiene que el único conocimiento verdadero es el que se basa en la observación y en los <strong>hechos verificables</strong>, y rechaza la especulación metafísica. Según su <strong>ley de los tres estadios</strong>, la humanidad pasa por un estadio teológico, en el que explica los fenómenos por dioses; uno metafísico, en el que recurre a esencias abstractas; y uno positivo, en el que busca leyes científicas. Propuso también una ciencia de la sociedad, a la que llamó <strong>sociología</strong>, para reorganizar la vida social de acuerdo con la ciencia. En su última etapa ideó una religión de la humanidad.</p>\n<p>El positivismo representa el punto culminante de la confianza moderna en la ciencia y el progreso. Influyó en John Stuart Mill y en la ciencia social del siglo XIX, y su espíritu reaparece en el neopositivismo del Círculo de Viena. Más tarde, la Escuela de Fráncfort, con Horkheimer, criticó esta visión por reducir la razón a un mero instrumento.</p>",
  "obras": [
   "Curso de filosofía positiva (1830-1842)",
   "Discurso sobre el espíritu positivo (1844)",
   "Sistema de política positiva (1851-1854)"
  ],
  "anecdota": "<p>En su madurez, Comte practicó lo que él llamaba «higiene cerebral»: decidió dejar de leer periódicos, revistas y libros de otros autores para que las ideas ajenas no perturbaran la elaboración de su propio sistema. Solo se permitía releer unos pocos clásicos, como poetas que le servían de descanso. La costumbre sorprendió incluso a sus admiradores, porque el fundador de una filosofía basada en los hechos se aislaba de las novedades de su tiempo. Muestra la ambición de Comte: construir él solo una síntesis completa de todas las ciencias.</p>",
  "fuente": "Testimonios del propio Comte y de sus discípulos",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-modernidad"
  ]
 },
 "feuerbach": {
  "name": "Ludwig Feuerbach",
  "dates": "1804 – 1872",
  "born": 1804,
  "died": 1872,
  "place": "Landshut (Baviera)",
  "role": "filósofo materialista alemán",
  "idea": "Dios no es más que la proyección de las cualidades humanas idealizadas; al adorarlo, el ser humano se aliena y se separa de su propia esencia.",
  "bio": "<p>Ludwig Feuerbach nació en Baviera y estudió teología y después filosofía en Berlín, donde fue alumno de Hegel. Formó parte de los llamados jóvenes hegelianos o hegelianos de izquierda. Sus ideas sobre la religión le cerraron la carrera universitaria, y vivió la mayor parte de su vida retirado, dedicado a escribir.</p>\n<p>En <em>La esencia del cristianismo</em> sostiene que la religión es una <strong>proyección</strong>: el ser humano atribuye a Dios sus propias cualidades (bondad, sabiduría, amor) y, al hacerlo, se empobrece a sí mismo. Es una forma de <strong>alienación</strong>. Frente al idealismo de Hegel, defiende un <strong>materialismo</strong> centrado en el ser humano concreto y sensible. Aparece en el temario porque influyó decisivamente en el joven Marx, que tomó de él la idea de alienación y la criticó en sus <em>Tesis sobre Feuerbach</em>.</p>",
  "obras": [
   "La esencia del cristianismo (1841)",
   "Tesis provisionales para la reforma de la filosofía (1842)",
   "Principios de la filosofía del futuro (1843)"
  ],
  "anecdota": "<p>En 1850, Feuerbach escribió una reseña de un libro de divulgación sobre la alimentación del fisiólogo Jacob Moleschott. En ella dejó una frase que se haría célebre gracias a un juego de palabras alemán: <em>Der Mensch ist, was er isst</em>, «el hombre es lo que come» (en alemán, «es» y «come» suenan casi igual). Con ese chiste serio resumía su materialismo: el ser humano no es un espíritu desencarnado, sino un ser concreto, sensible y dependiente de sus condiciones materiales. Sus críticos vieron en ello una vulgarización; él, una defensa del cuerpo frente al idealismo.</p>",
  "fuente": "Feuerbach, reseña de la obra de Moleschott sobre la alimentación (1850)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-capitalismo"
  ]
 },
 "mill": {
  "name": "John Stuart Mill",
  "dates": "1806 – 1873",
  "born": 1806,
  "died": 1873,
  "place": "Londres",
  "role": "filósofo y economista británico",
  "idea": "Es correcta la acción que promueve la mayor felicidad del mayor número, pero los placeres intelectuales y morales valen más que los puramente físicos.",
  "bio": "<p>John Stuart Mill nació en Londres. Su padre, el economista James Mill, amigo de Jeremy Bentham, le dio una educación muy exigente: aprendió griego a los tres años. A los veinte sufrió una crisis personal que le llevó a valorar los sentimientos y la poesía. Trabajó en la Compañía de las Indias Orientales, fue diputado en el Parlamento británico y colaboró estrechamente con Harriet Taylor, con quien se casó. Murió en Aviñón (Francia).</p>\n<p>Mill es, junto con Bentham, el gran representante del <strong>utilitarismo</strong>, una ética consecuencialista: una acción es buena si produce felicidad y mala si produce dolor. Mantiene el <strong>principio de la mayor felicidad</strong>, pero corrige a Bentham al introducir la calidad de los placeres: los <strong>placeres superiores</strong> (intelectuales y morales) valen más que los físicos, y por eso es preferible ser un ser humano insatisfecho que un cerdo satisfecho. En <em>Sobre la libertad</em> defiende que el individuo es soberano sobre sí mismo: la sociedad solo puede limitar su libertad para evitar daños a otros.</p>\n<p>Su obra es una referencia del <strong>liberalismo</strong> y de la defensa de la igualdad entre mujeres y hombres; reclamó el sufragio femenino en el Parlamento. En el curso se contrapone a la ética del deber de Kant, que juzga las acciones por la intención y no por sus consecuencias, y se relaciona con el liberalismo de Locke y Adam Smith.</p>",
  "obras": [
   "Sistema de lógica (1843)",
   "Sobre la libertad (1859)",
   "El utilitarismo (1863)",
   "El sometimiento de la mujer (1869)"
  ],
  "anecdota": "<p>En 1865, durante su campaña para el Parlamento, Mill se reunió con un público de trabajadores. Un adversario había difundido un cartel con una frase de uno de sus libros, donde decía que las clases trabajadoras, aunque se avergonzaban de mentir, solían ser mentirosas. Le preguntaron si la había escrito, y Mill respondió sin dudar: «Sí, la escribí». Según su <em>Autobiografía</em>, el público estalló en aplausos, porque valoraba que un candidato no intentara esquivar la pregunta. Mill ganó el escaño y lo usó para defender, entre otras causas, el voto de las mujeres.</p>",
  "fuente": "John Stuart Mill, Autobiografía",
  "tradicion": false,
  "block": "mod",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-utilitarismo",
   "hf-etica-deber"
  ]
 },
 "darwin": {
  "name": "Charles Darwin",
  "dates": "1809 – 1882",
  "born": 1809,
  "died": 1882,
  "place": "Shrewsbury (Inglaterra)",
  "role": "naturalista británico",
  "idea": "Todas las especies descienden de antepasados comunes y se transforman lentamente por selección natural, que favorece a los individuos mejor adaptados a su medio.",
  "bio": "<p>Charles Darwin nació en Shrewsbury, en una familia acomodada. Empezó Medicina en Edimburgo y después estudió en Cambridge para ser clérigo, pero su verdadera pasión era la historia natural. Entre 1831 y 1836 dio la vuelta al mundo como naturalista a bordo del <em>Beagle</em>. Observó fósiles, animales y plantas en Sudamérica y en las islas Galápagos, con sus conocidos pinzones. Durante más de veinte años reunió pruebas antes de publicar su teoría.</p>\n<p>En <em>El origen de las especies</em> sostiene que los seres vivos tienen un <strong>origen común</strong> y que las especies cambian por <strong>selección natural</strong>. En cada generación nacen individuos con pequeñas variaciones; los mejor adaptados sobreviven y se reproducen más, y transmiten esos rasgos. Alfred R. Wallace llegó a la misma idea de forma independiente, y ambos la presentaron juntos en 1858. En <em>El origen del hombre</em> aplicó la teoría al ser humano, emparentado con los demás primates.</p>\n<p>Su teoría acabó con el <strong>fijismo</strong> y, unida después a la genética de Mendel, dio lugar al neodarwinismo o teoría sintética. Cambió también la imagen que el ser humano tenía de sí mismo: Freud la consideró la segunda gran humillación, después de la de Copérnico. Algunas interpretaciones posteriores, como el darwinismo social, usaron sus ideas para justificar la desigualdad, algo que el propio Darwin no propuso.</p>",
  "obras": [
   "El viaje del Beagle (1839)",
   "El origen de las especies (1859)",
   "El origen del hombre (1871)"
  ],
  "anecdota": "<p>De estudiante en Cambridge, Darwin era un apasionado coleccionista de escarabajos. En su <em>Autobiografía</em> recuerda que un día arrancó la corteza de un árbol y encontró dos ejemplares raros; cogió uno con cada mano, y en ese momento vio un tercero, de una especie nueva que no quería perder. Sin pensarlo, se metió en la boca uno de los que tenía en la mano. El insecto soltó un líquido ácido que le quemó la lengua, lo escupió y perdió el tercero. Aquella curiosidad sin límites fue la que después alimentó su viaje en el <em>Beagle</em>.</p>",
  "fuente": "Charles Darwin, Autobiografía",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-sospecha"
  ]
 },
 "marx": {
  "name": "Karl Marx",
  "dates": "1818 – 1883",
  "born": 1818,
  "died": 1883,
  "place": "Tréveris (Prusia)",
  "role": "filósofo, economista y revolucionario",
  "idea": "La estructura económica determina la sociedad y sus ideas; bajo el capitalismo el trabajador está alienado y explotado, y solo una revolución puede crear una sociedad sin clases.",
  "bio": "<p>Karl Marx nació en Tréveris, en una familia de origen judío. Estudió Derecho y Filosofía en Bonn y Berlín, donde se unió a los jóvenes hegelianos. Como periodista crítico sufrió la censura y tuvo que exiliarse: vivió en París, Bruselas y, desde 1849, en Londres, casi siempre con apuros económicos. Allí contó con el apoyo de su amigo y colaborador Friedrich Engels. Participó en la organización del movimiento obrero internacional.</p>\n<p>Su filosofía es el <strong>materialismo histórico</strong>: la estructura económica determina la superestructura (política, derecho, moral, religión). La historia avanza por la <strong>lucha de clases</strong>. En el capitalismo, la burguesía posee los medios de producción y el proletariado solo su fuerza de trabajo; el obrero produce más valor del que recibe, y esa diferencia es la <strong>plusvalía</strong>. El trabajo, que debería realizar al ser humano, lo <strong>aliena</strong>: lo separa de su producto, de sí mismo y de los demás. Las ideas dominantes son <strong>ideología</strong>, una falsa conciencia que oculta la explotación, y la religión es el opio del pueblo.</p>\n<p>Marx es uno de los tres maestros de la sospecha, junto con Nietzsche y Freud. Partió de Hegel y de Feuerbach, pero quiso transformar el mundo y no solo interpretarlo. Su pensamiento inspiró los movimientos socialistas y comunistas, y fue discutido en el siglo XX por la Escuela de Fráncfort, Popper y Habermas.</p>",
  "obras": [
   "Manuscritos económico-filosóficos (1844)",
   "Tesis sobre Feuerbach (1845)",
   "Manifiesto comunista (1848)",
   "El capital (1867)"
  ],
  "anecdota": "<p>En sus últimos años, Marx vio con disgusto cómo algunos socialistas franceses que se llamaban «marxistas» simplificaban sus ideas hasta convertirlas en un dogma. Según relató Engels en una carta de 1882, Marx comentó ante ellos con ironía: «Lo único seguro es que yo no soy marxista». La frase muestra que Marx no quería que su obra se repitiera como una doctrina cerrada, sino que se usara como un método para analizar críticamente la sociedad. Paradójicamente, tras su muerte el «marxismo» se convirtió en una de las ideologías más influyentes del siglo XX.</p>",
  "fuente": "Carta de Engels a Eduard Bernstein (1882)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metodos",
   "hf-sospecha",
   "hf-capitalismo"
  ]
 },
 "kropotkin": {
  "name": "Piotr Kropotkin",
  "dates": "1842 – 1921",
  "born": 1842,
  "died": 1921,
  "place": "Moscú",
  "role": "geógrafo y pensador anarquista",
  "idea": "El apoyo mutuo es un factor de la evolución tan importante como la lucha: las especies que cooperan prosperan, y en ese instinto social se basa la moral.",
  "bio": "<p>Piotr Kropotkin nació en Moscú en una familia de la alta nobleza rusa; tenía el título de príncipe. Como oficial y geógrafo realizó expediciones científicas por Siberia. Renunció a sus privilegios, se hizo anarquista y fue encarcelado en Rusia y en Francia. Vivió muchos años exiliado en Inglaterra y regresó a Rusia tras la revolución de 1917, aunque criticó el autoritarismo bolchevique.</p>\n<p>En <em>El apoyo mutuo</em> discute las lecturas del darwinismo que solo ven competencia en la naturaleza. Con numerosos ejemplos de animales y sociedades humanas, sostiene que la <strong>cooperación</strong> es un factor decisivo de la evolución y que el <strong>instinto social</strong> está en la base de la moral. Aparece en el temario como alternativa al darwinismo social y a la visión del ser humano como egoísta y competitivo por naturaleza.</p>",
  "obras": [
   "La conquista del pan (1892)",
   "Campos, fábricas y talleres (1899)",
   "El apoyo mutuo (1902)"
  ],
  "anecdota": "<p>En 1876, Kropotkin estaba encerrado en la fortaleza de San Pedro y San Pablo de San Petersburgo y, enfermo, fue trasladado al hospital militar de la prisión. Allí sus compañeros prepararon una fuga minuciosa: durante el paseo en el patio, un violinista tocaba desde una casa cercana como señal de que el camino estaba libre. Kropotkin se quitó la pesada bata de preso, echó a correr hasta la puerta y saltó a un carruaje que lo esperaba. Días después huyó de Rusia y comenzó un largo exilio en Europa.</p>",
  "fuente": "Piotr Kropotkin, Memorias de un revolucionario",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-utilitarismo"
  ]
 },
 "nietzsche": {
  "name": "Friedrich Nietzsche",
  "dates": "1844 – 1900",
  "born": 1844,
  "died": 1900,
  "place": "Röcken (Sajonia, Prusia)",
  "role": "filósofo y filólogo alemán",
  "idea": "Tras la muerte de Dios y la caída de los valores tradicionales, el ser humano debe superar el nihilismo afirmando la vida y creando sus propios valores.",
  "bio": "<p>Friedrich Nietzsche nació en Röcken, hijo de un pastor protestante. Estudió filología clásica y con solo 24 años fue nombrado profesor en la Universidad de Basilea (Suiza). Su mala salud le obligó a dejar la cátedra en 1879; desde entonces vivió de forma errante por Suiza, Italia y el sur de Francia, escribiendo sin descanso. En 1889 sufrió en Turín un colapso mental del que no se recuperó. Murió en Weimar.</p>\n<p>Su filosofía es un <strong>vitalismo</strong>: la vida (instinto, cuerpo, pasión) es el valor supremo. Distingue lo dionisíaco y lo apolíneo, y acusa a Platón y al cristianismo de inventar un «mundo verdadero» contra la vida. Con su método <strong>genealógico</strong> muestra que la moral tradicional nace del resentimiento de los débiles: es una moral de esclavos. La muerte de Dios abre el <strong>nihilismo</strong>, que el <strong>superhombre</strong> debe superar creando sus propios valores mediante la <strong>voluntad de poder</strong> y aceptando el eterno retorno.</p>\n<p>Es uno de los tres maestros de la sospecha, junto con Marx y Freud. Escribió a menudo en aforismos y con un estilo literario. Tras su muerte, su hermana manipuló parte de sus escritos, lo que favoreció lecturas deformadas de su obra. Su perspectivismo y su crítica de la verdad influyeron en el existencialismo y en los pensadores posmodernos, como Foucault.</p>",
  "obras": [
   "El nacimiento de la tragedia (1872)",
   "La gaya ciencia (1882)",
   "Así habló Zaratustra (1883-1885)",
   "La genealogía de la moral (1887)"
  ],
  "anecdota": "<p>En 1882, con la vista cada vez más débil, Nietzsche compró una de las primeras máquinas de escribir, una curiosa «bola de escribir» danesa con las teclas dispuestas sobre una semiesfera. La usó durante unas semanas y llegó a teclear con ella algunas cartas y poemas breves, hasta que el aparato se estropeó. A su amigo Heinrich Köselitz, que notaba su estilo aún más conciso, le respondió que nuestras herramientas de escritura también colaboran en nuestros pensamientos. La frase anticipa una reflexión muy actual sobre cómo la técnica transforma nuestra forma de pensar.</p>",
  "fuente": "Carta de Nietzsche a Heinrich Köselitz (febrero de 1882)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-metodos",
   "hf-sospecha",
   "hf-posmodernidad"
  ]
 },
 "freud": {
  "name": "Sigmund Freud",
  "dates": "1856 – 1939",
  "born": 1856,
  "died": 1939,
  "place": "Freiberg (Moravia, hoy República Checa)",
  "role": "médico neurólogo, creador del psicoanálisis",
  "idea": "La mayor parte de la vida psíquica es inconsciente: deseos reprimidos que no controlamos gobiernan lo que pensamos y hacemos, y su represión produce neurosis.",
  "bio": "<p>Sigmund Freud nació en Freiberg, en una familia judía que pronto se trasladó a Viena. Allí estudió Medicina y se especializó en neurología. Tras trabajar en París con Charcot en el estudio de la histeria, desarrolló un método propio para tratar las enfermedades nerviosas: el <strong>psicoanálisis</strong>. Vivió y trabajó en Viena casi toda su vida, hasta que en 1938, tras la anexión de Austria por la Alemania nazi, se exilió en Londres, donde murió al año siguiente.</p>\n<p>Para Freud, la conciencia es solo la punta del iceberg: la mayor parte de la psique es <strong>inconsciente</strong> y está gobernada por deseos reprimidos, que se manifiestan en los sueños, los lapsus y los síntomas. Distingue tres instancias: el <strong>ello</strong> (los instintos, regido por el principio de placer), el yo (que media con la realidad) y el <strong>superyó</strong> (las normas morales interiorizadas). Las pulsiones básicas son Eros (vida) y Tánatos (muerte). La vida en sociedad exige reprimir deseos, y esa represión genera <strong>neurosis</strong>, por lo que la felicidad plena es imposible en la civilización.</p>\n<p>Es el tercero de los maestros de la sospecha, junto con Marx y Nietzsche. Presentó su teoría como la tercera gran humillación del ser humano, tras las de Copérnico y Darwin. Su influencia llega a la psicología, la literatura, el arte y la filosofía del siglo XX, en especial a la Escuela de Fráncfort.</p>",
  "obras": [
   "La interpretación de los sueños (1900)",
   "Tres ensayos sobre teoría sexual (1905)",
   "El yo y el ello (1923)",
   "El malestar en la cultura (1930)"
  ],
  "anecdota": "<p>En mayo de 1933, los nazis quemaron en Berlín y otras ciudades alemanas los libros de autores que consideraban enemigos, entre ellos los de Freud. Según su biógrafo Ernest Jones, Freud comentó con amarga ironía: «¡Cuánto hemos progresado! En la Edad Media me habrían quemado a mí; hoy se conforman con quemar mis libros». No podía imaginar entonces que el nazismo llegaría mucho más lejos. Cinco años después, la anexión de Austria le obligó a abandonar Viena. La frase muestra su lucidez y su humor ante la barbarie.</p>",
  "fuente": "Ernest Jones, Vida y obra de Sigmund Freud",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-sospecha"
  ]
 },
 "unamuno": {
  "name": "Miguel de Unamuno",
  "dates": "1864 – 1936",
  "born": 1864,
  "died": 1936,
  "place": "Bilbao",
  "role": "filósofo, novelista y poeta",
  "idea": "El ser humano de carne y hueso vive en agonía, en lucha permanente entre la razón, que niega la inmortalidad, y el corazón, que la anhela.",
  "bio": "<p>Miguel de Unamuno nació en Bilbao y estudió Filosofía y Letras en Madrid. Desde 1891 fue catedrático de griego en la Universidad de Salamanca, de la que llegó a ser rector. Es una de las grandes figuras de la Generación del 98. Por sus críticas a la dictadura de Primo de Rivera fue desterrado a Fuerteventura en 1924 y después vivió exiliado en Francia hasta 1930. En 1936 se enfrentó públicamente a los sublevados en Salamanca; fue destituido y murió poco después, bajo arresto domiciliario.</p>\n<p>Unamuno representa un <strong>existencialismo cristiano</strong>. Le interesa el «hombre de carne y hueso», el individuo concreto, no las abstracciones. En <em>Del sentimiento trágico de la vida</em> describe al ser humano como un ser en <strong>agonía</strong>, palabra que usa en su sentido griego de lucha: la razón le dice que no hay inmortalidad, pero el corazón la desea con todas sus fuerzas. De ese conflicto sin solución nace el <strong>sentimiento trágico de la vida</strong>. Su fe no es una certeza tranquila, sino una fe que duda. Frente al «pienso, luego existo» de Descartes, sitúa el sentimiento y la voluntad en el centro del ser humano.</p>\n<p>Expresó su pensamiento también en novelas como <em>Niebla</em> y <em>San Manuel Bueno, mártir</em>. Se le considera un precursor del existencialismo del siglo XX, con afinidades con Kierkegaard, y es, junto con Ortega y Gasset y María Zambrano, una de las voces principales de la filosofía española contemporánea.</p>",
  "obras": [
   "Del sentimiento trágico de la vida (1913)",
   "Niebla (1914)",
   "La agonía del cristianismo (1925)",
   "San Manuel Bueno, mártir (1931)"
  ],
  "anecdota": "<p>Unamuno tenía una afición inesperada: hacer pajaritas de papel. Las doblaba en tertulias, en cafés y en casa, y llegó a tomárselas tan en serio como para inventar una disciplina, la <strong>cocotología</strong>, a la que dedicó un tratado burlesco al final de su novela <em>Amor y pedagogía</em> (1902). Con falsa solemnidad científica clasificaba y analizaba las pajaritas, como si fueran especies animales. Era una forma de reírse de la pedantería de quienes creen que todo puede reducirse a sistema, algo muy propio de un pensador que desconfiaba de las abstracciones.</p>",
  "fuente": "Unamuno, Amor y pedagogía (1902), «Apuntes para un tratado de cocotología»",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-existencialismo"
  ]
 },
 "weber": {
  "name": "Max Weber",
  "dates": "1864 – 1920",
  "born": 1864,
  "died": 1920,
  "place": "Erfurt (Alemania)",
  "role": "sociólogo, economista y filósofo alemán",
  "idea": "El Estado moderno se apoya en una legitimidad legal y racional, y la acción política debe combinar la ética de la convicción con la ética de la responsabilidad.",
  "bio": "<p>Max Weber nació en Erfurt en una familia burguesa y se formó en derecho, economía e historia. Fue profesor en varias universidades, sobre todo en Heidelberg, aunque una grave crisis nerviosa le obligó a dejar la docencia durante años. Vivió la industrialización de Alemania y la Primera Guerra Mundial, y murió en Múnich en 1920. Se le considera uno de los fundadores de la <strong>sociología</strong> moderna.</p>\n<p>En filosofía política es conocido por su análisis del poder. Definió el Estado como la organización que reclama con éxito el monopolio de la violencia legítima, y distinguió tres fuentes de <strong>legitimidad</strong>: la tradición, el carisma de un líder y la legalidad racional, propia del Estado moderno, en la que se obedece a las leyes y no a las personas. En ética distinguió la <strong>ética de la convicción</strong>, que actúa según principios sin mirar los resultados, y la <strong>ética de la responsabilidad</strong>, que responde de las consecuencias previsibles. Para él, el buen político necesita ambas.</p>\n<p>Su estudio sobre el origen religioso del capitalismo y su idea del desencantamiento del mundo, es decir, la pérdida de las explicaciones mágicas o religiosas ante la ciencia y la burocracia, influyeron en la Escuela de Fráncfort y en Habermas. El temario recoge también su distinción de éticas a propósito de Joxe Azurmendi.</p>",
  "obras": [
   "La ética protestante y el espíritu del capitalismo (1905)",
   "La ciencia como vocación (1919)",
   "La política como vocación (1919)",
   "Economía y sociedad (1922)"
  ],
  "anecdota": "<p>En enero de 1919, con Alemania derrotada y Múnich en plena agitación revolucionaria, una asociación de estudiantes invitó a Weber a hablarles de política. Muchos jóvenes esperaban oír que la revolución traería un mundo nuevo. Weber les ofreció justo lo contrario: una reflexión sobria sobre el poder, la responsabilidad y los límites de las buenas intenciones. Terminó comparando la política con <strong>perforar lenta y tenazmente tablas duras</strong>, un trabajo que exige pasión y mesura a la vez. Esa conferencia, <em>La política como vocación</em>, es el texto donde distingue la ética de la convicción y la de la responsabilidad.</p>",
  "fuente": "Weber, La política como vocación (conferencia de 1919); Marianne Weber, biografía de Max Weber",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-etica-deber"
  ]
 },
 "russell": {
  "name": "Bertrand Russell",
  "dates": "1872 – 1970",
  "born": 1872,
  "died": 1970,
  "place": "Trellech (Gales)",
  "role": "filósofo, lógico y matemático británico",
  "idea": "El análisis lógico del lenguaje permite aclarar los problemas filosóficos, y la matemática puede fundarse en la lógica; defendió un lenguaje lógicamente perfecto que refleje la estructura del mundo.",
  "bio": "<p>Bertrand Russell pertenecía a una familia aristocrática británica y estudió en Cambridge, donde fue alumno de Whitehead y amigo de Moore; más tarde fue maestro de Wittgenstein. Fue pacifista durante la Primera Guerra Mundial, lo que le costó la cárcel, y luchó después contra las armas nucleares. En 1950 recibió el Premio Nobel de Literatura.</p>\n<p>Es uno de los fundadores de la <strong>filosofía analítica</strong> y del <strong>atomismo lógico</strong>: el mundo está formado por hechos simples, y un lenguaje lógicamente perfecto debería reflejar su estructura. Con Whitehead escribió los <em>Principia Mathematica</em>, un intento de derivar la aritmética de la lógica. En su <em>Historia de la filosofía occidental</em> explicó el paso del mito al logos atendiendo también a sus causas sociales, como la riqueza y el ocio de las colonias jonias.</p>",
  "obras": [
   "Principia Mathematica (1910-1913, con Whitehead)",
   "Los problemas de la filosofía (1912)",
   "Historia de la filosofía occidental (1945)"
  ],
  "anecdota": "<p>En 1902 Gottlob Frege estaba a punto de publicar el segundo volumen de su gran obra, donde fundaba la aritmética en la lógica. Entonces recibió una carta de un joven Russell, muy respetuosa, que le planteaba una dificultad: el conjunto de todos los conjuntos que no se contienen a sí mismos, ¿se contiene o no? Cualquier respuesta lleva a contradicción. Frege respondió con gran honestidad que aquello hacía tambalear sus cimientos y añadió un apéndice reconociéndolo. La <strong>paradoja de Russell</strong> obligó a repensar los fundamentos de las matemáticas y está en el origen de los <em>Principia Mathematica</em>.</p>",
  "fuente": "Correspondencia entre Russell y Frege (1902); Frege, apéndice al vol. II de Las leyes fundamentales de la aritmética",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-mito",
   "hf-analitica"
  ]
 },
 "moore": {
  "name": "George Edward Moore",
  "dates": "1873 – 1958",
  "born": 1873,
  "died": 1958,
  "place": "Londres (Inglaterra)",
  "role": "filósofo británico",
  "idea": "La filosofía debe partir del sentido común y aclarar con precisión el significado de lo que decimos; el bien es una cualidad simple que no puede definirse.",
  "bio": "<p>George Edward Moore estudió en el Trinity College de Cambridge, donde fue profesor durante casi toda su vida. Allí trabó amistad con Bertrand Russell, con quien rompió con el idealismo que dominaba entonces la filosofía británica. Formó parte del ambiente intelectual de Cambridge junto a Russell y Wittgenstein.</p>\n<p>Aparece en el temario como uno de los iniciadores de la <strong>filosofía analítica</strong>. Su método consistía en analizar con cuidado el significado de las afirmaciones filosóficas y en defender las creencias del <strong>sentido común</strong> (que existe un mundo exterior, que tengo un cuerpo) frente a las teorías que las niegan. En ética sostuvo en <em>Principia Ethica</em> que el bien es una cualidad simple e indefinible, y llamó <strong>falacia naturalista</strong> al error de identificarlo con algo natural, como el placer.</p>",
  "obras": [
   "Principia Ethica (1903)",
   "La refutación del idealismo (1903)",
   "Defensa del sentido común (1925)"
  ],
  "anecdota": "<p>En 1939, ante la Academia Británica, Moore ofreció la que presentó como una prueba de la existencia del mundo exterior. Levantó una mano y dijo, más o menos: «Aquí hay una mano». Luego levantó la otra: «Y aquí hay otra». Por tanto, concluyó, existen al menos dos objetos externos. Muchos oyentes lo tomaron por una broma, pero Moore hablaba en serio: estaba más seguro de tener manos que de cualquier premisa de un argumento escéptico. El gesto resume su defensa del <strong>sentido común</strong> frente a las filosofías que lo ponen en duda.</p>",
  "fuente": "Moore, «Prueba del mundo exterior» (1939)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-analitica"
  ]
 },
 "schlick": {
  "name": "Moritz Schlick",
  "dates": "1882 – 1936",
  "born": 1882,
  "died": 1936,
  "place": "Berlín (Alemania)",
  "role": "filósofo y físico alemán",
  "idea": "El significado de una proposición es su método de verificación: lo que no puede comprobarse empíricamente ni es una verdad lógica carece de sentido.",
  "bio": "<p>Moritz Schlick se doctoró en física en Berlín con Max Planck y se interesó pronto por la teoría de la relatividad de Einstein. En 1922 obtuvo una cátedra en la Universidad de Viena, donde reunió a un grupo de filósofos y científicos que acabaría conociéndose como el <strong>Círculo de Viena</strong>. En 1936 fue asesinado en la universidad por un antiguo alumno.</p>\n<p>Fue el fundador y principal organizador del <strong>neopositivismo lógico</strong>. Influido por el <em>Tractatus</em> de Wittgenstein, sostuvo que el sentido de una proposición depende de que pueda verificarse: solo tienen significado los enunciados comprobables mediante la experiencia y las verdades de la lógica. De ahí la conclusión, central en el temario, de que la <strong>metafísica</strong> no es falsa, sino carente de significado.</p>",
  "obras": [
   "Teoría general del conocimiento (1918)",
   "Problemas de ética (1930)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-analitica"
  ]
 },
 "ortega": {
  "name": "José Ortega y Gasset",
  "dates": "1883 – 1955",
  "born": 1883,
  "died": 1955,
  "place": "Madrid",
  "role": "filósofo y ensayista español",
  "idea": "La vida de cada persona es la realidad radical: el yo es inseparable de su circunstancia, y la verdad se alcanza sumando las distintas perspectivas.",
  "bio": "<p>José Ortega y Gasset nació en Madrid en una familia vinculada al periodismo. Amplió estudios en Alemania, sobre todo en Marburgo, donde conoció el neokantismo, y en 1910 obtuvo la cátedra de Metafísica de la Universidad de Madrid. Quiso modernizar España acercándola a la cultura europea: fundó la <em>Revista de Occidente</em> en 1923 y participó en la vida política. Con la Guerra Civil se exilió; vivió en Francia, Argentina y Portugal, y volvió a Madrid, donde murió en 1955.</p>\n<p>Su filosofía, el <strong>raciovitalismo</strong>, parte de que la vida de cada uno es la realidad radical. La razón pura, abstracta, no basta para comprenderla; hace falta una <strong>razón vital</strong>, ligada a la vida concreta. Lo resume su fórmula «Yo soy yo y mi circunstancia», de las <em>Meditaciones del Quijote</em>: no somos un yo aislado, sino un yo que se hace en una época, un cuerpo y una sociedad. De ahí su <strong>perspectivismo</strong>: cada persona ve la realidad desde un punto de vista, nadie posee la verdad absoluta y la verdad resulta de integrar las perspectivas.</p>\n<p>Fue el filósofo español más influyente del siglo XX y formó a una generación de pensadores, entre ellos María Zambrano, que desarrolló la razón poética a partir de su razón vital. Su análisis del hombre-masa en <em>La rebelión de las masas</em> tuvo gran difusión internacional.</p>",
  "obras": [
   "Meditaciones del Quijote (1914)",
   "El tema de nuestro tiempo (1923)",
   "La rebelión de las masas (1930)"
  ],
  "anecdota": "<p>En noviembre de 1930 Ortega publicó en el diario <em>El Sol</em> un artículo, «El error Berenguer», que terminaba con una frase latina convertida en consigna: «Delenda est Monarchia», la monarquía debe ser destruida. Meses después llegó la Segunda República, y Ortega fue elegido diputado. Pero pronto le decepcionó el rumbo de la nueva política y, en septiembre de 1931, escribió otro artículo con un lamento que se hizo igual de famoso: <strong>«¡No es esto, no es esto!»</strong>. Las dos frases muestran su idea del intelectual que interviene en la vida pública sin renunciar a juzgarla.</p>",
  "fuente": "Ortega y Gasset, «El error Berenguer» (El Sol, 1930) y «Un aldabonazo» (Crisol, 1931)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-existencialismo"
  ]
 },
 "sapir": {
  "name": "Edward Sapir",
  "dates": "1884 – 1939",
  "born": 1884,
  "died": 1939,
  "place": "Lauenburg (Pomerania, hoy Polonia)",
  "role": "lingüista y antropólogo estadounidense",
  "idea": "La lengua que hablamos no se limita a expresar el pensamiento, sino que influye en cómo percibimos y organizamos la realidad.",
  "bio": "<p>Edward Sapir nació en Pomerania, en una familia judía que emigró a Estados Unidos cuando él era niño. Estudió en la Universidad de Columbia con el antropólogo Franz Boas y se especializó en las lenguas indígenas de Norteamérica. Fue profesor en Chicago y en Yale, donde tuvo como alumno a Benjamin Lee Whorf.</p>\n<p>Aparece en el temario por la llamada <strong>hipótesis de Sapir-Whorf</strong>, formulada sobre todo por Whorf a partir de sus ideas: la estructura de cada lengua influye en el modo en que sus hablantes piensan y organizan la realidad. En su versión fuerte, el lenguaje determinaría el pensamiento; en la débil, solo lo condicionaría. Esta idea, relacionada con el <strong>giro lingüístico</strong>, inspiró a Txillardegi su defensa de que cada lengua encierra una cosmovisión.</p>",
  "obras": [
   "El lenguaje (1921)"
  ],
  "anecdota": "<p>En el verano de 1915 Sapir trabajó en San Francisco con Ishi, un hombre considerado el último hablante de yahi, una lengua indígena de California. Ishi había vivido oculto con su pequeño grupo durante décadas y, tras quedarse solo, se había instalado en el museo de antropología de la Universidad de California. Durante semanas Sapir anotó con él palabras, relatos y estructuras gramaticales para que la lengua no se perdiera. La escena resume el trabajo de Sapir: documentar lenguas amenazadas convencido de que cada una encierra una manera única de organizar el mundo.</p>",
  "fuente": "Testimonios de Alfred Kroeber y documentación del museo de antropología de la Universidad de California; Theodora Kroeber, Ishi en dos mundos",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-analitica"
  ]
 },
 "wittgenstein": {
  "name": "Ludwig Wittgenstein",
  "dates": "1889 – 1951",
  "born": 1889,
  "died": 1951,
  "place": "Viena (Austria)",
  "role": "filósofo austriaco del lenguaje",
  "idea": "Los problemas filosóficos nacen de malentendidos del lenguaje; primero concibió las proposiciones como figuras de los hechos y después el significado como uso en juegos de lenguaje.",
  "bio": "<p>Ludwig Wittgenstein nació en Viena en una de las familias más ricas de Austria. Estudió ingeniería en Berlín y Mánchester, y su interés por los fundamentos de las matemáticas le llevó a Cambridge para trabajar con Russell. Combatió en el ejército austriaco en la Primera Guerra Mundial, época en la que terminó el <em>Tractatus</em>. Creyendo haber resuelto los problemas de la filosofía, la abandonó: repartió su herencia y fue maestro rural. En 1929 volvió a Cambridge, donde enseñó hasta poco antes de morir.</p>\n<p>En su primera etapa sostuvo la <strong>teoría pictórica del significado</strong>: una proposición tiene sentido si es una figura de un hecho posible, como un mapa lo es del terreno. Las proposiciones científicas tienen sentido; las de la metafísica y la ética intentan decir lo que no se puede decir, y solo se muestra. Por eso la filosofía es una actividad de clarificación. En su segunda etapa, en las <em>Investigaciones filosóficas</em>, criticó esa visión: el lenguaje es una caja de herramientas y el significado de una palabra es su <strong>uso</strong> en diversos <strong>juegos de lenguaje</strong>. La filosofía se vuelve una <strong>terapia</strong> que disuelve los problemas.</p>\n<p>El <em>Tractatus</em> inspiró al Círculo de Viena de Schlick y Carnap, aunque él no compartía su desprecio por lo que no puede decirse. Su segunda filosofía impulsó el análisis del lenguaje ordinario. Es la figura central del giro lingüístico del siglo XX.</p>",
  "obras": [
   "Tractatus logico-philosophicus (1921)",
   "Investigaciones filosóficas (1953)"
  ],
  "anecdota": "<p>En 1929, al volver a Cambridge, Wittgenstein necesitaba un doctorado para poder enseñar, y presentó como tesis el <em>Tractatus</em>, ya famoso. Sus examinadores fueron nada menos que Russell y Moore. El examen fue casi una charla entre viejos amigos, hasta que Wittgenstein, al terminar, les dio una palmada en el hombro y les dijo que no se preocuparan, que sabía que nunca lo entenderían. Moore escribió en su informe que la obra era un trabajo de genio que, en cualquier caso, cumplía los requisitos para el doctorado.</p>",
  "fuente": "Testimonios recogidos en la biografía de Ray Monk, Ludwig Wittgenstein. El deber de un genio",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-analitica"
  ]
 },
 "heidegger": {
  "name": "Martin Heidegger",
  "dates": "1889 – 1976",
  "born": 1889,
  "died": 1976,
  "place": "Messkirch (Alemania)",
  "role": "filósofo alemán",
  "idea": "El ser humano, el Dasein, es un ser arrojado al mundo y abocado a la muerte, que solo vive con autenticidad cuando asume su finitud y su libertad.",
  "bio": "<p>Martin Heidegger nació en Messkirch, en el sur de Alemania, en una familia católica modesta, y estudió teología y filosofía en Friburgo. Fue ayudante de Husserl, el fundador de la fenomenología, y profesor en Marburgo, donde fue alumna suya Hannah Arendt. En 1933 fue nombrado rector de la Universidad de Friburgo y se afilió al partido nazi; dimitió al año siguiente, pero tras la guerra se le prohibió enseñar durante unos años. Su relación con el nazismo sigue siendo objeto de debate.</p>\n<p>En <em>Ser y tiempo</em> (1927) plantea la pregunta por el <strong>ser</strong>, que distingue de los entes, las cosas concretas. Para responderla analiza al ser humano, al que llama <strong>Dasein</strong> («ser-ahí»): un ser-en-el-mundo arrojado a la existencia sin haberlo elegido. El Dasein es <strong>ser-para-la-muerte</strong>. Quien huye de su finitud vive de modo inauténtico, haciendo lo que «se» hace; la existencia auténtica surge al aceptar la muerte y asumir la propia libertad. En su obra posterior dedicó gran atención al arte: la obra de arte es un <strong>acontecer de la verdad</strong>, un modo en que se abre un mundo.</p>\n<p>Es uno de los filósofos más influyentes del siglo XX. Su análisis de la existencia marcó el existencialismo de Sartre, y su pensamiento sobre el arte y la interpretación fue continuado por Gadamer.</p>",
  "obras": [
   "Ser y tiempo (1927)",
   "¿Qué es metafísica? (1929)",
   "El origen de la obra de arte",
   "Carta sobre el humanismo (1947)"
  ],
  "anecdota": "<p>En 1924 Heidegger empezó en Marburgo un curso sobre Aristóteles de una forma que sus alumnos no olvidaron. En lugar de repasar la biografía del filósofo, la despachó en una sola frase: Aristóteles nació, trabajó y murió. Lo que importaba no era la vida anecdótica del autor, sino su pensamiento. Aquellas clases, intensas y exigentes, dieron a Heidegger fama de «rey secreto» de la filosofía entre los estudiantes alemanes, antes incluso de publicar <em>Ser y tiempo</em>. Entre quienes lo escuchaban en Marburgo estaban Hannah Arendt y Hans-Georg Gadamer.</p>",
  "fuente": "Heidegger, curso de Marburgo de 1924 (Conceptos fundamentales de la filosofía aristotélica); testimonios de sus alumnos, como Arendt",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-existencialismo"
  ]
 },
 "carnap": {
  "name": "Rudolf Carnap",
  "dates": "1891 – 1970",
  "born": 1891,
  "died": 1970,
  "place": "Ronsdorf (Alemania)",
  "role": "filósofo y lógico alemán",
  "idea": "Con el análisis lógico del lenguaje se muestra que los enunciados de la metafísica no son falsos, sino carentes de significado, porque no pueden verificarse.",
  "bio": "<p>Rudolf Carnap estudió física, matemáticas y filosofía en Jena, donde asistió a las clases de Frege. En 1926 se incorporó a la Universidad de Viena y fue uno de los miembros más destacados del <strong>Círculo de Viena</strong>. Después enseñó en Praga y, ante el ascenso del nazismo, emigró en 1935 a Estados Unidos, donde fue profesor en Chicago y en Los Ángeles.</p>\n<p>Aparece en el temario como uno de los grandes representantes del <strong>neopositivismo lógico</strong>. Quiso reconstruir el conocimiento científico a partir de la experiencia y de la lógica. En un artículo célebre de 1932 sostuvo que la <strong>metafísica</strong> se supera mediante el análisis lógico del lenguaje: sus enunciados no pueden verificarse, así que no dicen nada, aunque puedan expresar un sentimiento ante la vida.</p>",
  "obras": [
   "La construcción lógica del mundo (1928)",
   "La superación de la metafísica mediante el análisis lógico del lenguaje (1932)",
   "Sintaxis lógica del lenguaje (1934)"
  ],
  "anecdota": "<p>De adolescente, Carnap aprendió por su cuenta el <strong>esperanto</strong>, la lengua artificial creada para facilitar el entendimiento entre pueblos. En 1908 asistió en Dresde a un congreso mundial de esperantistas y le impresionó ver a gente de muchos países conversando con fluidez en un idioma inventado. Según contó en su autobiografía intelectual, aquella experiencia le convenció de que una lengua construida podía funcionar como una natural. Más tarde dedicaría buena parte de su filosofía a diseñar lenguajes lógicos precisos, capaces de expresar el conocimiento científico sin ambigüedades.</p>",
  "fuente": "Carnap, «Autobiografía intelectual», en P. A. Schilpp (ed.), The Philosophy of Rudolf Carnap",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-analitica"
  ]
 },
 "horkheimer": {
  "name": "Max Horkheimer",
  "dates": "1895 – 1973",
  "born": 1895,
  "died": 1973,
  "place": "Stuttgart (Alemania)",
  "role": "filósofo y sociólogo alemán",
  "idea": "La razón ilustrada se ha degradado en razón instrumental, que calcula los medios más eficaces sin preguntarse por los fines, y se ha convertido así en instrumento de dominación.",
  "bio": "<p>Max Horkheimer nació en Stuttgart en 1895, en una familia judía de industriales. Estudió filosofía y psicología y en 1930 pasó a dirigir el Instituto de Investigación Social de Fráncfort, núcleo de la llamada <strong>Escuela de Fráncfort</strong>. Con la llegada de Hitler al poder, el Instituto se trasladó al exilio, primero a Ginebra y después a Nueva York, vinculado a la Universidad de Columbia. En 1949 Horkheimer regresó a Fráncfort, donde reabrió el Instituto y llegó a ser rector de la universidad.</p>\n<p>Horkheimer definió la <strong>teoría crítica</strong>: una filosofía que no se limita a describir la sociedad, sino que busca desenmascarar la dominación y contribuir a la emancipación. Junto con Adorno escribió <em>Dialéctica de la Ilustración</em> (1944), donde sostienen que la razón ilustrada, nacida para liberar al ser humano de los mitos, ha acabado convirtiéndose en instrumento de dominio. Es la <strong>razón instrumental</strong>: una racionalidad que solo calcula los medios más eficaces y renuncia a preguntar si los fines son justos. La técnica, la economía y la <strong>industria cultural</strong> extienden así el control a toda la vida social.</p>\n<p>Su obra amplió la crítica de Marx más allá de la economía, hacia la cultura y la propia idea de razón. Influyó en Habermas, que renovó la teoría crítica desde la acción comunicativa, y en los movimientos estudiantiles de los años sesenta, aunque él se mantuvo distante de ellos. Murió en Núremberg en 1973.</p>",
  "obras": [
   "Teoría tradicional y teoría crítica (1937)",
   "Dialéctica de la Ilustración (1944, con Adorno)",
   "Crítica de la razón instrumental (1947)"
  ],
  "anecdota": "<p>El filósofo marxista Georg Lukács dedicó a Horkheimer, Adorno y sus colegas de Fráncfort una pulla que se hizo célebre. En 1962 escribió que se habían instalado en el «Gran Hotel Abismo»: un hotel lujoso, con todas las comodidades, asomado al borde del precipicio, desde el que contemplaban la catástrofe del mundo entre buenas comidas y obras de arte. Lukács les reprochaba criticar la sociedad sin comprometerse con la acción política. La broma resume un debate de fondo sobre la <strong>teoría crítica</strong>: si basta con pensar críticamente o hace falta transformar la realidad.</p>",
  "fuente": "Lukács, prólogo de 1962 a Teoría de la novela",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-capitalismo"
  ]
 },
 "whorf": {
  "name": "Benjamin Lee Whorf",
  "dates": "1897 – 1941",
  "born": 1897,
  "died": 1941,
  "place": "Winthrop (Massachusetts, EE. UU.)",
  "role": "lingüista estadounidense",
  "idea": "La gramática de cada lengua condiciona la manera en que sus hablantes perciben, clasifican y piensan la realidad: es la tesis del relativismo lingüístico.",
  "bio": "<p>Benjamin Lee Whorf nació en Winthrop (Massachusetts) en 1897. Se formó como ingeniero químico y trabajó toda su vida como inspector de prevención de incendios para una compañía de seguros. Por afición se dedicó a la lingüística: estudió lenguas indígenas americanas, sobre todo el hopi, y desde 1931 asistió en la Universidad de Yale a los cursos del antropólogo y lingüista Edward Sapir. Murió en 1941.</p>\n<p>Whorf defendió el <strong>relativismo lingüístico</strong>: la estructura gramatical de cada lengua condiciona el modo en que sus hablantes perciben y organizan la realidad, por ejemplo el tiempo o la causalidad. Esta tesis, conocida como <strong>hipótesis de Sapir-Whorf</strong>, sigue siendo objeto de debate y hoy suele aceptarse solo en una versión moderada. Aparece en el temario porque inspiró la idea de Txillardegi de que cada lengua encierra una cosmovisión propia.</p>",
  "obras": [
   "Lenguaje, pensamiento y realidad (1956)"
  ],
  "anecdota": "<p>Su trabajo como inspector de incendios dio a Whorf uno de sus ejemplos favoritos. En un almacén observó que los trabajadores tenían cuidado cerca de los bidones de gasolina llenos, pero fumaban tranquilamente junto a los que llevaban la etiqueta «vacíos». Sin embargo, esos bidones estaban llenos de vapores inflamables, mucho más peligrosos. La palabra «vacío», que sugiere ausencia de riesgo, guiaba la conducta más que la realidad física. Whorf usó casos así para defender que el lenguaje condiciona cómo percibimos las situaciones y cómo actuamos en ellas.</p>",
  "fuente": "Whorf, «La relación del pensamiento y el comportamiento habituales con el lenguaje» (1939)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-analitica"
  ]
 },
 "ryle": {
  "name": "Gilbert Ryle",
  "dates": "1900 – 1976",
  "born": 1900,
  "died": 1976,
  "place": "Brighton (Inglaterra)",
  "role": "filósofo analítico británico",
  "idea": "El dualismo cartesiano es un error categorial: la mente no es un fantasma dentro de la máquina del cuerpo, sino un conjunto de capacidades y disposiciones para actuar.",
  "bio": "<p>Gilbert Ryle nació en Brighton (Inglaterra) en 1900. Estudió y enseñó en la Universidad de Oxford, donde fue catedrático de filosofía y, durante más de dos décadas, director de la revista <em>Mind</em>, una de las más influyentes de la filosofía anglosajona. Fue una de las figuras centrales de la llamada filosofía del lenguaje ordinario, que analiza cómo usamos realmente las palabras. Murió en 1976.</p>\n<p>En <em>El concepto de lo mental</em> (1949) criticó el dualismo de Descartes, que concibe la mente como una sustancia distinta del cuerpo, y lo llamó irónicamente el <strong>fantasma en la máquina</strong>. Según Ryle, ese dualismo comete un <strong>error categorial</strong>: trata la mente como si fuera una cosa más, cuando hablar de lo mental es hablar de capacidades y disposiciones para actuar. Aparece en el temario como crítico clásico del dualismo cartesiano.</p>",
  "obras": [
   "El concepto de lo mental (1949)",
   "Dilemas (1954)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-descartes-makro"
  ]
 },
 "popper": {
  "name": "Karl Popper",
  "dates": "1902 – 1994",
  "born": 1902,
  "died": 1994,
  "place": "Viena",
  "role": "filósofo de la ciencia",
  "idea": "Una teoría es científica si es falsable, es decir, si puede ser refutada por la experiencia; la ciencia, como la política, avanza corrigiendo sus errores.",
  "bio": "<p>Karl Popper nació en Viena en 1902, en una familia de origen judío. Estudió matemáticas, física y psicología, y se relacionó con el Círculo de Viena, aunque discrepó de sus tesis. Ante la amenaza nazi emigró en 1937 a Nueva Zelanda, donde enseñó filosofía y escribió su gran obra política. Desde 1946 fue profesor en la London School of Economics. Murió en Londres en 1994.</p>\n<p>En filosofía de la ciencia, Popper sostuvo que ninguna cantidad de observaciones puede demostrar definitivamente una teoría general, pero basta un solo caso contrario para refutarla. Por eso propuso la <strong>falsabilidad</strong> como criterio para distinguir la ciencia: una teoría es científica si puede concebirse un experimento capaz de desmentirla. La ciencia avanza mediante <strong>conjeturas y refutaciones</strong>, por ensayo y error. En política defendió la <strong>sociedad abierta</strong> y criticó el <strong>historicismo</strong>, la pretensión de conocer leyes de la historia que permitirían predecir el futuro, que atribuyó a Platón, Hegel y Marx. Frente a las utopías totalitarias propuso la <strong>ingeniería social fragmentaria</strong>: reformas graduales que se corrigen si fallan.</p>\n<p>Popper calificó de pseudocientíficos el psicoanálisis, porque podía explicar cualquier conducta, y el marxismo, cuyos defensores reinterpretaban los hechos para evitar su refutación. Su imagen de la ciencia fue matizada por Thomas Kuhn con la noción de paradigma. Su polémica con la Escuela de Fráncfort sobre el método de las ciencias sociales enfrentó su racionalismo crítico con la teoría crítica de Adorno y Habermas.</p>",
  "obras": [
   "La lógica de la investigación científica (1934)",
   "La sociedad abierta y sus enemigos (1945)",
   "La miseria del historicismo (1957)",
   "Conjeturas y refutaciones (1963)"
  ],
  "anecdota": "<p>El 25 de octubre de 1946 Popper dio una charla en el Club de Ciencias Morales de Cambridge, presidido por Wittgenstein. Popper defendió que existen problemas filosóficos genuinos; Wittgenstein sostenía que solo hay enigmas del lenguaje. La discusión se acaloró y, según varios testigos, Wittgenstein agitó un atizador de chimenea mientras hablaba. Popper contó que, cuando le pidieron un ejemplo de regla moral, respondió: «No amenazar con atizadores a los conferenciantes invitados». Los asistentes no se ponen de acuerdo sobre lo que ocurrió exactamente, y algunos niegan esa versión. El enfrentamiento duró apenas diez minutos.</p>",
  "fuente": "Popper, Búsqueda sin término (autobiografía); testimonios de los asistentes a la sesión",
  "tradicion": true,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-capitalismo"
  ]
 },
 "adorno": {
  "name": "Theodor W. Adorno",
  "dates": "1903 – 1969",
  "born": 1903,
  "died": 1969,
  "place": "Fráncfort del Meno (Alemania)",
  "role": "filósofo, sociólogo y musicólogo",
  "idea": "La industria cultural convierte la cultura en mercancía que adormece a las masas; solo el arte autónomo y difícil conserva su capacidad crítica frente a la sociedad.",
  "bio": "<p>Theodor W. Adorno nació en Fráncfort del Meno en 1903, en una familia de origen judío. Se formó en filosofía, sociología y música, y estudió composición en Viena con Alban Berg. Colaboró con el Instituto de Investigación Social y, con la llegada del nazismo, se exilió en Oxford y después en Estados Unidos, donde trabajó con Horkheimer. En 1949 regresó a Fráncfort y llegó a dirigir el Instituto. Murió en Visp (Suiza) en 1969.</p>\n<p>Con Horkheimer escribió <em>Dialéctica de la Ilustración</em> (1944), donde muestran que la razón ilustrada se ha convertido en <strong>razón instrumental</strong>, un medio de dominio sobre la naturaleza y sobre los propios seres humanos. Allí acuñaron el concepto de <strong>industria cultural</strong>: la producción en serie de entretenimiento estandarizado (cine, radio, música ligera), que fomenta la pasividad y el conformismo. Frente a ella, Adorno ve en el arte auténtico, difícil y autónomo, una forma de <strong>conocimiento crítico</strong> y la última forma de resistencia, porque se niega a reconciliarse con una sociedad injusta.</p>\n<p>Adorno es, junto con Horkheimer, el principal representante de la primera generación de la <strong>Escuela de Fráncfort</strong>. Discutió con Walter Benjamin su confianza en el cine y en el arte de masas, polemizó con Popper sobre el método de las ciencias sociales y fue maestro de Habermas, que trabajó como su asistente. Su influencia llega a la estética, la sociología de la cultura y la crítica de los medios de comunicación.</p>",
  "obras": [
   "Dialéctica de la Ilustración (1944, con Horkheimer)",
   "Minima moralia (1951)",
   "Dialéctica negativa (1966)",
   "Teoría estética (1970)"
  ],
  "anecdota": "<p>Durante su exilio en California, Adorno se hizo amigo de Thomas Mann, que vivía cerca y estaba escribiendo <em>Doktor Faustus</em>, la novela de un compositor que pacta con el diablo. Mann sabía que necesitaba a alguien que entendiera de verdad la música moderna, y Adorno, discípulo de Alban Berg, se convirtió en su consejero: le explicaba la música dodecafónica e incluso le describía obras imaginarias del protagonista. El propio Mann reconoció esa colaboración en un libro sobre la génesis de la novela. Para Adorno, la música seria era una forma de conocimiento de su época.</p>",
  "fuente": "Thomas Mann, Los orígenes del Doktor Faustus (1949)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-capitalismo"
  ]
 },
 "zambrano": {
  "name": "María Zambrano",
  "dates": "1904 – 1991",
  "born": 1904,
  "died": 1991,
  "place": "Vélez-Málaga (Málaga)",
  "role": "filósofa y ensayista española",
  "idea": "Frente a una razón fría que ignora sentimientos y sueños, propone una razón poética que una filosofía y poesía para comprender en profundidad la vida humana.",
  "bio": "<p>María Zambrano nació en Vélez-Málaga en 1904. Estudió filosofía en Madrid con José Ortega y Gasset y Xavier Zubiri, y participó en la vida intelectual y política de la Segunda República. Al terminar la Guerra Civil, en 1939, marchó al exilio, que duró 45 años: vivió en México, Cuba, Puerto Rico, Italia, Francia y Suiza. Regresó a Madrid en 1984 y murió allí en 1991.</p>\n<p>Zambrano parte de la <strong>razón vital</strong> de su maestro Ortega, pero va más allá con su propuesta de la <strong>razón poética</strong>. A su juicio, la razón occidental, desde los griegos, se ha separado de la poesía y ha ignorado las entrañas del ser humano: los sueños, los sentimientos, la esperanza y lo sagrado. La filosofía busca dominar la realidad mediante conceptos; la poesía, en cambio, la acoge tal como se da. La razón poética quiere unir ambas para comprender la realidad profunda de la vida humana, sin renunciar al rigor.</p>\n<p>Su obra, escrita en gran parte en el exilio, fue poco conocida en España hasta los años ochenta. Recibió el Premio Príncipe de Asturias en 1981 y en 1988 fue la primera mujer galardonada con el Premio Cervantes. Hoy se la considera una de las voces más originales de la filosofía española del siglo XX, junto a Unamuno y Ortega. Su estilo, entre el ensayo y la prosa poética, se aprecia en obras como <em>Claros del bosque</em>.</p>",
  "obras": [
   "Filosofía y poesía (1939)",
   "El hombre y lo divino (1955)",
   "Persona y democracia (1958)",
   "Claros del bosque (1977)"
  ],
  "anecdota": "<p>María Zambrano vivió buena parte de su exilio junto a su hermana Araceli, y ambas acogían a numerosos gatos, a los que ella tenía un cariño enorme. En Roma, a mediados de los años sesenta, una denuncia de vecinos por los gatos acabó con una orden para que abandonaran el país. Las hermanas se trasladaron entonces a una casa aislada en La Pièce, en el Jura francés, cerca de Suiza, donde vivieron años de gran soledad y escritura. Allí maduraron algunas de sus obras más personales, como <em>Claros del bosque</em>.</p>",
  "fuente": "Testimonios biográficos y correspondencia de María Zambrano",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-existencialismo"
  ]
 },
 "sartre": {
  "name": "Jean-Paul Sartre",
  "dates": "1905 – 1980",
  "born": 1905,
  "died": 1980,
  "place": "París",
  "role": "filósofo y escritor francés",
  "idea": "No hay una naturaleza humana previa: la existencia precede a la esencia, y cada persona, condenada a ser libre, es plenamente responsable de lo que hace de sí misma.",
  "bio": "<p>Jean-Paul Sartre nació en París en 1905. Estudió en la Escuela Normal Superior, donde conoció a Simone de Beauvoir, su compañera durante toda la vida. Fue profesor de filosofía en institutos y en 1940 fue hecho prisionero por los alemanes. Tras la guerra fundó la revista <em>Les Temps modernes</em> y se convirtió en el intelectual más influyente de Francia, comprometido con causas de izquierda. En 1964 rechazó el Premio Nobel de Literatura. Murió en París en 1980.</p>\n<p>Sartre defiende un <strong>existencialismo ateo</strong>: si no hay un Dios que haya diseñado al ser humano, <strong>la existencia precede a la esencia</strong>. Primero existimos y después nos hacemos con nuestras decisiones; el ser humano es un proyecto. Por eso afirma que estamos condenados a ser libres: no elegimos ser libres, y hasta no elegir es una elección. Esta libertad radical implica una responsabilidad total y produce <strong>angustia</strong>, de la que huimos mediante la <strong>mala fe</strong>, el autoengaño de quien dice que no tenía opción. La vida auténtica exige asumir la propia libertad.</p>\n<p>Sartre recibió la influencia de Husserl y Heidegger, y más tarde intentó conciliar el existencialismo con el marxismo. Su pensamiento está estrechamente ligado al de Beauvoir, que aplicó estas ideas a la condición de la mujer, y al de Albert Camus, con quien rompió en 1952 por diferencias políticas. Su obra marcó la cultura europea de posguerra y difundió la figura del intelectual comprometido.</p>",
  "obras": [
   "La náusea (1938)",
   "El ser y la nada (1943)",
   "El existencialismo es un humanismo (1946)",
   "Crítica de la razón dialéctica (1960)"
  ],
  "anecdota": "<p>Simone de Beauvoir relata una escena de 1933 en un bar de Montparnasse. Raymond Aron, que volvía de estudiar en Berlín, señaló su vaso de cóctel de albaricoque y le dijo a Sartre que, si era fenomenólogo, podía hablar de aquel cóctel y eso sería filosofía. Sartre, según Beauvoir, casi palideció de emoción: era justo lo que buscaba, una filosofía que partiera de las cosas concretas tal como las vivimos. Poco después se fue a Berlín a estudiar a Husserl, y de ahí nació su existencialismo.</p>",
  "fuente": "Simone de Beauvoir, La plenitud de la vida (La force de l'âge)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-existencialismo",
   "hf-beauvoir"
  ]
 },
 "arendt": {
  "name": "Hannah Arendt",
  "dates": "1906 – 1975",
  "born": 1906,
  "died": 1975,
  "place": "Linden (Hannover, Alemania)",
  "role": "filósofa y teórica política",
  "idea": "El totalitarismo destruye la pluralidad y el espacio público; incluso el mal extremo puede surgir de personas corrientes que renuncian a pensar por sí mismas.",
  "bio": "<p>Hannah Arendt nació en 1906 en Linden, hoy parte de Hannover, en una familia judía, y creció en Königsberg. Estudió filosofía con Martin Heidegger y Karl Jaspers, bajo cuya dirección se doctoró con una tesis sobre el amor en Agustín de Hipona. En 1933, tras ser detenida brevemente por la Gestapo, huyó a Francia, y en 1941 llegó a Estados Unidos, donde se nacionalizó y enseñó en varias universidades. Murió en Nueva York en 1975.</p>\n<p>En <em>Los orígenes del totalitarismo</em> (1951) analizó el nazismo y el estalinismo como una forma nueva de poder: el <strong>totalitarismo</strong>, que busca el dominio total mediante el terror y la propaganda, destruye la vida pública y convierte a la población en una masa aislada. Frente a ello defendió la <strong>pluralidad</strong> y el <strong>espacio público</strong>, donde las personas actúan y deliberan juntas. Al cubrir el juicio al criminal nazi Adolf Eichmann acuñó la expresión <strong>banalidad del mal</strong>: grandes crímenes pueden ser cometidos por personas corrientes que dejan de pensar. También reflexionó sobre la verdad y la política: la mentira puede destruir una verdad de hecho, pero no ocupar su lugar.</p>\n<p>Arendt prefirió definirse como teórica política antes que como filósofa. Su distinción entre labor, trabajo y acción, expuesta en <em>La condición humana</em> (1958), sigue siendo una referencia. Hoy es una de las pensadoras más leídas para reflexionar sobre la democracia, la condición de los apátridas y la relación entre verdad y poder.</p>",
  "obras": [
   "Los orígenes del totalitarismo (1951)",
   "La condición humana (1958)",
   "Eichmann en Jerusalén (1963)",
   "Verdad y política (1967)"
  ],
  "anecdota": "<p>En 1964 Arendt concedió una larga entrevista a la televisión alemana, con el periodista Günter Gaus. Fumando con calma, habló del exilio, del nazismo y de su oficio. Cuando Gaus le preguntó qué le quedaba de la Europa anterior a Hitler, respondió que le quedaba la <strong>lengua materna</strong>: el alemán, en el que se sabía de memoria muchos poemas y que nunca había perdido pese a vivir en Estados Unidos. La lengua, dijo, no se había vuelto loca. La entrevista se sigue viendo hoy como un retrato de su independencia de pensamiento.</p>",
  "fuente": "Entrevista con Günter Gaus, «Zur Person» (ZDF, 1964)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-capitalismo"
  ]
 },
 "beauvoir": {
  "name": "Simone de Beauvoir",
  "dates": "1908 – 1986",
  "born": 1908,
  "died": 1986,
  "place": "París",
  "role": "filósofa y escritora francesa",
  "idea": "La identidad femenina es una construcción cultural y no un destino biológico: la mujer ha sido definida como lo otro del hombre y debe conquistar su libertad.",
  "bio": "<p>Simone de Beauvoir nació en París en 1908. Estudió filosofía en la Sorbona y en 1929 obtuvo la agregación, la oposición al profesorado, en segundo lugar, detrás de Sartre, con quien mantuvo una relación intelectual y personal durante toda su vida. Fue profesora de instituto hasta 1943 y después se dedicó a escribir novelas, ensayos y memorias. Murió en París en 1986.</p>\n<p>Beauvoir parte del existencialismo: la existencia precede a la esencia. Pero observa que a las mujeres se les ha negado esa libertad de hacerse a sí mismas. En <em>El segundo sexo</em> (1949) sostiene que no se nace mujer, sino que se llega a serlo: la feminidad es una construcción cultural, no un destino biológico. El hombre se ha definido como el Sujeto y ha convertido a la mujer en <strong>lo otro</strong>, siempre definida en relación con él. Relegada a la <strong>inmanencia</strong> de las tareas repetitivas, se le ha negado la <strong>trascendencia</strong>, la capacidad de proyectarse y crear. La salida no es invertir la dominación, sino alcanzar la <strong>reciprocidad</strong> entre dos libertades.</p>\n<p><em>El segundo sexo</em> fue una obra fundamental para la <strong>segunda ola del feminismo</strong>, en los años sesenta y setenta, que amplió la lucha por los derechos legales a una crítica de toda la cultura. Beauvoir apoyó activamente el movimiento feminista y la despenalización del aborto en Francia. Su influencia llega a pensadoras posteriores como Judith Butler, que radicalizó la distinción entre sexo y género.</p>",
  "obras": [
   "Para una moral de la ambigüedad (1947)",
   "El segundo sexo (1949)",
   "Los mandarines (1954)",
   "La vejez (1970)"
  ],
  "anecdota": "<p>En 1929, junto a la fuente Médicis de los jardines de Luxemburgo, en París, Beauvoir le expuso a Sartre la teoría moral que había ido construyendo. Él la desmontó durante horas, pieza a pieza. Beauvoir, acostumbrada a ser la mejor de su clase, reconoció que ya no estaba segura de lo que pensaba, ni siquiera de si pensaba. Lejos de desanimarse, lo vivió como un estímulo: había encontrado a alguien con quien discutir de igual a igual. Así lo cuenta en sus memorias, y así empezó un diálogo filosófico que duró cincuenta años.</p>",
  "fuente": "Simone de Beauvoir, Memorias de una joven formal (1958)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-beauvoir"
  ]
 },
 "turing": {
  "name": "Alan Turing",
  "dates": "1912 – 1954",
  "born": 1912,
  "died": 1954,
  "place": "Londres",
  "role": "matemático y pionero informático",
  "idea": "Si una máquina puede conversar de modo que no la distingamos de un ser humano, tenemos motivos para decir que piensa: es la idea del test de Turing.",
  "bio": "<p>Alan Turing nació en Londres en 1912. Estudió matemáticas en Cambridge y en 1936 describió una máquina teórica capaz de realizar cualquier cálculo siguiendo reglas, la <strong>máquina de Turing</strong>, base conceptual de la informática. Durante la Segunda Guerra Mundial trabajó en Bletchley Park descifrando los mensajes alemanes cifrados con la máquina Enigma. En 1952 fue condenado por su homosexualidad, entonces delito en el Reino Unido, y en 1954 murió envenenado con cianuro; la investigación oficial concluyó que fue un suicidio.</p>\n<p>En el artículo <em>Maquinaria computacional e inteligencia</em> (1950) se preguntó si las máquinas pueden pensar y propuso sustituir esa pregunta por una prueba: el juego de imitación, hoy llamado <strong>test de Turing</strong>. Aparece en el temario porque reabre el debate con Descartes, que había negado que una máquina pudiera usar el lenguaje como nosotros.</p>",
  "obras": [
   "Sobre los números computables (1936)",
   "Maquinaria computacional e inteligencia (1950)"
  ],
  "anecdota": "<p>En mayo de 1926, cuando Turing iba a empezar su primer curso en el internado de Sherborne, una huelga general paralizó los trenes en Gran Bretaña. En lugar de esperar, el muchacho de trece años cogió su bicicleta y recorrió unos cien kilómetros desde Southampton, parando a dormir en una posada por el camino. La hazaña llamó tanto la atención que la recogió la prensa local. Anticipa rasgos que lo acompañaron siempre: independencia, tenacidad y afición por las largas distancias, pues de adulto fue un corredor de fondo de nivel casi olímpico.</p>",
  "fuente": "Andrew Hodges, Alan Turing: The Enigma; prensa local de 1926",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-descartes-makro"
  ]
 },
 "ricoeur": {
  "name": "Paul Ricoeur",
  "dates": "1913 – 2005",
  "born": 1913,
  "died": 2005,
  "place": "Valence (Francia)",
  "role": "filósofo francés, hermeneuta",
  "idea": "Comprender al ser humano exige interpretar sus símbolos, textos y relatos, porque solo nos conocemos a nosotros mismos a través del rodeo de la interpretación.",
  "bio": "<p>Paul Ricoeur nació en Valence y quedó huérfano muy pronto. Durante la Segunda Guerra Mundial pasó cinco años como prisionero en Alemania. Después fue profesor en Estrasburgo, en la Sorbona, en Nanterre y en la Universidad de Chicago. Es una de las grandes figuras de la <strong>hermenéutica</strong>, la filosofía de la interpretación.</p>\n<p>En su libro sobre Freud (1965) llamó a Marx, Nietzsche y Freud los <strong>maestros de la sospecha</strong>: los tres enseñan a desconfiar de lo que la conciencia cree saber de sí misma y a buscar lo que se oculta detrás. Por eso el temario usa su expresión para presentar a estos autores. Ricoeur, sin embargo, no se quedó en la sospecha: propuso completarla con una escucha atenta del sentido de los símbolos y los relatos.</p>",
  "obras": [
   "Freud: una interpretación de la cultura (1965)",
   "La metáfora viva (1975)",
   "Tiempo y narración (1983-1985)",
   "Sí mismo como otro (1990)"
  ],
  "anecdota": "<p>Durante sus cinco años como prisionero de guerra en Alemania, Ricoeur no dejó de hacer filosofía. En el campo, junto con otros oficiales como Mikel Dufrenne, organizó cursos y lecturas para sus compañeros, una especie de universidad improvisada. Allí mismo tradujo al francés las <em>Ideas</em> de Husserl, escribiendo a lápiz en los márgenes del único ejemplar que tenía porque le faltaba papel. Esa traducción, publicada después de la guerra, lo dio a conocer como uno de los grandes especialistas franceses en fenomenología.</p>",
  "fuente": "Ricoeur, Autobiografía intelectual (Réflexion faite)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-sospecha"
  ]
 },
 "rawls": {
  "name": "John Rawls",
  "dates": "1921 – 2002",
  "born": 1921,
  "died": 2002,
  "place": "Baltimore (Estados Unidos)",
  "role": "filósofo político estadounidense",
  "idea": "Una sociedad justa es la que elegiríamos tras un velo de ignorancia: con iguales libertades básicas y desigualdades admitidas solo si benefician a los peor situados.",
  "bio": "<p>John Rawls nació en Baltimore y estudió en la Universidad de Princeton. Combatió como soldado en el Pacífico durante la Segunda Guerra Mundial, experiencia que lo marcó profundamente. Fue profesor en la Universidad de Harvard durante casi cuarenta años. Con <em>Teoría de la justicia</em> (1971) devolvió a la filosofía política un lugar central en el pensamiento del siglo XX.</p>\n<p>Rawls renueva la tradición del <strong>contrato social</strong>. Propone un experimento mental, la <strong>posición original</strong>: elegir las reglas de la sociedad tras un <strong>velo de ignorancia</strong>, sin saber si seremos ricos o pobres, hombres o mujeres, sanos o enfermos. En esa situación, cualquier persona razonable seguiría la estrategia <em>maximin</em>, es decir, asegurar que la peor posición posible sea la mejor. De ahí salen dos principios: iguales libertades básicas para todos e igualdad de oportunidades, con el <strong>principio de diferencia</strong>: las desigualdades solo son justas si benefician a los que menos tienen. Así llama a su teoría <em>justicia como equidad</em>, que da una base moral al Estado de bienestar.</p>\n<p>Su obra se inspira en Locke, Rousseau y, sobre todo, Kant. Provocó un intenso debate: Robert Nozick la criticó desde el liberalismo, y los comunitaristas le reprocharon un individualismo abstracto. Mantuvo un conocido diálogo con Habermas, y Martha Nussbaum desarrolló su enfoque de las capacidades en discusión con él. En <em>Liberalismo político</em> (1993) se preguntó cómo pueden convivir personas con visiones del mundo muy distintas.</p>",
  "obras": [
   "Teoría de la justicia (1971)",
   "Liberalismo político (1993)",
   "El derecho de gentes (1999)",
   "La justicia como equidad: una reformulación (2001)"
  ],
  "anecdota": "<p>Al terminar la guerra, Rawls formó parte de las tropas de ocupación en Japón y pasó en tren junto a las ruinas de Hiroshima, pocas semanas después de la bomba atómica. Aquella visión no lo abandonó. Cincuenta años más tarde, en 1995, publicó el artículo «Cincuenta años después de Hiroshima», donde sostuvo que el bombardeo de ciudades llenas de civiles había sido un grave error moral, aunque lo ordenara un gobierno democrático. Para Rawls, también en la guerra hay límites que la justicia no permite cruzar.</p>",
  "fuente": "Thomas Pogge, John Rawls: su vida y su teoría de la justicia; J. Rawls, «Fifty Years after Hiroshima», Dissent (1995)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-historicidad",
   "hf-capitalismo"
  ]
 },
 "lyotard": {
  "name": "Jean-François Lyotard",
  "dates": "1924 – 1998",
  "born": 1924,
  "died": 1998,
  "place": "Versalles (Francia)",
  "role": "filósofo francés de la posmodernidad",
  "idea": "La posmodernidad es la incredulidad hacia los metarrelatos: ya no creemos en grandes narraciones que den sentido a toda la historia, sino en pequeños relatos plurales.",
  "bio": "<p>Jean-François Lyotard nació en Versalles. Fue profesor de filosofía en un instituto de Constantina, en la Argelia colonial, y durante años militó en el grupo marxista crítico <em>Socialismo o Barbarie</em>. Más tarde se alejó del marxismo. Enseñó en la Universidad de París VIII (Vincennes) y participó en la fundación del Colegio Internacional de Filosofía.</p>\n<p>Su obra más conocida, <em>La condición posmoderna</em> (1979), nació como un informe sobre el saber en las sociedades más desarrolladas. Allí define la posmodernidad como la <strong>incredulidad hacia los metarrelatos</strong>. Un metarrelato es una gran narración que pretende explicar toda la realidad y dar una meta a la historia: la emancipación del ser humano en la Ilustración y el marxismo, o el progreso indefinido gracias a la ciencia. Tras las guerras mundiales y los totalitarismos, esos relatos han perdido credibilidad. En su lugar quedan <strong>pequeños relatos</strong> locales y diversos, sin una verdad única que los ordene. Lyotard observa además que el saber se legitima cada vez más por su eficacia técnica y económica, lo que llama <strong>performatividad</strong>.</p>\n<p>Heredero de la sospecha de Nietzsche, Lyotard se convirtió en la referencia del debate sobre la posmodernidad. Su principal oponente fue Habermas, que defendía la modernidad como un proyecto inacabado que había que corregir, no abandonar.</p>",
  "obras": [
   "Discurso, figura (1971)",
   "Economía libidinal (1974)",
   "La condición posmoderna (1979)",
   "La diferencia (1983)"
  ],
  "anecdota": "<p>En 1985 Lyotard hizo algo poco habitual para un filósofo: fue comisario de una gran exposición en el Centro Pompidou de París, titulada <em>Les Immatériaux</em> («Los inmateriales»). En lugar de cuadros colgados en orden, el público recorría un laberinto de espacios en penumbra, con auriculares que emitían textos distintos según la zona, y se encontraba con pantallas, objetos sintéticos y nuevas tecnologías de la información. No había un recorrido único ni una conclusión: la exposición quería que el visitante viviera en su propio cuerpo la condición posmoderna, sin un gran relato que lo guiara.</p>",
  "fuente": "Exposición Les Immatériaux (Centro Pompidou, 1985) y su catálogo",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-posmodernidad"
  ]
 },
 "bauman": {
  "name": "Zygmunt Bauman",
  "dates": "1925 – 2017",
  "born": 1925,
  "died": 2017,
  "place": "Poznań (Polonia)",
  "role": "sociólogo y filósofo polaco",
  "idea": "Vivimos en una modernidad líquida, donde los vínculos, las instituciones y las identidades ya no son sólidos, sino precarios y cambiantes, y el ciudadano se vuelve consumidor.",
  "bio": "<p>Zygmunt Bauman nació en Poznań en una familia judía. En 1939 huyó con ella a la Unión Soviética ante la invasión nazi y combatió en el ejército polaco formado allí. Tras la guerra fue profesor de sociología en la Universidad de Varsovia, pero en 1968 fue expulsado en una campaña antisemita del régimen comunista. Tras pasar por Israel, enseñó desde 1971 en la Universidad de Leeds (Reino Unido). En 2010 recibió el Premio Príncipe de Asturias de Comunicación y Humanidades.</p>\n<p>Bauman distingue una modernidad sólida, con instituciones estables, trabajos para toda la vida y proyectos duraderos, de la actual <strong>modernidad líquida</strong>. Como los líquidos, que no conservan su forma, hoy los empleos, las parejas, las comunidades y las identidades son frágiles y cambiantes. La sociedad ya no nos trata sobre todo como productores o ciudadanos, sino como <strong>consumidores</strong>, y la libertad se confunde con elegir entre productos. Quien no puede consumir ni resulta útil al mercado queda como <strong>residuo humano</strong>: personas sobrantes, como los parados de larga duración o los refugiados.</p>\n<p>Antes había analizado en <em>Modernidad y Holocausto</em> (1989) cómo la burocracia y la racionalidad técnica hicieron posible el genocidio, en la línea de la crítica a la razón instrumental de Adorno y Horkheimer y de las reflexiones de Hannah Arendt. Su diagnóstico de la sociedad de consumo dialoga con autores como Byung-Chul Han.</p>",
  "obras": [
   "Modernidad y Holocausto (1989)",
   "Modernidad líquida (2000)",
   "Amor líquido (2003)",
   "Vidas desperdiciadas (2004)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-siglo21"
  ]
 },
 "foucault": {
  "name": "Michel Foucault",
  "dates": "1926 – 1984",
  "born": 1926,
  "died": 1984,
  "place": "Poitiers (Francia)",
  "role": "filósofo e historiador francés",
  "idea": "El sujeto no es algo natural, sino una construcción de discursos y relaciones de poder; todo saber produce poder, y el poder produce formas de saber.",
  "bio": "<p>Michel Foucault nació en Poitiers, hijo de un cirujano. Estudió en la Escuela Normal Superior de París y se interesó pronto por la psicología y la psiquiatría. Trabajó en Suecia, Polonia, Alemania y Túnez antes de obtener en 1970 una cátedra en el Collège de France, que tituló Historia de los sistemas de pensamiento. Participó en movimientos a favor de los presos. Murió en París en 1984 a causa del sida.</p>\n<p>Foucault estudió la historia de instituciones como el manicomio, el hospital o la cárcel para mostrar que conceptos que creemos naturales, como locura, enfermedad o delincuencia, se construyen históricamente. Inspirado en Nietzsche, practicó una <strong>genealogía</strong> que une <strong>saber y poder</strong>: la medicina, la psiquiatría o el derecho no solo describen a las personas, sino que las clasifican, las vigilan y las normalizan. En <em>Vigilar y castigar</em> (1975) describe la <strong>sociedad disciplinaria</strong>, simbolizada por el panóptico, una cárcel en la que el preso se sabe siempre observado. Al final de <em>Las palabras y las cosas</em> (1966) anuncia la muerte del hombre, que el temario presenta como <strong>muerte del sujeto</strong>: el yo es un producto de discursos y redes de poder.</p>\n<p>Es uno de los rupturistas que se oponen a Habermas, pues desconfía de toda razón que se presente como universal. Su influencia es enorme: en Judith Butler, Paul B. Preciado o en la idea de Byung-Chul Han de que hemos pasado de la sociedad disciplinaria a la del rendimiento.</p>",
  "obras": [
   "Historia de la locura en la época clásica (1961)",
   "Las palabras y las cosas (1966)",
   "Vigilar y castigar (1975)",
   "Historia de la sexualidad (1976-1984)"
  ],
  "anecdota": "<p>En noviembre de 1971 la televisión holandesa sentó frente a frente a Foucault y a Noam Chomsky para debatir sobre la naturaleza humana. Chomsky defendió que existe una naturaleza humana, con capacidades innatas como el lenguaje, y que en ella puede apoyarse una idea de justicia. Foucault desconfió: para él, conceptos como naturaleza humana o justicia se han formado dentro de nuestra sociedad y sirven también para ejercer el poder. Los dos se trataron con cortesía, pero apenas se pusieron de acuerdo. El debate se convirtió en un clásico.</p>",
  "fuente": "Debate televisado Chomsky-Foucault (Eindhoven, 1971), publicado como La naturaleza humana: justicia versus poder",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-posmodernidad"
  ]
 },
 "chomsky": {
  "name": "Noam Chomsky",
  "dates": "n. 1928",
  "born": 1928,
  "died": null,
  "place": "Filadelfia (Estados Unidos)",
  "role": "lingüista y activista político",
  "idea": "Los grandes medios de comunicación, sin necesidad de censura directa, fabrican el consentimiento: seleccionan de qué se habla y moldean la opinión pública al servicio de las élites.",
  "bio": "<p>Noam Chomsky nació en Filadelfia en una familia judía de origen ucraniano y bielorruso. Estudió en la Universidad de Pensilvania y desde 1955 fue profesor en el Instituto Tecnológico de Massachusetts (MIT). Su carrera tiene dos caras: es uno de los lingüistas más influyentes de la historia y, desde la guerra de Vietnam, un destacado crítico de la política exterior de Estados Unidos.</p>\n<p>En lingüística creó la <strong>gramática generativa</strong>. Sostiene que todos los seres humanos nacen con una capacidad innata para el lenguaje, una <strong>gramática universal</strong> común a todas las lenguas. Así explica que un niño aprenda a hablar tan rápido a partir de pocos ejemplos, frente al conductismo, que lo reducía todo a estímulos y respuestas. En el temario aparece sobre todo por su crítica de los medios. En <em>Los guardianes de la libertad</em> (1988), escrito con Edward S. Herman, analiza la <strong>fabricación del consentimiento</strong> mediante un <strong>modelo de propaganda</strong>. Los medios dependen de grandes empresas, de la publicidad y de fuentes oficiales, y esos filtros deciden qué noticias existen y cómo se cuentan. No prohíben pensar: marcan los límites de lo que se puede pensar.</p>\n<p>Su trabajo cambió la lingüística e influyó en la psicología cognitiva y la filosofía de la mente. Su crítica de los medios conecta con la Escuela de Fráncfort y con la preocupación de Habermas por una esfera pública libre de manipulación.</p>",
  "obras": [
   "Estructuras sintácticas (1957)",
   "Aspectos de la teoría de la sintaxis (1965)",
   "Los guardianes de la libertad (1988, con E. S. Herman)"
  ],
  "anecdota": "<p>Con solo diez años, Chomsky escribió su primer artículo en el periódico de su escuela de Filadelfia. El tema no era infantil: la caída de Barcelona en manos de las tropas de Franco, a comienzos de 1939, y la expansión del fascismo por Europa. El propio Chomsky ha recordado muchas veces este texto. La guerra civil española lo marcó desde niño: más tarde se acercó a las ideas anarquistas y admiró las colectividades de Cataluña y Aragón, que consideró uno de los grandes experimentos de autogestión del siglo XX.</p>",
  "fuente": "Testimonio del propio Chomsky en entrevistas; R. Barsky, Noam Chomsky: una vida de discrepancia",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-siglo21"
  ]
 },
 "habermas": {
  "name": "Jürgen Habermas",
  "dates": "1929 – 2026",
  "born": 1929,
  "died": 2026,
  "place": "Düsseldorf (Alemania)",
  "role": "filósofo y sociólogo alemán",
  "idea": "La razón no debe abandonarse, sino hacerse dialógica: las normas y verdades se legitiman por el consenso alcanzado en un diálogo libre, igual y sin coacción.",
  "bio": "<p>Jürgen Habermas nació en Düsseldorf y pasó su adolescencia bajo el nazismo. Conocer tras la guerra los crímenes del régimen lo convenció de la importancia de la democracia y del diálogo público. Fue asistente de Adorno en el Instituto de Investigación Social de Fráncfort y es la gran figura de la segunda generación de la <strong>Escuela de Fráncfort</strong>. Enseñó en Heidelberg y Fráncfort y dirigió un instituto de la Sociedad Max Planck. En 2003 recibió el Premio Príncipe de Asturias de Ciencias Sociales.</p>\n<p>Habermas critica a Marx por reducirlo todo al trabajo: los seres humanos no solo producimos, también nos comunicamos. Frente a la <strong>razón instrumental</strong>, que solo calcula medios, propone una <strong>razón dialógica</strong> basada en la <strong>acción comunicativa</strong>, es decir, en el diálogo orientado a entenderse. En una <strong>comunidad ideal de habla</strong>, donde todos participan en igualdad y sin coacción, solo cuenta la fuerza del mejor argumento. De ahí su teoría consensual de la verdad y su defensa de una <strong>democracia deliberativa</strong> con una esfera pública libre, que el Estado debe proteger.</p>\n<p>Frente a los rupturistas posmodernos, como Lyotard, Foucault o Derrida, defendió que la modernidad es un proyecto inacabado: hay que corregir la razón, no renunciar a ella, porque sin razón no podemos criticar la injusticia. Mantuvo debates célebres con Rawls sobre la justicia y con Joseph Ratzinger sobre religión y democracia.</p>",
  "obras": [
   "Historia y crítica de la opinión pública (1962)",
   "Conocimiento e interés (1968)",
   "Teoría de la acción comunicativa (1981)",
   "El discurso filosófico de la modernidad (1985)"
  ],
  "anecdota": "<p>En 1953, con 24 años y recién doctorado, Habermas leyó la reedición de un curso de Heidegger de 1935, <em>Introducción a la metafísica</em>, donde seguía hablando de la «verdad interna y grandeza» del movimiento nacionalsocialista, sin una sola palabra de rectificación. Indignado, publicó un artículo en el <em>Frankfurter Allgemeine Zeitung</em> exigiendo explicaciones al filósofo más admirado de Alemania. Heidegger nunca respondió de verdad. Aquel texto anuncia al Habermas de toda su vida: un intelectual que interviene en el debate público y cree que ninguna autoridad está por encima de la discusión.</p>",
  "fuente": "J. Habermas, «Pensar con Heidegger contra Heidegger», Frankfurter Allgemeine Zeitung (1953)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-capitalismo",
   "hf-posmodernidad"
  ]
 },
 "wilson": {
  "name": "Edward O. Wilson",
  "dates": "1929 – 2021",
  "born": 1929,
  "died": 2021,
  "place": "Birmingham (Alabama, Estados Unidos)",
  "role": "biólogo y naturalista",
  "idea": "La conducta social tiene bases biológicas, y el éxito evolutivo de especies como la humana se explica en buena parte por la cooperación dentro de los grupos.",
  "bio": "<p>Edward O. Wilson nació en Birmingham (Alabama) y fue profesor en la Universidad de Harvard durante toda su carrera. Fue el mayor especialista mundial en hormigas y un pionero de la defensa de la <strong>biodiversidad</strong>. Ganó dos premios Pulitzer por sus libros de divulgación.</p>\n<p>En <em>Sociobiología</em> (1975) propuso estudiar las bases biológicas de la conducta social, también la humana, lo que provocó una fuerte polémica. En <em>La conquista social de la Tierra</em> (2012) sostuvo que el éxito de los insectos sociales y de los humanos se debe en gran parte a la <strong>cooperación</strong> dentro del grupo, una tesis discutida por otros biólogos. El temario lo cita junto a Kropotkin y Azurmendi frente al darwinismo social: la naturaleza humana no es solo competitiva.</p>",
  "obras": [
   "Sociobiología (1975)",
   "Sobre la naturaleza humana (1978)",
   "Las hormigas (1990, con B. Hölldobler)",
   "La conquista social de la Tierra (2012)"
  ],
  "anecdota": "<p>A los siete años, pescando en un muelle de Florida, el pequeño Edward tiró con demasiada fuerza del sedal y la aleta espinosa de un pez le hirió el ojo derecho, que acabó perdiendo casi por completo. Con el izquierdo, en cambio, veía de cerca con gran detalle. Él mismo contó que por eso dejó de fijarse en aves y mamíferos, difíciles de observar a distancia, y se centró en los seres pequeños que podía examinar sosteniéndolos cerca de la cara. Así empezó su pasión por las hormigas.</p>",
  "fuente": "E. O. Wilson, El naturalista (autobiografía, 1994)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-utilitarismo"
  ]
 },
 "txillardegi": {
  "name": "Txillardegi",
  "dates": "1929 – 2012",
  "born": 1929,
  "died": 2012,
  "place": "San Sebastián (Guipúzcoa)",
  "role": "lingüista, escritor y político vasco",
  "idea": "No pensamos antes de hablar, sino gracias a una lengua que estructura nuestra visión del mundo; por eso el euskera es la columna vertebral de la identidad vasca.",
  "bio": "<p>Txillardegi es el seudónimo de José Luis Álvarez Enparantza, nacido en San Sebastián en una familia castellanohablante. Estudió ingeniería industrial y aprendió euskera de joven. En los años cincuenta participó en el grupo Ekin y en 1959 fue uno de los fundadores de ETA, que abandonó en 1967 por discrepancias ideológicas. Vivió años en el exilio. Fue una figura clave en la creación del <strong>euskera batua</strong>, el euskera unificado, y más tarde profesor de sociolingüística en la Universidad del País Vasco y político de la izquierda abertzale.</p>\n<p>En sus ensayos sobre lengua y pensamiento siguió a Humboldt y a la <strong>hipótesis de Sapir-Whorf</strong>, según la cual la lengua condiciona el modo de pensar. Para Txillardegi el lenguaje no es un instrumento que usamos después de pensar: pensamos porque tenemos lenguaje, y la estructura de cada lengua organiza de manera inconsciente cómo percibimos la realidad. Cada lengua lleva consigo una <strong>cosmovisión</strong> (<em>Weltanschauung</em>). De ahí su defensa del euskera como núcleo de la identidad vasca: si una lengua desaparece, se pierde una forma propia de estructurar el mundo.</p>\n<p>También fue escritor: su novela <em>Leturiaren egunkari ezkutua</em> (1957), influida por el existencialismo, se considera la primera novela moderna en euskera. Su influencia en la normalización del euskera fue muy grande. En el temario completa la reflexión de Wittgenstein sobre el lenguaje con una perspectiva centrada en la relación entre lengua, pensamiento y comunidad.</p>",
  "obras": [
   "Leturiaren egunkari ezkutua (1957)",
   "Huntaz eta hartaz (1965)",
   "Hizkuntza eta pentsakera (1983)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-analitica"
  ]
 },
 "baudrillard": {
  "name": "Jean Baudrillard",
  "dates": "1929 – 2007",
  "born": 1929,
  "died": 2007,
  "place": "Reims (Francia)",
  "role": "sociólogo y filósofo francés",
  "idea": "Vivimos en la hiperrealidad: los simulacros, copias sin original, ya no representan la realidad, sino que la sustituyen y parecen más reales que ella.",
  "bio": "<p>Jean Baudrillard nació en Reims, en una familia de origen campesino. Fue profesor de alemán en institutos y tradujo a autores como Bertolt Brecht. Después se dedicó a la sociología y fue profesor en la Universidad de Nanterre desde 1966, en plena efervescencia de Mayo del 68. Partió del marxismo, pero pronto se alejó de él.</p>\n<p>En sus primeras obras analizó la <strong>sociedad de consumo</strong>: los objetos ya no se compran por su utilidad, sino como signos de estatus y de estilo de vida. Después radicalizó esa idea con la teoría del <strong>simulacro</strong>. Un simulacro es una copia sin original: una imagen que ya no remite a ninguna realidad, sino que la sustituye. El resultado es la <strong>hiperrealidad</strong>, un mundo en el que lo simulado parece más real que lo real, como ocurre con la publicidad, los parques temáticos, la televisión o los mundos virtuales. Evocando un cuento de Borges, dijo que hoy el mapa precede al territorio. Sus provocadores artículos sobre la guerra del Golfo de 1991 sostenían que el público solo conoció ese conflicto como espectáculo televisivo.</p>\n<p>Baudrillard continúa las reflexiones de Walter Benjamin sobre la reproductibilidad técnica y de Guy Debord sobre la sociedad del espectáculo. Es uno de los pensadores rupturistas de la posmodernidad, y sus ideas se han usado para pensar las redes sociales, los filtros y las noticias falsas.</p>",
  "obras": [
   "El sistema de los objetos (1968)",
   "La sociedad de consumo (1970)",
   "Cultura y simulacro (1981)",
   "La guerra del Golfo no ha tenido lugar (1991)"
  ],
  "anecdota": "<p>En la película <em>Matrix</em> (1999), el protagonista, Neo, esconde discos pirata en un ejemplar vaciado de <em>Cultura y simulacro</em>, el libro de Baudrillard. Los directores pidieron a los actores que lo leyeran antes del rodaje, e incluso invitaron al filósofo a colaborar en las secuelas. Baudrillard declinó. En una entrevista de 2004 explicó que la película lo había malinterpretado: <em>Matrix</em> separa claramente un mundo falso de uno real al que se puede escapar, mientras que para él lo inquietante es que esa frontera ya no existe.</p>",
  "fuente": "Película Matrix (1999); entrevista a Baudrillard en Le Nouvel Observateur (2004)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-posmodernidad"
  ]
 },
 "derrida": {
  "name": "Jacques Derrida",
  "dates": "1930 – 2004",
  "born": 1930,
  "died": 2004,
  "place": "El Biar (Argelia)",
  "role": "filósofo francés",
  "idea": "La deconstrucción muestra que los conceptos de la filosofía se apoyan en oposiciones jerárquicas ocultas y que ningún significado escapa por completo al juego del lenguaje.",
  "bio": "<p>Jacques Derrida nació en 1930 en El Biar, cerca de Argel, en una familia judía, cuando Argelia era colonia francesa. Estudió en la École Normale Supérieure de París, donde después enseñó durante años. En 1967 publicó tres libros que le dieron fama internacional, entre ellos <em>De la gramatología</em>. Fue profesor invitado en universidades de Estados Unidos, donde su influencia fue enorme. Murió en París en 2004.</p>\n<p>Derrida sostiene que no tenemos un acceso directo a las cosas: todo está mediado por el lenguaje y por los textos. Critica el <strong>logocentrismo</strong>, la creencia de la tradición occidental en un significado pleno y presente, anterior al lenguaje. Su método, la <strong>deconstrucción</strong>, analiza las parejas de conceptos con las que pensamos (habla y escritura, razón y emoción, masculino y femenino) y muestra que uno de los términos siempre se presenta como superior al otro. Desmontar esas jerarquías revela sus contradicciones internas y las relaciones de poder que esconden. Para nombrar el aplazamiento continuo del sentido acuñó el término <strong>différance</strong>.</p>\n<p>Heredero de Nietzsche y de Heidegger, Derrida es una figura central del pensamiento posmoderno junto a Foucault, Lyotard o Vattimo. Habermas criticó su posición porque, a su juicio, renuncia a una razón capaz de fundamentar la crítica social. Su influencia llega a la teoría literaria, el derecho, la arquitectura y los estudios de género.</p>",
  "obras": [
   "De la gramatología (1967)",
   "La escritura y la diferencia (1967)",
   "Márgenes de la filosofía (1972)",
   "Espectros de Marx (1993)"
  ],
  "anecdota": "<p>A finales de 1981 Derrida viajó a Praga para dar un seminario clandestino a intelectuales disidentes del régimen comunista checoslovaco. Al salir, la policía lo detuvo en el aeropuerto: afirmaron haber encontrado droga en su maleta, que en realidad habían colocado ellos mismos. Pasó una noche en la cárcel y fue acusado de tráfico de estupefacientes. La protesta del Gobierno francés, con el presidente Mitterrand al frente, logró su liberación y expulsión del país. El filósofo de la sospecha sobre los textos acabó siendo víctima de un expediente fabricado.</p>",
  "fuente": "Benoît Peeters, Derrida (biografía, 2010); prensa de la época",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-posmodernidad"
  ]
 },
 "vattimo": {
  "name": "Gianni Vattimo",
  "dates": "1936 – 2023",
  "born": 1936,
  "died": 2023,
  "place": "Turín (Italia)",
  "role": "filósofo italiano",
  "idea": "El pensamiento débil renuncia a las verdades absolutas y a los fundamentos últimos, y acepta la pluralidad de interpretaciones como base de una ética de la tolerancia.",
  "bio": "<p>Gianni Vattimo nació en Turín en 1936. Estudió filosofía en su ciudad con Luigi Pareyson y amplió estudios en Heidelberg con Hans-Georg Gadamer, cuya obra tradujo al italiano. Fue catedrático en la Universidad de Turín durante décadas y participó también en la política: fue diputado en el Parlamento Europeo. Murió en 2023.</p>\n<p>Vattimo es uno de los principales representantes de la <strong>hermenéutica</strong>, la filosofía de la interpretación. Siguiendo a Nietzsche y a Heidegger, sostiene que no hay hechos puros, sino interpretaciones. En la época posmoderna han caído los grandes relatos y los fundamentos últimos de la verdad. En lugar de lamentarlo, propone el <strong>pensamiento débil</strong>: una filosofía que renuncia a las verdades fuertes y definitivas, que suelen justificar la violencia y el autoritarismo. Aceptar la <strong>pluralidad de interpretaciones</strong> conduce a una ética de la tolerancia y del diálogo. En <em>La sociedad transparente</em> analizó cómo los medios de comunicación multiplican las visiones del mundo y debilitan la idea de una única realidad.</p>\n<p>Su propuesta forma parte del debate sobre la posmodernidad junto a Lyotard, Derrida o Foucault. Frente a Habermas, que quiere reconstruir una razón universal, Vattimo ve en el debilitamiento de la verdad una oportunidad emancipadora. En sus últimas obras reinterpretó el cristianismo desde esta misma perspectiva.</p>",
  "obras": [
   "El pensamiento débil (1983, con Pier Aldo Rovatti)",
   "El fin de la modernidad (1985)",
   "La sociedad transparente (1989)",
   "Creer que se cree (1996)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-posmodernidad"
  ]
 },
 "azurmendi": {
  "name": "Joxe Azurmendi",
  "dates": "1941 – 2025",
  "born": 1941,
  "died": 2025,
  "place": "Zegama (Gipuzkoa)",
  "role": "filósofo y ensayista vasco",
  "idea": "Los valores dependen del contexto histórico y cultural, pero eso no los vuelve arbitrarios: su relativismo relativo rechaza los absolutos sin caer en el nihilismo.",
  "bio": "<p>Joxe Azurmendi nació en Zegama en 1941. Se formó en el colegio franciscano de Arantzazu, ingresó en la orden y estudió Filosofía en Alemania y Teología en Roma. Desde los años sesenta fue uno de los impulsores del movimiento cultural en torno a la revista <em>Jakin</em>. En los años ochenta dejó la orden y fue profesor de la Facultad de Filosofía de la Universidad del País Vasco en San Sebastián. Escribió casi toda su obra en euskera. Murió en San Sebastián en 2025.</p>\n<p>Azurmendi rechaza los fundamentos absolutos de la ética moderna: ni Dios ni la razón pueden fundamentar una moral universal. Sin embargo, esto no significa que todo valga igual. Los valores son relativos a su contexto, pero dentro de cada comunidad adquieren una validez casi absoluta. Es su <strong>relativismo relativo</strong>, que exige juzgar caso por caso combinando, como Max Weber, la <strong>ética de la convicción</strong> y la <strong>ética de la responsabilidad</strong>. Además, critica el <strong>darwinismo social</strong> y, apoyándose en Kropotkin y en el biólogo Edward O. Wilson, subraya que la moral nace del sentido de comunidad y de la <strong>cooperación</strong>.</p>\n<p>Su obra dialoga con Kant, Nietzsche, Humboldt y el pensamiento alemán sobre el lenguaje y la nación. Es uno de los referentes del pensamiento vasco contemporáneo.</p>",
  "obras": [
   "Gizona abere hutsa da (1975)",
   "Espainolak eta euskaldunak (1992)",
   "Humboldt: hizkuntza eta pentsamendua (2007)",
   "Historia, arraza, nazioa (2014)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-utilitarismo",
   "hf-etica-deber"
  ]
 },
 "ingham": {
  "name": "Geoffrey Ingham",
  "dates": "n. 1942",
  "born": 1942,
  "died": null,
  "place": "",
  "role": "sociólogo y economista político británico",
  "idea": "El dinero no es una simple mercancía, sino una promesa de pago creada socialmente, sobre todo como crédito bancario, y es un pilar del capitalismo.",
  "bio": "<p>Geoffrey Ingham es un sociólogo británico nacido en 1942. Estudió Sociología en la Universidad de Leicester y se doctoró en Cambridge, donde ha sido profesor de Sociología y Economía Política y miembro del Christ's College. Es profesor emérito de esa universidad.</p>\n<p>Su trabajo se centra en la naturaleza del dinero y del capitalismo. En <em>The Nature of Money</em> (2004) defiende que el dinero es una <strong>construcción social y política</strong>: una promesa de pago, no un objeto con valor propio. En <em>Capitalism</em> (2008) sostiene que el capitalismo mantiene tres elementos en todas sus fases: un sistema monetario que crea dinero como <strong>crédito bancario</strong>, el intercambio de mercado y la empresa privada que produce mercancías. Aparece en el temario para explicar qué permanece en el capitalismo a lo largo de su historia.</p>",
  "obras": [
   "The Nature of Money (2004)",
   "Capitalism (2008)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-utilitarismo"
  ]
 },
 "nussbaum": {
  "name": "Martha Nussbaum",
  "dates": "n. 1947",
  "born": 1947,
  "died": null,
  "place": "Nueva York (EE. UU.)",
  "role": "filósofa estadounidense",
  "idea": "La justicia no se mide por la riqueza de un país, sino por las capacidades reales que cada persona tiene para llevar una vida digna.",
  "bio": "<p>Martha Nussbaum nació en Nueva York en 1947. Se doctoró en la Universidad de Harvard con una tesis sobre Aristóteles y ha enseñado en Harvard, en Brown y, desde los años noventa, en la Universidad de Chicago, donde une filosofía y derecho. Especialista en filosofía griega, ha escrito sobre ética, emociones, educación y justicia. En 2012 recibió el Premio Príncipe de Asturias de Ciencias Sociales.</p>\n<p>Junto al economista Amartya Sen desarrolló el <strong>enfoque de las capacidades</strong>. Frente a la costumbre de medir el progreso por el PIB, sostiene que hay que preguntarse qué puede hacer y ser realmente cada persona. Propone una lista de <strong>capacidades centrales</strong>: vida, salud, integridad física, sentidos e imaginación, emociones, razón práctica, afiliación, relación con otras especies, juego y control sobre el propio entorno. Una sociedad es justa si garantiza a todos un mínimo de esas capacidades. Presta especial atención a las mujeres de los países pobres, a las personas con discapacidad y a los animales. También defiende el valor de las <strong>humanidades</strong> para formar ciudadanos críticos.</p>\n<p>Su pensamiento combina la ética de Aristóteles, centrada en la vida buena, con la tradición liberal. Dialoga críticamente con la teoría de la justicia de John Rawls, a la que reprocha dejar fuera a quienes no pueden participar en el contrato social. Es una referencia en los debates sobre desarrollo humano.</p>",
  "obras": [
   "La fragilidad del bien (1986)",
   "Las mujeres y el desarrollo humano (2000)",
   "Sin fines de lucro (2010)",
   "Crear capacidades (2011)"
  ],
  "anecdota": "<p>Antes de dedicarse a la filosofía, la joven Martha quiso ser actriz. Dejó sus estudios universitarios para trabajar durante un tiempo en una compañía de teatro de repertorio que recorría el país representando obras clásicas. Después volvió a la universidad y se centró en la lengua y la literatura griegas, en la Universidad de Nueva York. Aquella pasión por la tragedia no desapareció: en <em>La fragilidad del bien</em> estudia a Sófocles y Eurípides para mostrar que la literatura también enseña a pensar la vida moral.</p>",
  "fuente": "Perfil de R. Aviv en The New Yorker (2016) y entrevistas",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-siglo21"
  ]
 },
 "butler": {
  "name": "Judith Butler",
  "dates": "n. 1956",
  "born": 1956,
  "died": null,
  "place": "Cleveland (Ohio, EE. UU.)",
  "role": "referente de la teoría queer",
  "idea": "El género no es una esencia biológica, sino una performance: se produce al repetir gestos y normas, y por eso puede transformarse y subvertirse.",
  "bio": "<p>Judith Butler nació en Cleveland (Estados Unidos) en 1956. Se doctoró en Filosofía en la Universidad de Yale con un estudio sobre Hegel y la filosofía francesa. Durante décadas ha enseñado Literatura Comparada en la Universidad de California en Berkeley. Su libro <em>El género en disputa</em> (1990) le dio fama internacional y es uno de los textos fundadores de la <strong>teoría queer</strong>.</p>\n<p>Butler sostiene que el género no es algo que se es, sino algo que se hace. Es una <strong>performance</strong> o, más exactamente, un acto performativo: se produce al repetir gestos, formas de vestir y de hablar según las normas sociales. Esa repetición crea la apariencia de una identidad natural y fija. Butler va más allá de la distinción clásica entre sexo biológico y género cultural, pues considera que también nuestra forma de entender el sexo está atravesada por normas. Si el género es un guion que se repite, puede repetirse de otra manera: es posible <strong>subvertirlo</strong>. En obras posteriores ha reflexionado sobre la <strong>vulnerabilidad</strong>, el duelo y la no violencia.</p>\n<p>Su pensamiento parte de Simone de Beauvoir, a quien reinterpreta, y se apoya en Foucault, Derrida y el psicoanálisis. Ha influido en el feminismo, los movimientos LGTBI y pensadores como Paul B. Preciado. También ha recibido críticas desde otras corrientes feministas.</p>",
  "obras": [
   "El género en disputa (1990)",
   "Cuerpos que importan (1993)",
   "Vida precaria (2004)",
   "La fuerza de la no violencia (2020)"
  ],
  "anecdota": "<p>Butler ha contado que, con unos catorce años, hablaba tanto en las clases de la escuela hebrea de su sinagoga en Cleveland que el rabino decidió imponerle como castigo unas tutorías individuales. El supuesto castigo resultó un regalo: en aquellas sesiones de ética judía el rabino le propuso leer y discutir a filósofos como Spinoza y otros pensadores de la tradición. Allí empezó su interés por la filosofía y por preguntas que nunca ha abandonado, como la relación entre la ética, la violencia y la comunidad.</p>",
  "fuente": "Testimonio de Judith Butler en entrevistas",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-siglo21"
  ]
 },
 "han": {
  "name": "Byung-Chul Han",
  "dates": "n. 1959",
  "born": 1959,
  "died": null,
  "place": "Seúl (Corea del Sur)",
  "role": "filósofo surcoreano-alemán",
  "idea": "En la sociedad del rendimiento ya no nos explota un amo exterior: nos autoexplotamos creyéndonos libres, y el resultado es el cansancio y la depresión.",
  "bio": "<p>Byung-Chul Han nació en Seúl en 1959. Estudió Metalurgia en Corea, pero en los años ochenta se trasladó a Alemania para estudiar filosofía, literatura y teología en Friburgo y Múnich. Se doctoró con una tesis sobre Heidegger y ha sido profesor en Karlsruhe y en la Universidad de las Artes de Berlín. Escribe en alemán ensayos breves que se han traducido a muchas lenguas.</p>\n<p>En <em>La sociedad del cansancio</em> (2010) sostiene que hemos pasado de la <strong>sociedad disciplinaria</strong>, descrita por Foucault, basada en prohibiciones y en el control externo, a la <strong>sociedad del rendimiento</strong>. En ella nadie nos obliga desde fuera: nos exigimos a nosotros mismos bajo el lema de que todo es posible. El sujeto se convierte en empresario de sí mismo y se <strong>autoexplota</strong> creyendo que es libre. Cuando no alcanza sus metas, no se rebela contra el sistema, sino que se culpa a sí mismo. De ahí la epidemia de agotamiento, ansiedad y depresión. Han también ha analizado la <strong>transparencia</strong> y el control a través de los datos digitales.</p>\n<p>Su obra dialoga con Foucault, Heidegger y la tradición crítica de la Escuela de Fráncfort. Se le considera uno de los pensadores más leídos sobre el mundo digital y el capitalismo actual.</p>",
  "obras": [
   "La sociedad del cansancio (2010)",
   "La sociedad de la transparencia (2012)",
   "Psicopolítica (2014)",
   "La expulsión de lo distinto (2016)"
  ],
  "anecdota": "<p>El crítico de la vida digital prefiere la tierra a las pantallas. Durante tres primaveras y tres veranos, Han cultivó un jardín en Berlín y lo convirtió en libro: <em>Loa a la tierra</em> (2018). Cuenta que aprendió los nombres de las flores, que plantaba incluso en invierno y que el jardín le enseñó a esperar, a cuidar y a aceptar ritmos que no se pueden acelerar. Frente a la prisa y la autoexplotación que describe en sus ensayos, trabajar con las manos en la tierra le parece una forma de recuperar el tiempo.</p>",
  "fuente": "Byung-Chul Han, Loa a la tierra. Un viaje al jardín (2018)",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-siglo21"
  ]
 },
 "herrero": {
  "name": "Yayo Herrero",
  "dates": "n. 1965",
  "born": 1965,
  "died": null,
  "place": "Madrid",
  "role": "antropóloga y activista ecofeminista",
  "idea": "Los seres humanos somos ecodependientes e interdependientes, así que la economía y la política deben poner en el centro la vida y los cuidados.",
  "bio": "<p>Yayo Herrero nació en Madrid en 1965. Es ingeniera técnica agrícola, antropóloga y educadora social, y se doctoró en Ciencias Sociales y Políticas por la Universidad del País Vasco. Fue coordinadora estatal de Ecologistas en Acción y ha trabajado como investigadora y docente en ecología política, ecofeminismo y educación para la sostenibilidad. Es una de las voces más conocidas del ecologismo en España.</p>\n<p>Desde el <strong>ecofeminismo</strong>, que relaciona la crisis ecológica con la desigualdad de las mujeres, Herrero recuerda dos hechos básicos. Somos <strong>ecodependientes</strong>, porque dependemos de la naturaleza para vivir, e <strong>interdependientes</strong>, porque necesitamos los cuidados de otras personas a lo largo de la vida. Nuestra economía olvida ambas cosas: crece como si el planeta fuera infinito y no valora los <strong>cuidados</strong>, realizados sobre todo por mujeres y de forma invisible. Por eso afirma que el sistema actual está enfrentado a la vida. Propone poner la vida y los cuidados en el centro de la política y la economía, y reconocer la vulnerabilidad humana.</p>\n<p>Su pensamiento se vincula con la ética del cuidado, con el decrecimiento y con otras críticas al capitalismo actual. Comparte con Martha Nussbaum la idea de que el bienestar no se mide por el crecimiento económico.</p>",
  "obras": [
   "Los cinco elementos (2021)",
   "Ausencias y extravíos (2021)",
   "Toma de tierra (2023)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-siglo21"
  ]
 },
 "chalmers": {
  "name": "David Chalmers",
  "dates": "n. 1966",
  "born": 1966,
  "died": null,
  "place": "Sídney (Australia)",
  "role": "filósofo de la mente australiano",
  "idea": "Explicar las funciones del cerebro no basta para explicar la experiencia subjetiva: por qué sentimos algo es el problema difícil de la conciencia.",
  "bio": "<p>David Chalmers nació en Sídney en 1966. Estudió matemáticas y después se doctoró en Filosofía y Ciencia Cognitiva en la Universidad de Indiana. Ha enseñado en Australia y en Estados Unidos, sobre todo en la Universidad de Nueva York. Es uno de los filósofos de la mente más citados de la actualidad.</p>\n<p>En los años noventa formuló el <strong>problema difícil de la conciencia</strong>. La ciencia puede explicar cómo el cerebro procesa información, pero no por qué esos procesos van acompañados de experiencia subjetiva: qué se siente al ver un color o sentir dolor. Chalmers defiende un <strong>dualismo</strong> de propiedades: la conciencia no se reduce a lo físico. Por eso aparece en el temario junto a Descartes, cuyo dualismo de alma y cuerpo reabre en un lenguaje contemporáneo.</p>",
  "obras": [
   "La mente consciente (1996)",
   "Reality+ (2022)"
  ],
  "anecdota": "<p>En 1998, tras un congreso en Bremen, el neurocientífico Christof Koch apostó con Chalmers una caja de buen vino: Koch creía que en veinticinco años se habría encontrado en el cerebro la señal neuronal que explica la conciencia; Chalmers, que no. En 2023, en un congreso sobre la conciencia celebrado en Nueva York, se revisó la apuesta. Los experimentos no habían resuelto la cuestión, así que Koch reconoció la derrota y pagó. Chalmers no ganó por demostrar su dualismo, sino porque el problema difícil seguía abierto.</p>",
  "fuente": "Crónicas del congreso de la Association for the Scientific Study of Consciousness (Nueva York, 2023), entre ellas la de Nature",
  "tradicion": false,
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-descartes-makro"
  ]
 },
 "klein": {
  "name": "Naomi Klein",
  "dates": "n. 1970",
  "born": 1970,
  "died": null,
  "place": "Montreal (Canadá)",
  "role": "periodista y ensayista canadiense",
  "idea": "El capitalismo del desastre aprovecha el shock de las crisis, guerras y catástrofes para imponer reformas neoliberales que en tiempos normales serían rechazadas.",
  "bio": "<p>Naomi Klein nació en Montreal en 1970. Es periodista, ensayista y activista. Se dio a conocer con <em>No Logo</em> (2000), que se convirtió en un libro de referencia del movimiento antiglobalización de aquellos años. Ha escrito en numerosos medios internacionales y es profesora en la Universidad de la Columbia Británica, en Canadá.</p>\n<p>En <em>No Logo</em> analiza el poder de las grandes <strong>marcas</strong>: ya no venden solo objetos, sino identidades y estilos de vida, mientras trasladan la producción a países con salarios bajos. En <em>La doctrina del shock</em> (2007) describe el <strong>capitalismo del desastre</strong>. Según Klein, gobiernos y grandes corporaciones aprovechan el desconcierto que provocan las crisis, como guerras, golpes de Estado o catástrofes naturales, para imponer <strong>reformas neoliberales</strong>: privatizaciones, recortes y desregulación. La población, en estado de shock, no puede oponerse. En <em>Esto lo cambia todo</em> (2014) relaciona la crisis climática con el modelo económico basado en el crecimiento ilimitado.</p>\n<p>Su obra forma parte de la crítica contemporánea a la globalización neoliberal, junto a autores como Noam Chomsky y Zygmunt Bauman. Su análisis de la crisis climática la acerca también a pensadoras ecologistas como Yayo Herrero.</p>",
  "obras": [
   "No Logo (2000)",
   "La doctrina del shock (2007)",
   "Esto lo cambia todo (2014)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-siglo21"
  ]
 },
 "preciado": {
  "name": "Paul B. Preciado",
  "dates": "n. 1970",
  "born": 1970,
  "died": null,
  "place": "Burgos",
  "role": "filósofo y comisario de arte",
  "idea": "En la era farmacopornográfica el capitalismo gobierna los cuerpos mediante fármacos, hormonas e imágenes, pero el cuerpo puede convertirse en espacio de resistencia.",
  "bio": "<p>Paul B. Preciado nació en Burgos en 1970. Estudió filosofía en Madrid y en la New School for Social Research de Nueva York, donde fue alumno de Jacques Derrida, y se doctoró en Teoría de la Arquitectura en la Universidad de Princeton. Ha sido profesor, comisario de exposiciones de arte y columnista, y vive en París. En 2015 hizo pública su transición de género y adoptó el nombre de Paul.</p>\n<p>En <em>Manifiesto contrasexual</em> (2000) critica que el sexo y el género se presenten como naturales. En <em>Testo yonqui</em> (2008), escrito mientras se administraba testosterona, describe la <strong>era farmacopornográfica</strong>. Según Preciado, desde mediados del siglo XX el capitalismo ya no controla los cuerpos solo con disciplinas externas, sino desde dentro: mediante <strong>fármacos y hormonas</strong>, como la píldora anticonceptiva, y mediante la industria de las imágenes y la pornografía. La identidad sexual se fabrica técnicamente. Por eso el cuerpo puede convertirse en un espacio de <strong>resistencia política</strong>, donde se experimentan identidades que rompen el binarismo hombre-mujer.</p>\n<p>Su pensamiento continúa el análisis del biopoder de Foucault y la teoría de la performatividad de Judith Butler. Es una de las voces más influyentes de la <strong>teoría queer</strong> y de los movimientos trans. En 2023 dirigió la película documental <em>Orlando, mi biografía política</em>.</p>",
  "obras": [
   "Manifiesto contrasexual (2000)",
   "Testo yonqui (2008)",
   "Un apartamento en Urano (2019)",
   "Yo soy el monstruo que os habla (2020)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-siglo21"
  ]
 },
 "bostrom": {
  "name": "Nick Bostrom",
  "dates": "n. 1973",
  "born": 1973,
  "died": null,
  "place": "Helsingborg (Suecia)",
  "role": "filósofo sueco",
  "idea": "Si las civilizaciones avanzadas pueden simular mundos con seres conscientes y deciden hacerlo, es probable que nosotros mismos vivamos en una simulación informática.",
  "bio": "<p>Nick Bostrom nació en Helsingborg (Suecia) en 1973. Estudió filosofía, física y neurociencia computacional, y se doctoró en la London School of Economics. Fue profesor en la Universidad de Oxford, donde dirigió el Instituto para el Futuro de la Humanidad, dedicado a los riesgos que amenazan a la especie humana.</p>\n<p>En 2003 formuló el <strong>argumento de la simulación</strong>. Según él, al menos una de estas tres posibilidades es cierta: casi ninguna civilización llega a una tecnología capaz de simular mentes conscientes; las que llegan no quieren hacerlo; o casi seguro que vivimos en una <strong>simulación</strong>. Aparece en el temario porque su hipótesis recuerda al <strong>genio maligno</strong> de Descartes. También es conocido por sus estudios sobre los riesgos de la inteligencia artificial.</p>",
  "obras": [
   "¿Vives en una simulación por ordenador? (2003)",
   "Superinteligencia (2014)"
  ],
  "block": "con",
  "subjects": [
   "hf"
  ],
  "temas": [
   "hf-descartes-makro"
  ]
 }
};
